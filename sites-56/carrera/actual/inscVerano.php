...
$q = "insert into participantes (idevento, atleta, nombre, apellidos, sexo, fnac, direccion, localidad, cp, telf, correo,
		...
		"(9, 'S', '".$d['nomb']."', '".$d['ape']."', 'F', '".$feArr[2]."-".$feArr[1]."-".$feArr[0]."', '".$d['direc']."', '".$d['loc']."', '".$d['cp']."', '".$d['tel']."', '".$d['correo'].
			"', '".$d['cole']."', '".$d['dni']."', '".$d['cuota']."', '".$d['club']."', '".$d['obs']."', '".$d['catego']."', ".time().")";
//echo $q;
$conn->execute($q);
$idPart = $conn->lastId();
//		echo $q;
...
$mensaje = "Tu solicitud de inscripción ha sido recibida correctamente, en los próximos días recibirás un correo del Club de Atletismo Santutxu para aceptar tu inscripción en el Campus de Verano de la Escuela de ".
