...
// 	$_REQUEST['TransactionType'] = '1';
// 	$_REQUEST['TransactionName'] = 'AutorizaciÃ³n';
// 	$_REQUEST['CardCountry'] = '';
// 	$_REQUEST['BankDateTime'] = '20170330164924';
// 	$_REQUEST['Signature'] = 'b5ff1a37b93e28c0d967a76fdaaaa065';
// 	$_REQUEST['Order'] = '170330164336';
...
//    $d = $_REQUEST;
	...
	$cojin = " and estado = 'A'";
    if ($d['Ds_Code'] == 100 ) {// operación Aceptada
...
//		error_log("<br>\ncambioRateCimex=".$cambioRate);
    ...
    if (($est == "A" || $est == "B" || $est == "V") && $pasarela != 3 ) {
		...
		$query .= " estado = '$estado', fecha_mod = ".time()." where idtransaccion = '$idtrans' and estado in ('P', 'D')";
		//fwrite($handle, "<br>\n".$query);
		$correoMi .=  "<br>\n".$query;
		$temp->query($query);
        ...
        $query = "update tbl_reserva set id_transaccion = '".$idtrans."', bankId = '".$codautorizacion."', fechaPagada = ".$temp->f('fecha_mod').",
                    ...
                    where codigo = '".$temp->f('identificador')."' and estado in ('P', 'D') and id_comercio = ".$temp->f('idcomercio');
    //	echo $query;
        $temp->query($query);
        $correoMi .=  "<br>\n".$query."\n<br><br>\n";
	}
	...
	$query = "select idtransaccion, t.idcomercio, identificador, codigo, idioma, fecha_mod, valor, moneda, t.estado, c.nombre, 
				c.url, t.tipoEntorno, t.valor/100 precio, c.url_llegada, p.nombre, p.comercio, 
	...
	//			$correoMi .=  "respuCurl=".str_replace('<script', '<scr|ipt', $output)."||<br>\n";
				...
				$correoMi .= "Funcionalidad de Cubana y Saratoga y H Nacional<br>\n";
				$data = array(
						"comercio"		=> $valores[1],
