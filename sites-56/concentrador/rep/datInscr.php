...
//			}
			...
			if ($can > 0) {
				...
				$q = "insert into tbl_aisCliente (idcimex, nombre, papellido, sapellido, fnacimiento, numDocumento, fechaDocumento, correo, 
							telf1, paisResidencia, provincia, ciudad, direccion, CP, paisNacimiento, sexo, ocupacion, salariomensual, 
		...
		$q = "update tbl_aisCliente set nombre = '$nombre', papellido = '$ape1', sapellido = '$ape2', numDocumento = '$numid', 
					...
					paisNacimiento = '$paisNace', salariomensual = '$sal', fecha = '".time()."', fnacimiento = '".$fechaNac."',
					subfichero = 1
				where idcimex = '$id'";
		$pase = false;
	}
...
function enviaDato($data) {
	...
	while (strlen($salidaCurl) == 0 && $i < 4) {
		...
		$correoMi .= "Enviado a TefPay, ".date('d-m-y H:i:s:u')." envío - $i - ".htmlspecialchars_decode($salidaCurl)." |<br>\n";
		$i++;
		sleep(10);
	}
	curl_close($ch);
