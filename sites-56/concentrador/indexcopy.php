...
//$d = array("comercio"=>$PPTcomercio, "transaccion"=>$PPTtransaccion, "importe"=>$PPTimporte, "moneda"=>$PPTmoneda, "operacion"=>$PPToperacion, "idioma"=>$PPTidioma, "firma"=>$PPTfirma, "pasarela"=>"12");
/****************************comentar*************************************************/
...
/******************************* Integración por tipo de pasarela *************************************/
		...
		$q = "select t.nombre, dato1, dato4, p.datos, t.comercio from tbl_terminal t, tbl_pasarela p "
				. "where p.idPasarela = t.idpasarela and p.activo = 1 and t.idpasarela = $pasarela and t.idmoneda = '$moneda'";
		$temp->query($q);
		$correoMi .= "query= " . $q . "<br>\n";
		...
		if ($temp->num_rows() > 0) {
			...
			if ($variante = 'redsys') {
				...
				if (_MOS_CONFIG_DEBUG) echo "<br>" . $query;
				$correoMi .= "query= " . $query . "<br>\n";
				$temp->query($query);
				if ($temp->getErrorMsg()) $correoMi .= $temp->getErrorMsg()."<br>\n";
...
//				$moneda1 = '0' . $moneda;
	...
	//			if (_MOS_CONFIG_DEBUG) $idi = '002';
				...
				if (_MOS_CONFIG_DEBUG) echo "idioma=$idioma<br>";
				if (_MOS_CONFIG_DEBUG) echo "$importe . $trans . $idCom . $moneda . $tipoTrans . $urlcomercio . $clave<br>";
				...
				if (_MOS_CONFIG_DEBUG) echo "tira=$message<br>";
				
				if ($pasarela == 23 ) $Digest = sha1($message);
				else $Digest = strtoupper(sha1($message));
				...
				if (_MOS_CONFIG_DEBUG) $cadenSal .= '<input type="submit" /></form>';
				else $cadenSal .= '</form><script language=\'javascript\'>document.envia.submit();</script>';
				
				$correoMi .= "cadenSal= " . $cadenSal . "<br>\n";
				echo $cadenSal;
			}
//			print_r($arrDat);
		} //else muestraError ("falla por pasarela - moneda", $correoMi);
...
//                $message = $importe . $trans . $idCom . $moneda . $clave;
			...
			if ($pasarela == 21) { // Caixa 3D
				...
				else if ($moneda == '392') $terminal = '23'; //yenes
...
//                $message = $importe . $trans . $idCom . $moneda . $clave;
			...
			} else if ($pasarela == 38) {	//Caixa Bank
				$urlPasarela = "https://sis-t.redsys.es:25443/sis/realizarPago";
				if ($moneda == '978') {$terminal = '1'; $clave = 'qwertyasdf0123456789'; $idCom = '331000810';} 
				elseif ($moneda == '840') {$terminal = '1'; $clave = 'qwertyasdf0123456789'; $idCom = '333006054';} 
				elseif ($moneda == '826') {$terminal = '1'; $clave = 'qwertyasdf0123456789'; $idCom = '333006187';} 
                else {
					muestraError ("falla por moneda en Caixa Bank", $correoMi);
                }
...
//				$urlPasarela = 'https://sis-t.redsys.es:25443/sis/realizarPago';
                ...
                else if		($moneda == '392') {$terminal = '008'; $clave = '9T478O740NP1P45Q';} //JPY
