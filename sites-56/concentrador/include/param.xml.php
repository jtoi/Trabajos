...
*/
	 ...
	 */
	function toArray() {
		return $this->mosObjectToArray( $this->_params );
	}

	function renderSalida($key, $value='') {
		return str_replace($key.'='.$this->get($key), $key.'='.$value, $this->_raw);
	}
	...
	*/
	function _form_mos_category( $name, $value, &$node, $control_name ) {
		...
		if( $scope== 'content' ) {
			...
			. "\n LEFT JOIN tbl_sections AS s ON s.id=c.section"
			. "\n WHERE c.published = 1"
