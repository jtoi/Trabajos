...
// 						$this->pasa = $this->temp->f ( 'idPasarela' );
						...
						$this->db("select idpasarela from tbl_rotComPas where idcom = ".$this->datCom['id']." and tipo = 0 and activo = 1 and orden = 1");
						if ($this->temp->num_rows()) {
							$this->pasa = $this->temp->f('idpasarela');
						} else {
...
// 						$q = "select idPasarela from tbl_pasarela p, tbl_colComerPasar c where c.idpasarelaW = p.idPasarela and 
// 									p.secure = (select secure from tbl_pasarela where idPasarela = " . $this->pasa . ") and c.idcomercio = '{$this->comer}'
// 									and " . time () . " between c.fechaIni and c.fechaFin";
// 						$this->db ( $q );
// 						$this->pasa = $this->temp->f ( 'idPasarela' );
				...
				$this->db ( "select id from tbl_rotComPas where idcom = " . $this->datCom ['id'] . " and tipo = 0 and activo = 1 and horas != 0" );
				...
				if ($numpas > 0) {
					...
					$this->db ( "select idpasarela from tbl_rotComPas where idcom = " . $this->datCom ['id'] . " and tipo = 0 and activo = 1 and fecha > " . time () );
					if ($this->temp->num_rows () > 0) {
						$this->pasa = $this->temp->f ( 'idpasarela' );
					} else { // es tiempo de cambio de pasarela

						$this->db("select count(id) cant from tbl_rotComPas where idcom = ".$this->datCom['id']." and activo = 1 and tipo = 0");
						$cantTo = $this->temp->f('cant');

						$this->log .= "/**********************/<br>select id, horas, idpasarela from tbl_rotComPas where idcom = ".$this->datCom ['id']." and tipo = 0 and activo = 1 and orden = (select case when orden = $cantTo then 1 else orden+1 end from tbl_rotComPas r, tbl_comercio c where c.id = r.idcom and r.idpasarela = c.pasaRot and c.id = ".$this->datCom ['id'].")\n<br>/**********************/<br>";

						$verpos = leeSetup('ordenCub')+1;
						if ($verpos>$cantTo) $verpos = 1;
						actSetup($verpos,'ordenCub');
						
						$this->db ( "select id, horas, idpasarela from tbl_rotComPas where idcom = " . $this->datCom ['id'] . " and tipo = 0 and activo = 1 " . "and orden = ($verpos)" );
		...
		if ($this->pasa != 1) { //Si la pasarela es la de prueba salta todo
			...
			if ($this->comer != '122327460662') {// si no es Prueba
				if ($this->segura == 0) {
					$this->pasa = 44; // si la pasarela es no segura pone a sabadell3 de primero
				} else {
					if ($this->pweb == 0) {//si no es pago desde web busco el array 
						...
						$this->db("select idPasarela from tbl_rotComPas where idcom = " . $this->datCom ['id'] . " and activo = 1 and tipo = 0 and horas = 0 order by orden");
						if ($this->temp->num_rows() > 0) {
...
// 			$this->log = '';
		...
		if (isset ( $datPas ['secure'] ) && $datPas ['secure'] == 0) {
			if ($this->pasa != 44 && $this->comer != '122327460662') { // Si la pasarela no es Sabadel
				$this->pasa = 44;
				// if ($this->pasa != 20) {
				// if ($this->pasa != 20) {
				// $this->pasa = 20;
			...
			// if ($this->comer != '411691546810') { //quito a Soy Cubano de buscar las operaciones anteriores
			// if ($this->segura == 0 && $verif == true) {
			// $q = "select pasarela from tbl_transacciones where idcomercio = ".$this->comer." order by fecha desc limit 0,1";
			// $this->db($q);
			// switch ($this->temp->f('pasarela')) {
			// case '44': // Si va por Sabadell3
			// $this->pasa = 53; //pasa por Abanca2
			// break;
			// case '53':// Si va por Abanca2
			// $this->pasa = 52;// pasa por Bankia6
			// break;
			// case '52':// Si va por Bankia6
			// $this->pasa = 56;// pasa por La Caixa3
			// break;
			// // case '44':
			// // $this->pasa = 53;
			// // break;
			// default:
			// $this->pasa = 44; //Si no está en ninguna pasa por Sabadell3
			// break;
			// $this->log .= "La anterior fué por ".$this->temp->f('pasarela')." y pasó para ".$this->pasa."<br>\n";
			// }
			// }
			// }
...
// 		$this->log .= "<br>select count(idtransaccion) total from tbl_transacciones where idcomercio = '". $this->comer ."' and estado = 'A' and pasarela = ". $this->pasa ." and fecha > ".(time()-60*60*6);
		$this->db("select count(idtransaccion) total from tbl_transacciones where idcomercio = '". $this->comer ."' and estado = 'A' and pasarela = ". $this->pasa ." and  fecha > ".(time()-60*60*6));
		...
		if ($this->comer != '122327460662') {
			if ($this->pasa == 44 && ( // Si es de Sabadell3 y además de alguna de estas IP
		...
		if ($this->comer != '122327460662') {
			if ($this->pasa == 53 && ( // Si es de Abanca2 y además de alguna de estas IP
				$this->ip != '200.55.188.130' 		// Hotel Nacional
		...
		// Chequeo de límites trimestrales para Cimex
		if ($this->comer == '527341458854') { // Si el comercio es Cimex verifico que no rebase el límite de 3000 en 90 días para el cliente
			...
			$q = "select sum(o.recibe) total from tbl_aisOrden o, tbl_transacciones t, tbl_aisCliente c
			...
			$this->db ( $q );
			$this->log .= $this->temp->f ( 'total' ) . " + " . $this->imp . " = " . ($this->temp->f ( 'total' ) + $this->imp) . " >= " . ($vlim * 100) . "\n<br>";
			
			if (($this->temp->f ( 'total' ) + $this->imp) >= ($vlim * 100)) {
				$this->err = $this->log .= $causa = "El remitente tiene un acumulado de " . number_format ( $this->temp->f ( 'total' ) / 100, 2 ) . " con esta operación de " . number_format ( $this->imp / 100, 2 ) . " llega al tope que es de $vlim para un trimestre natural\n<br>";
				$pase = 0;
			} else {
				$q = "select sum(o.recibe) total from tbl_aisOrden o, tbl_transacciones t, tbl_aisBeneficiario c
					where o.idtransaccion = t.idtransaccion and o.idcliente = c.id and t.estado = 'A'
						and t.fecha between $fec1Ais and $fec2Ais and c.idcimex = " . $this->datAis ['iddestin'];
				$this->db ( $q );
				$this->log .= $this->temp->f ( 'total' ) . " + " . $this->imp . " = " . ($this->temp->f ( 'total' ) + $this->imp) . " >= " . ($vlim * 100) . "\n<br>";
				if (($this->temp->f ( 'total' ) + $this->imp) >= ($vlim * 100)) {
					$this->err = $this->log .= $causa = "El beneficiario tiene un acumulado de " . number_format ( $this->temp->f ( 'total' ) / 100, 2 ) . " con esta operación de " . number_format ( $this->imp / 100, 2 ) . " llega al tope que es de $vlim para un trimestre natural\n<br>";
					$pase = 0;
				}
			}
		}
		
		//Restricción de uso de Abanca para las ip descritas solamente
// 		if ($this->comer != '122327460662') {
// 			if ($this->pasa == 68 && ( // Si es de Abanca3 y además no es de alguna de estas IP
// 				$this->ip != '200.55.188.130' && // Hotel Nacional
// 				$this->ip != '200.68.72.41' && // Havanatur Argentina
// 				$this->ip != '190.179.230.61' && // Outdoor Argentina
// 				$this->ip != '190.6.92.42') // Hotel Saratoga
// 			) {
// 				$this->log .= $causa = "Cambio de pasarela obligado Abanca3\n<br>";
// 				$pase = 0;
// 			}
// 		}
		...
		// // )
		// ) {
		// $this->log .= $causa = "Cambio de pasarela obligado Caixa3\n<br>";
		// $pase = 0;
		// }
		
// 		if ($this->comer != '122327460662') {
// 			if ($this->pasa == 52) { // si es la Bankia6
// 				if ($this->comer == '411691546810' 		//Soy Cubano
// 					|| $this->comer == '145918467582' 	// FCBC
// 					|| $this->ip == '190.179.230.61' 	// Outdoor Argentina
// 				) {
// 					$this->log .= $causa = "Cambio de pasarela obligado Bankia6\n<br>";
// 					$pase = 0;
// 				}
// 			}
// 		}

		//Chequeo del correo del cliente que exista para las pasarelas Navarrap
		if ($pase) {
			if (
					$this->pasa == 71
					|| $this->pasa == 72
		...
		$q = "select nombre, LimMinOper, LimMaxOper, LimDiar, LimMens, LimAnual, LimOperIpDia, LimOperTarDia, LimOperDia";
		if ($verif)
			$q .= ", secure";
		...
		if ($this->comer != '129025985109') // Para todos los comercios excepto Cubana
			// && $this->comer != '135334103888' //y el H. Nacional
...
// 		$pase = 0;
		...
		if ($pase == 0 && $this->comer != '122327460662') { // si ha saltado algún límite.. y el comercio no es Prueba y la pasarela no es prueba
...
//					$this->err = "Alcanzado n&uacute;mero m&aacute;ximo de operaciones No Seguras, por favor usar pasarela Segura";
	...
	function verComer() {
		...
		$this->db ( sprintf ( "select id, nombre, prefijo_trans, estado, pasarela, pasarelaAlMom, url from tbl_comercio where activo = 'S' " . "and idcomercio = '%s'", $this->comer ) );
...
//						$npas = '12'; // lo paso a Abanca 3D
		...
		if (strpos ( $this->datPas ['datPas'], '@' )) {
			...
			switch ($arrPas [0]) {
				...
				case 'pasoB' : // Tefpay
					...
					$this->db ( "insert into tbl_aisOrden (idtransaccion, idcliente, idbeneficiario, envia, recibe, comision, subida) values ('$tr', 
								...
								{$this->datAis['importenvia']}, {$this->datAis['importerecive']}, {$this->datAis['comision']}, 0)
								" );
					$tr .= '000000000';
					// $tipoTrans = 1;
					$tipoTrans = 46;
	...
	private function varRedsys() {
		// $cad = str_replace('<form name="envia" action=";urlPasarela;" method="post">', "", $cad);
		// $arrCad = explode("¿",$cad);
		// print_r($arrCad);
		// echo "\n";
		// foreach ($arrCad as $key => $val) {
		// $arrA = explode("=", $val);
		// if (
		// $arrA[0] != 'Ds_Merchant_ProductDescription'
		// && $arrA[0] != 'Ds_Merchant_Titular'
		// && $arrA[0] != 'Ds_Merchant_MerchantName'
		// && $arrA[0] != 'Ds_Merchant_PayMethods'
		// && $arrA[0] != 'Ds_Merchant_ConsumerLanguage'
		// && $arrA[0] != 'Ds_Merchant_MerchantSignature'
		// ) {
		// echo $arrA[0]."-".$arrA[1]."\n";
		// $this->obj->setParameter(strtoupper($arrA[0]),$arrVals[$arrA[1]]);
		// }
		// }
		// for ($i=0; $i<count($arrCad)-1;$i++){
		// echo $arrCad[$i][0]."-".$arrCad[$i][1]."\n";
		// $this->obj->setParameter($arrCad[$i][0],$arrVals[$arrCad[$i][1]]);
		// }
		$amount = $this->imp;
		$id = $this->idTrn;
