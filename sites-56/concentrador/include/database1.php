	...
	*/
	function &getInstance( $options	= array() ) {
		...
		if (empty($instances[$signature])) {
			$driver		= array_key_exists('driver', $options) 		? $options['driver']	: 'mysqli';
			$select		= array_key_exists('select', $options)		? $options['select']	: true;
			$database	= array_key_exists('database', $options)	? $options['database']	: $db;
			
			$options = array('driver'=>'mysqli','select'=>true,'database'=>'concentramf_db');

			$driver = preg_replace('/[^A-Z0-9_\.-]/i', '', $driver);
			$path	= _PATH_SITIO."include/".$driver.'.php';
echo $path;
			if (file_exists($path)) {
				require_once($path);
			} else {
				$error = 'Unable to load Database Driver:';
				return $error;
			}

			$adapter	= 'JDatabase'.$driver;
			$instance	= new $adapter($options);
echo $instance->getErrorMsg();
