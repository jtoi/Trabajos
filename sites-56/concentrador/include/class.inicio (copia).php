...
// 						$this->pasa = $this->temp->f ( 'idPasarela' );
				...
				if ($numpas > 0) {
					...
					if ($this->temp->num_rows() > 0) {
						...
						$this->log .= "/**********************/<br>select id, horas, idpasarela from tbl_rotComPas where idcom = " . $this->datCom['id'] . " and activo = 1 and orden = (select case when orden = $cantTo then 1 else orden+1 end from tbl_rotComPas r, tbl_comercio c where c.id = r.idcom and r.idpasarela = c.pasaRot and c.id = " . $this->datCom['id'] . ")\n<br>/**********************/<br>";
			...
			if ($this->comer != '122327460662') { // si no es Prueba
				if ($this->segura == 0) {
					...
					if ($this->pweb == 0) { //si no es pago desde web busco el array 
						...
						$this->db("select idPasarela from tbl_rotComPas where idcom = " . $this->datCom['id'] . " and activo = 1 and horas = 0 order by orden");
						if ($this->temp->num_rows() > 0) {
							$this->psArray = $this->temp->loadResultArray();
							$this->pasa = $this->psArray[0];
						}
	...
	private function CheqLimites($verif = true) {
		...
		// busca los limites de las pasarelas
		// $q = "select nombre, LimMinOper, LimMaxOper, LimDiar, LimMens, LimAnual, LimOperIpDia, LimOperTarDia, LimOperDia, pasarLim";
		$q = "select c.valor, p.pasarLim ";

		if ($verif) $q .= ", p.secure";

		$q .= " from tbl_colPasarLimite c, tbl_pasarela p where p.idPasarela = c.idPasar and c.idPasar = " . $this->pasa . " and c.idmoneda = " . $this->mon . " order by c.idLimite";
//		$q .= " from tbl_pasarela where idPasarela = " . $this->pasa;
		$this->db($q);
//		$arrV = $this->temp->loadAssocList();
//		$datPas = $arrV[0];

		$arrLim = $this->temp->loadAssocList();
		foreach ($arrLim[2] as $key => $value) {
			$this->log .= "$key => $value / ";
		}

		$datPas['LimMinOper'] = $arrLim[0]['valor'];
		$datPas['LimMaxOper'] = $arrLim[1]['valor'];
		$datPas['LimDiar'] = $arrLim[2]['valor'];
		$datPas['LimMens'] = $arrLim[3]['valor'];
		$datPas['LimAnual'] = $arrLim[4]['valor'];
		$datPas['LimOperIpDia'] = $arrLim[5]['valor'];
		$datPas['LimOperDia'] = $arrLim[6]['valor'];
		$datPas['pasarLim'] = $arrLim[2]['pasarLim'];
		...
		if ($this->comer != '129025985109' && $this->comer != '152295812637') // Para todos los comercios excepto Cubana y Soy Cubano IT
	...
	function verComer() {
		...
		if ($this->mon != 978 && $this->datCom['usdxamex'] == 0 && $this->amex == 1) {
			// $q = "select count(*) total from tbl_comercio where usdxamex = 1 and id_comercio = ".$this->comer;
			// $this->db($q);
			// if ($this->temp->f('total') == 0) {
				$pase = 0;
		...
		if ($this->comer === '129025985109' && $this->mon === '978') {
			...
			switch ($pasEurCub) {
				...
				case '29': // si es SabadellPlus DCC			Vuelto a poner en rotación
					$npas = '58'; // lo paso a Ibercaja 3D		el 19/2/19
					break;
		...
		if ($this->opr == 'D') {
			...
			if ($this->datPas['tipo'] == 'form' || $this->datPas['tipo'] == 'melt') {
				...
				if ($this->datPas['tipo'] == 'melt') {
					$this->db("select concat(a.url, '/paid/paid.php') url from tbl_agencias a, tbl_pasarela p where p.idagencia = a.id and p.idPasarela = " . $this->pasa);
