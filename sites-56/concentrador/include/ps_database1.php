...
class ps_DB extends JDatabaseMySQLi {
...
//		echo $this->_database->_sql;
		...
		$this->record = null;
		$this->record = Array(0);
//echo $this->_database->loadObjectList();
		if (strtoupper(substr( ltrim($this->_sql) , 0, 6 )) == "SELECT" ) {
			$this->record = $this->_database->loadObjectList();
		} elseif (stristr('baticora', $q)) {
			$this->_database->query();
		} else {
			$this->_database->query();
//			$this->query("insert into #__baticora (idadmin, texto, fecha) values (".$_SESSION['id'].", '".str_replace("'", "\'", $q)."', UNIX_TIMESTAMP(now()))");
		}
  ...
  */
	...
	*/
	...
	function getErrorMsg($escaped=false) {
		return $this->_database->getErrorMsg($escaped);
	}
	function getNumRows($cur = null) {
		return $this->_database->getNumRows($cur);
	}
	function stderr($showSQL = null) {
		return $this->_database->stderr($showSQL);
	}
	function getEscaped( $text, $extra = false ) {
		return $this->_database->getEscaped( $text, $extra = false );
	}
	function getFields( $text ) {
		return $this->_database->getTableFields( $text );
	}
}
?>