...
if ($d['fun'] == 'cargDatos') {
	...
	if (isset($d['ben'])) {
		...
		if ($temp->num_rows() == 1) {
			...
			$arrSal[0]['error'] = '';
		} else $arrSal[0]['error'] = utf8_encode('No se encuentra esa operaciï¿½n');
		...
		if ($temp->num_rows()) {
			...
			if ($d['firma'] == $fima) {
				...
				if ($temp->num_rows()) {
					...
					$q = sprintf("select distinct motivo 'error', concat('D') estado from tbl_saltosPasar s, tbl_comercio c 
							where c.id = s.idcomercio
							and s.identificador = '%s'
							and c.idcomercio = '%s'
							order by s.id desc limit 1", $d['transaccion'], $d['comercio']);
					$traza .= "<br>".stripslashes($q);
					$temp->query($q);
					...
					} else echo json_encode(array("identificador"=>"{$d['transaccion']}","error"=>"Operacion rechazada a la entrada de la aplicacion","estado"=>"D"));
				} 
			} else echo "Las firmas no coinciden: $fima";
		} else echo "El comercio no existe";
	} else echo "Datos no válidos";
}
	...
	if (strlen($d['comercio']) < 20) {
		...
		if ($temp->num_rows()) {
			if ($d['firma'] == hash("sha256", $d['comercio'].$d['operacion'].$temp->f('palabra'))) {
				...
				if ($temp->f('id') == '633') $com = '633,39'; else $com = $temp->f('id');//si el comercio es Tocopay se buscan los de AIS tambiï¿½n
...
if ($d['metodo'] == 'tasa') {
	...
	} else echo "ERROR: Datos no válidos";
}
