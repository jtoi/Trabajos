...
// 		}
		...
		$q = "select id, idcomercio, nombre, cierrePer, maxCierre, cierrePor from tbl_comercio where activo = 'S' and llevacierre = 1 and id in (
				select distinct cierrePor from tbl_comercio)";
		$temp->query($q);
// 		echo "$q\n<br>";
		$texto .= "$q\n<br>";
	...
	// 		echo "$q\n<br>";
			...
			$q = "select idcierre, r.idtransaccion, t.fecha_mod 'fechaCierre' "
					. " from tbl_cierreTransac r, tbl_transacciones t "
					. " where t.idtransaccion = r.idtransaccion and t.idcomercio in ('{$strid}') order by t.fecha desc limit 0,1";
			$texto .= "$q\n<br>";
// echo "$q\n<br>";
			$temp->query($q);
...
//			print_r($arrSal);
			...
			if (count($arrSal) == 0) {
				$q = "select idcomercio, idtransaccion, fecha 'fechaCierre' from tbl_transacciones where idcomercio in ('{$strid}')"
...
// 			print_r($arrSal);
			...
			foreach ($arrSal as $item) {
				...
				/*Comienza variación*/
				if($com[3] == 'M') { //realizar el cierre mensual
					$texto .= date('j')." == 1 || (".mktime(0, 0, 0, date('n'), 1, date('Y'))." > ".$ffeUn." && ".date('n')." != ".date('n',$ffeUn).")<br>";
					if (date('j') == 1 || (mktime(0, 0, 0, date('n'), 1, date('Y')) > $ffeUn && date('n') != date('n',$ffeUn)+1)) { 
						//Si es día 1ro o si la fecha del último cierre es menor que el día 1ro del mes pero no coinciden los meses del ult. cierre +1 y del mes actual
						$fecTop = mktime(0, 0, 0, date('n'), 1, date('Y'));
						$q = "select count(idtransaccion) cant from tbl_transacciones "
								. " where estado = 'A' "
									. " and tipoEntorno = 'P' "
									. " and idcomercio in ('{$strid}')"
									. " and fecha_mod between ".($ffeUn+10)." and ".$fecTop;//le sumo unos segundos para evitar tomar la últ. oper. del cierre anterior
					}
...
//					$texto .= $menImp;
				...
				$q = "select sum(euroEquiv) suma from tbl_transacciones where estado = 'A' and idcomercio in ('{$strid}') and fecha > $ffeUn";
// if ($com[0] == 23) echo "$q\n<br>";
				$temp->query($q);
				$valeT = $temp->f('suma');
//					$texto .= $q."<br>\n";
...
//		echo "entra";
		...
		*/
		...
		if($temp->f('total') == 1) {
			foreach($den as $item) {
				...
				if (!$rat > 0) {
					$rat = 0;
					$error[] = "Error cambio Abanca\n<br>";
				} else $rat = $rat+$tasaInc;

				$q = "update tbl_cambio set abanca = $rat where moneda = '$item' and from_unixtime(fecha,'%d/%m%/%Y') = from_unixtime($horaTasa,'%d/%m%/%Y')";
                $texto .= $q."<br>";
				$temp->query($q);
				...
				if (!$rat > 0) {
					$rat = 0;
					$error[] = "Error cambio Caixa\n<br>";
				} else $rat = $rat+$tasaInc;

				$q = "update tbl_cambio set caixa = $rat where moneda = '$item' and from_unixtime(fecha,'%d/%m%/%Y') = from_unixtime($horaTasa,'%d/%m%/%Y')";
                $texto .= $q."<br>";
				$temp->query($q);
				
				//según Sabadell
				$rat = rateSabadell($item);
				if (!$rat > 0) {
					$rat = 0;
					$error[] = "Error cambio Sabadell\n<br>";
				} else $rat = $rat+$tasaInc;

				$q = "update tbl_cambio set sabadell = $rat where moneda = '$item' and from_unixtime(fecha,'%d/%m%/%Y') = from_unixtime($horaTasa,'%d/%m%/%Y')";
                $texto .= $q."<br>";
				$temp->query($q);
			}
		}
		...
		*/
		...
		foreach ($den as $mon) {
			if (strlen($banco) == 0) {
				$q = "select greatest( visa, bce, bnc, xe, abanca, caixa ) cambio from tbl_cambio where moneda = '$mon' order by fecha desc limit 0,1";
                $texto .= $q."<br>";
				$temp->query($q);
// 				$arrVal = $temp->f('cambio');
...
for ($i=0; $i<count($arrVals); $i++) {
	...
	$corCreo->to(alejandro.garcia@cubana.avianet.cu');
...
// $temp->query("Paso6");
	 ...
	 */
	$q = "update tbl_admin set activo = 'N' where fecha_visita < ($fechaHoy-60*60*24*60) and activo = 'S'";
	$temp->query($q);
...
function rateAbanca ($denom) {
	include_once('include/class.html.parser.php');
	...
	$options = array(
			...
			CURLOPT_URL				=> 'http://corporacionbancaria.abanca.com/es/Generales/Cambio-de-Divisas/'
	);
	$ch = curl_init();
	...
	$cade = '[html][body][div][div][div4][div2][div2][div][table][tbody]';
	for ($i=1;$i<100;$i++){
		if(is_array($parseResult[$cade."[tr$i][td1]"])) {
			if (strstr($parseResult[$cade."[tr$i][td1]"]['text'][0],$denom)) return $parseResult[$cade."[tr$i][td6]"]['text'][0];
		} else break;
	}
	return 0;
}

function rateCaixa($demon) {
	include_once('include/class.html.parser.php');
	$parser = new html_parser;
	...
	for ($i=1;$i<100;$i++){
		if(is_array($parseResult[$cade."[tr$i][td1]"])) {
			if (strstr($parseResult[$cade."[tr$i][td1]"]['text'][0],$demon)) return $parseResult[$cade."[tr$i][td3]"]['text'][0];
		} else break;
	}
	return 0;
}

function rateSabadell ($denom) {
	include_once('include/class.html.parser.php');
	$parser = new html_parser;
	...
	for ($i=1;$i<100;$i++){
		if(is_array($parseResult[$cade."[tr$i][td1][a]"])) {
			$tiron = 0;
			if (strstr($parseResult[$cade."[tr$i][td1][a]"]['attr'][0]['value'],'USD')) return $parseResult[$cade."[tr$i][td2]"]['text'][0];
		} else {
