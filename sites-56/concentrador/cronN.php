...
//$mensage .='sdfgsdfhgdfh';
				$mensage .= "<br />Porciento de transacciones<br /><table><tr><td>Estado</td><td style='text-align:center;width:60px'>%</td><td style='text-align:center;width:60px'>$text</td></tr>";
				$q = "select estado, format((count(t.idtransaccion)*100/(select count(idtransaccion) from tbl_transacciones where fecha between $fecMod and $fechaHoy and tipoEntorno = 'P')),2) '%', ".
						"format(((select count(idtransaccion) from tbl_transacciones f where f.fecha between $fecha30Dante and $fechaHoy and f.estado = t.estado and f.tipoEntorno = 'P') * 100 / ".
						"(select count(idtransaccion) from tbl_transacciones where fecha between $fecha30Dante and $fechaHoy and tipoEntorno = 'P')),2) '% -30' ".
						"from tbl_transacciones t where t.fecha between $fecMod and $fechaHoy and t.tipoEntorno = 'P' group by t.estado";
//			echo $q;
				$temp->query($q);
				$arrR = $temp->loadRowList();
				foreach ($arrR as $item) {
				...
				$mensage .= "<br />Porciento de valores por monedas<br /><table><tr><td>Moneda</td><td style='text-align:center;width:60px'>%</td><td style='text-align:center;width:60px'>$text</td></tr>";
				$q = "select m.moneda, format((sum(euroEquiv)*100/(select sum(euroEquiv) from tbl_transacciones r where t.estado in ('A','V') and t.tipoEntorno = 'P' and fecha between $fecMod ".
						"and $fechaHoy)),2) , format(((select sum(euroEquiv) from tbl_transacciones a where a.moneda = m.idmoneda and a.estado in ('A','V') and a.tipoEntorno = 'P' and ".
						"a.fecha between $fecha30Dante and $fechaHoy)*100/ (select sum(euroEquiv) from tbl_transacciones a where a.estado in ('A','V') and a.tipoEntorno = 'P' and a.fecha ".
						"between $fecha30Dante and $fechaHoy)),2) ".
						"from tbl_transacciones t, tbl_moneda m ".
						"where t.moneda = m.idmoneda and t.estado in ('A','V') and t.tipoEntorno = 'P' and t.fecha between $fecMod and $fechaHoy ".
						"group by t.moneda order by m.moneda";
//				echo $q;
				$temp->query($q);
				$arrR = $temp->loadRowList();
				foreach ($arrR as $item) {
				...
				$mensage .= "<br />Porciento de transacciones Aceptadas por Monedas<br /><table><tr><td>Moneda</td><td style='text-align:center;width:60px'>%</td><td style='text-align:center;width:60px'>$text</td></tr>";
				$q = "select m.moneda, format((count(idtransaccion)*100/(select count(idtransaccion) from tbl_transacciones r where r.moneda = t.moneda and r.fecha between ".
						"$fecMod and $fechaHoy and r.tipoEntorno = 'P')),2) '%', format(((select count(idtransaccion) from tbl_transacciones f where ".
						"f.moneda = t.moneda and f.estado = 'A' and f.fecha between $fecha30Dante and $fechaHoy and f.tipoEntorno = 'P')*100/(select count(idtransaccion) from tbl_transacciones r where ".
						"r.moneda = t.moneda and r.fecha between $fecha30Dante and $fechaHoy and r.tipoEntorno = 'P')),2) '% -30' from tbl_transacciones t, tbl_moneda m where t.moneda = m.idmoneda ".
						"and t.tipoEntorno = 'P' and t.estado = 'A' and t.fecha between $fecMod and $fechaHoy group by t.moneda";
//			echo $q;
				$temp->query($q);
				$arrR = $temp->loadRowList();
				foreach ($arrR as $item) {
				...
				$mensage .= "<br />Porciento de transacciones Aceptadas por Pasarelas<br /><table><tr><td>Pasarela</td><td style='text-align:center;width:60px'>%</td><td style='text-align:center;width:60px'>$text</td></tr>";
				$q = "select p.nombre, format((count(idtransaccion)*100/(select count(idtransaccion) from tbl_transacciones r where r.pasarela = t.pasarela and ".
						"r.fecha between $fecMod and $fechaHoy and r.tipoEntorno = 'P')),2) '%', ".
						"format(((select count(idtransaccion) from tbl_transacciones f where f.pasarela = t.pasarela and f.tipoEntorno = 'P' and f.estado = 'A' and f.fecha between $fecha30Dante and ".
						"$fechaHoy) * 100 / (select count(idtransaccion) from tbl_transacciones s where s.tipoEntorno = 'P' and s.pasarela = t.pasarela and s.fecha between ".
						"$fecha30Dante and $fechaHoy)),2) '% -30'".
						"from tbl_transacciones t, tbl_pasarela p where t.pasarela = p.idPasarela and t.tipoEntorno = 'P' and ".
						"t.estado = 'A' and t.fecha between $fecMod and $fechaHoy group by pasarela";
//echo $q;
				$temp->query($q);
				$arrR = $temp->loadRowList();
				foreach ($arrR as $item) {
				...
				$mensage .= "<br />Porciento de transacciones Aceptadas por Comercios<br /><table><tr><td>Comercio</td><td>Eur/tr</td><td style='text-align:center;width:60px'>%</td><td style='text-align:center;width:60px'>$text</td></tr>";
				$q = "SELECT c.nombre, format(sum(case t.estado when 'B' then if (fecha_mod < fechaPagada, (-1 * ((valor_Inicial-valor)/100/tasa)), (valor/100/tasa)) when 'V' then if (fecha_mod < fechaPagada, (-1 * ((valor_Inicial-valor)/100/tasa)),  valor/100/tasa) when 'A' then (valor/100/tasa) else '0.00' end) / count(*), 2), format((count( idtransaccion ) *100 / ( ".
							"SELECT count( idtransaccion ) FROM tbl_transacciones r WHERE r.idcomercio = t.idcomercio AND r.fecha BETWEEN $fecMod AND $fechaHoy AND r.tipoEntorno = 'P' ".
						")) , 2) '%', format((select count(idtransaccion) from tbl_transacciones r where r.idcomercio = t.idcomercio and r.fecha > $fecha30Dante and $fechaHoy and r.tipoEntorno = 'P' ".
							"and r.estado = 'A')*100/(select count(idtransaccion) from tbl_transacciones r where r.idcomercio = t.idcomercio and r.fecha > $fecha30Dante and $fechaHoy and ".
								"r.tipoEntorno = 'P') , 2) '% -30' ".
...
//			echo $q;
				...
				$arrR = $temp->loadRowList();
				foreach ($arrR as $item) {
					$mensage .= "<tr><td>{$item[0]}</td><td>{$item[1]}</td><td style='text-align:center;width:60px'>{$item[2]}</td><td style='text-align:center;width:60px'>{$item[3]}</td></tr>";
				}
				$mensage .= "</table>";

				$mensage .= "<br />Porciento de transacciones aceptadas por países<br /><table><tr><td>Pa&iacute;s</td><td style='text-align:center;width:60px'>%</td><td style='text-align:center;width:60px'>$text</td></tr>";
				$q = "select p.nombre, format(count(idtransaccion)*100/(select count(idtransaccion) from tbl_transacciones where estado = 'A' and fecha between $fecMod and $fechaHoy),2) '%', ".
						"format((select count(idtransaccion) from tbl_transacciones f where f.idpais = p.id and f.estado = 'A' and fecha between $fecha30Dante and $fechaHoy) *100 / ".
						"(select count(idtransaccion) from tbl_transacciones s where s.estado = 'A' and fecha between $fecha30Dante and $fechaHoy),2) '% -30' from tbl_transacciones t, tbl_paises p ".
						"where t.idpais = p.id and estado in ('A','V','B') and t.fecha between $fecMod and $fechaHoy group by t.idpais order by count(idtransaccion) desc limit 0,10";
//			echo $q;
				$temp->query($q);
				$arrR = $temp->loadRowList();
				foreach ($arrR as $item) {
				...
				$mensage .= "<br />Porciento de transacciones aceptadas por países AIS<br /><table><tr><td>Pa&iacute;s</td><td style='text-align:center;width:60px'>%</td><td style='text-align:center;width:60px'>$text</td></tr>";
				$q = "select p.nombre, format(count(idtransaccion)*100/(select count(idtransaccion) from tbl_transacciones where estado = 'A' and idcomercio in ('527341458854') and fecha ".
						"between $fecMod and $fechaHoy),2) '%', format((select count(idtransaccion) from tbl_transacciones f where  f.idcomercio in ('527341458854') and f.idpais = p.id and f.estado = 'A' ".
						"and fecha between $fecha30Dante and $fechaHoy) *100 / (select count(idtransaccion) from tbl_transacciones s where  s.idcomercio in ('527341458854') and s.estado = 'A'  and ".
						"fecha between $fecha30Dante and $fechaHoy),2) '% -30' from tbl_transacciones t, tbl_paises p where t.idcomercio in ('527341458854') and t.idpais = p.id and t.estado = 'A' and ".
						"t.fecha between $fecMod and $fechaHoy group by t.idpais order by count(idtransaccion) desc limit 0,10";
//			echo $q;
				$temp->query($q);
				$arrR = $temp->loadRowList();
				foreach ($arrR as $item) {
				...
				$mensage .= "<br />Porciento de transacciones aceptadas por países Cubana<br /><table><tr><td>Pa&iacute;s</td><td style='text-align:center;width:60px'>%</td><td style='text-align:center;width:60px'>$text</td></tr>";
				$q = "select p.nombre, format(count(idtransaccion)*100/(select count(idtransaccion) from tbl_transacciones where estado = 'A' and idcomercio in ('129025985109') and fecha ".
						"between $fecMod and $fechaHoy),2) '%', format((select count(idtransaccion) from tbl_transacciones f where  f.idcomercio in ('129025985109') and f.idpais = p.id and f.estado = 'A' ".
						"and fecha between $fecha30Dante and $fechaHoy) *100 / (select count(idtransaccion) from tbl_transacciones s where  s.idcomercio in ('129025985109') and s.estado = 'A'  and ".
						"fecha between $fecha30Dante and $fechaHoy),2) '% -30' from tbl_transacciones t, tbl_paises p where t.idcomercio in ('129025985109') and t.idpais = p.id and t.estado = 'A' and ".
						"t.fecha between $fecMod and $fechaHoy group by t.idpais order by count(idtransaccion) desc limit 0,10";
			//echo $q;
				$temp->query($q);
				$arrR = $temp->loadRowList();
				...
				$mensage .= "<br />Estado de transacciones Cubana<br /><table><tr><td>Estado</td><td>Cant</td><td style='text-align:center;width:60px'>%</td></tr>";
				$q = "select estado, count(idtransaccion), format((count(idtransaccion)/(select count(idtransaccion) from tbl_transacciones where idcomercio = '129025985109' and fecha between $fecMod and $fechaHoy)*100),2) from tbl_transacciones t where t.idcomercio in ('129025985109') and t.fecha between $fecMod and $fechaHoy group by t.estado order by estado";
//			echo $q;
				$temp->query($q);
				$arrR = $temp->loadRowList();
				foreach ($arrR as $item) {
...
//        echo $texto;
        ...
        //Escanea los directorios del concentrador buscando cambios de códigos o nuevos ficheros metidos dentro.
//        $scan->direct = str_replace('cron.php', '', $_SERVER['SCRIPT_FILENAME']);
//        $texto .= "<br>\n".str_replace('cron.php', '', $_SERVER['SCRIPT_FILENAME']);
        //$scan->direct = '/home/julio/www/phpclasses/scanner/';
        //        $q = "show table like 'tbl_verifica'";
        //        echo "sale=".$temp->query($q);
//        $resl = $scan->scan();
        ...
        if (is_array($resl)) {
            ...
            foreach ($resl as $key => $value) {
                ...
                foreach ($value as $keys => $values) {
                     $texto .= "$keys => $values<br>";
                }
            }
        }
		
		/* Calcula y Avisa de la realización de algún cierre que se haya puesto*/
		$men = '';
		$q = "select max(idcierre), c.idcomercio, r.idtransaccion, cierrePer, maxCierre, c.nombre, max(r.fechaCierre) from tbl_cierreTransac r, tbl_comercio c where r.idcomercio = c.id group by r.idcomercio";
		$temp->query($q);
...
//			$q = "select fecha_mod from tbl_transacciones where idtransaccion = '".$item[2]."'";
//			$temp->query($q);
//			$ffeUn = $temp->f('fecha');
			$ffeUn = $item[6];
			if($item[3] == 'M') $dcon = 30;
			elseif($item[3] == 'D') $dcon = 1;
			elseif($item[3] == 'Q') $dcon = 15;
			elseif($item[3] == 'S') $dcon = 7;
			
			$dias = floor((time()-$ffeUn)/(60*60*24));
			$texto .= "<br>".$dias;
			
			$q = "select count(idtransaccion) cant from tbl_transacciones where fecha between (select t.fecha from tbl_transacciones t "
					. "where t.idtransaccion = ".$item[2].") and ".time()." and idcomercio = '".$item[1]."'";
			$temp->query($q);
			$cant = $temp->f('cant');
			if ($temp->getErrorMsg()) $texto .= "\n<br>$q \n<br>Error: ".$temp->getErrorMsg();
			...
			if ($cant > 0) {
				...
				if ($dcon <= $dias) $menImp .= "El comercio <strong>".$item[5]."</strong> ha llegado al límite por tiempo, $dcon días, necesita se le realice el Cierre de inmediato.\n<br>";
				//llegegará en las próximas 24 hr al número de días.
				elseif ($dcon <= $dias+1) $menNimp .= "El comercio <strong>".$item[5]."<strong/> llegará en las próximas 24 hrs al límite máximo por días para que se le realice el Cierre.\n<br>";

				$q = "select sum(euroEquiv) suma from tbl_transacciones where estado = 'A' and idcomercio = '".$item[1]."' and fecha > $ffeUn";
				$temp->query($q);
				$valeT = $temp->f('suma');

				//llegó al monto límite o está pasado
				if ($valeT >= $item[4]) $menImp .= "El comercio <strong>".$item[5]."</strong> tiene acumulado hasta este momento $valeT Euros, "
						. "lo que hace que haya llegado al límite por monto, necesita se le realice el Cierre de inmediato.\n<br>";
				//llegegará en las próximas 24 hr .
				elseif ($valeT >= $item[4]+($valeT/$dias)) $menNimp .= "El comercio <strong>".$item[5]."</strong> tiene acumulado hasta este momento $valeT Euros, "
						. "llegará en las próximas hrs al límite máximo por monto para que se le realice el Cierre.\n<br>";

				if($menImp) $men .= $menImp;
				else $men .= $menNimp;
			}
		}
		
		$corCreo->todo(37,"Avisos de Cierres", $men);
...
for ($i=0; $i<count($arrVals); $i++) {
	...
	$messs = "Hola Alejandro\n<br>\n<br>"
			...
			. "la misma ha llegado desde Amadeus con la sesión: ".$arrVals[$i]['sesion']."\n<br>\n<br>Saludos\n<br>\n<br>Administrador de Comercios\n<br>AMFGlobalitems" ;
	
//	$corCreo->to('Alejandro <serv.tecnico@amfglobalitems.com>');
...
if (time() >= leeSetup('horaAis') || 1==1) {
	if (time() > mktime(0, 0, 1, date("m"), date("d"), date("Y")) && time() < mktime(9, 59, 59, date("m"), date("d"), date("Y"))) { // va cambiando AIS de pasarela entre las 12 pm a las 10 am
		...
		if ($pasr == '36') { // si la pasarela es Caixa2 3D lo paso a Sabadell2 3D y le pongo dos horas
			$pasr = '32';
			$pas = 2;
			$texto .= "Cambiando AIS a Bankia4 3D";
		} else if ($pasr == '32') {
			$pasr = '31';
			$pas = 2;
			$texto .= "Cambiando AIS a Sabadell2 3D";
		} else if ($pasr == '31') {
			$pasr = '36';
			$pas = 2;
			$texto .= "Cambiando AIS a Caixa2 3D";
		}
	} else {
...
/**
 * Listado de IPs con más de 15 accesos al Cocentrador sin envío de datos en el mes anterior
 */
...
$corCreo->envia(5);
$q = 'insert into tbl_traza (titulo,traza,fecha) values ("Ejecución del Cron","'.html_entity_decode($texto).'",'.time().')';
$temp->query($q);
//mail('jotate@amfglobalitems.com', 'Ejecución del Cron', $texto."\nEjecutado satisfactoriamente a las ".date('d/m/Y H:i'));
echo $texto."\n<br>Ejecutado satisfactoriamente a las ".date('d/m/Y H:i');
?>