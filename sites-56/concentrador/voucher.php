...
//	$d = 'FC686-785-844_2016';
//	$c = '148166515065';
//}

if (strlen($d) < 20) {
	if (!$ent->isUrl($d, 20)) exit;
	if (!$ent->isEntero($c, 20)) exit;
//	if (!$ent->isAlfabeto($i, 3)) exit;

    $q = "select r.id_transaccion, r.nombre cliente, r.codigo, t.codigo code, c.nombre comercio, c.datos, servicio, r.valor, r.idioma, from_unixtime(r.fechaPagada, '%e - %b - %Y %H:%i') fechaPago,
                    m.moneda, r.moneda idmoneda, c.palabra, c.voucherEn, c.voucherEs, a.nombre comTPV
                from tbl_reserva r, tbl_comercio c, tbl_moneda m, tbl_transacciones t, tbl_pasarela p, tbl_agencias a
