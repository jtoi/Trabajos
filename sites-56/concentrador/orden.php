...
if (($id = $ent->isReal($d['op'], 12))) {
	...
	$temp->query(sprintf("select t.cliente, t.concepto, t.idioma, t.valor, m.moneda, p.cuenta, t.fecha, c.nombre comercio, a.email, t.vista, t.idCom, t.moneda idmon, t.idPasarela from tbl_transferencias t, tbl_moneda m, tbl_pasarela p, tbl_comercio c, tbl_admin a where c.id = t.idCom and p.idPasarela = t.idPasarela and t.idadmin = a.idadmin and t.moneda = m.idmoneda and t.estado = 'P' and vista = 0 and (t.fecha + 10*86400) > unix_timestamp() and t.idTransf = '%d'",$id));
	...
	if ($temp->num_rows() > 0) {
		...
		$fecha = $temp->f('fecha');
		$comercio = utf8_decode($temp->f('comercio'));
		$email = $temp->f('email');
		$vista = $temp->f('vista');
		$idCom = $temp->f('idCom');
		$idmon = $temp->f('idmon');
