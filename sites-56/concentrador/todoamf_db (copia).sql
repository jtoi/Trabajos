...
begin
	...
	select valor into fechaU from apoyo where nombre = 'fecResultDatos';
	...
	insert into resultDatos select t.idtransaccion, t.idcomercio, t.identificador, t.codigo,
			...
			t.carDevCom, p.idbanco, p.idempresa, FROM_UNIXTIME(fecha_mod) 'fecha_act'
			
		from tbl_transacciones t, tbl_pasarela p
		where p.idPasarela = t.pasarela
			and t.fecha_mod > fechaU;
	set time_zone='America/Havana';
	
	update apoyo set 
		valor = (select fecha_mod from resultDatos order by fecha_mod desc limit 0,1), 
