...
/****************************comentar**************************************************/
if (stripos(_ESTA_URL, 'localhost') > 0 || stripos(_ESTA_URL, '192.168.0.243') > 0 || stripos(_ESTA_URL, '192.168.0.1') > 0) {
	
// 	date_default_timezone_set('Europe/Berlin');
	$d['comercio']='152390901739'; //Amistur
	// $d['comercio']='146161323238'; //Claim
	// $d['transaccion']='200';
...
if ($cuenta = ipBloqueada($dirIp)) {//verifica que la ip no esté bloqueada
	echo '<div style="background: url(\'template/images/degrada.png\') repeat-x scroll 0 0 transparent;"><div style="text-align:center; background: url(\'template/images/banner.jpg\') no-repeat scroll left center transparent; height: 73px; margin:0">&nbsp;</div></div><div style="text-align:center; margin-top:100px; font-family:sans serif; font-size:10px; background: url("images/banner.png") no-repeat scroll left center transparent; height: 73px;">Su IP '.$dirIp.' est&aacute; bloqueada / Your IP '.$dirIp.' is banned</div>';	if ($cuenta == 5) $correo->todo ( 11, 'IP banned entrada '.$dirIp, "Se ha bloqueado el acceso de la operación desde la IP ".$dirIp."<br>".$correoMi );
	exit();
}
...
// 			|| $inicio->comer == '122327460662'
	...
	if (!($inicio->mon = $ent->isReal($d['moneda'], 3))) {
		muestraError ("falla por moneda1", $correoMi);
	} else {
	...
	if (strlen($d['firma']) < 40 && $d['comercio'] != '129025985109') muestraError ("La firma no se ha codificado con SHA256", $correoMi);
...
function muestraError ($etiqueta, $textoCorreo) {
	...
	if ($temp->num_rows()>0) {//si la operación es al momento redirijo el error a la página de pago
		...
		<input type='hidden' value='".$etiquetajs."' name='etiqueta'>
		</form>
