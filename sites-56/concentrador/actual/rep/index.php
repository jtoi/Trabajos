	...
	if ($d['Ds_SignatureVersion'] == "HMAC_SHA256_V1") {// si la operación es del Redsys nuevo
	    ...
	    if ($sabad == 'ok') {
			...
			if (strlen($d['Ds_AuthorisationCode']) > 2 && $d['Ds_AuthorisationCode'] < 12) {
				$temp->query("update tbl_transacciones set codigo = '".$d['Ds_AuthorisationCode']."' where idtransaccion = ".$cookie);
				$temp->query("update tbl_reserva set bankId = '".$d['Ds_AuthorisationCode']."' where id_transaccion = ".$cookie);
 ...
 //   echo $query;
	...
	if ($valores[14] == 12 || $valores[14] == 53 || $valores[14] == 50) {
		...
		if ($valores[8] != $pasEs) {
			...
			if ($valores[8] != 'A') {
				$valores[8] = $pasEs;
				if ($valore[3] == '' && $d['Ds_AuthorisationCode']) $valores[3] = $d['Ds_AuthorisationCode'];
				if ($idpas != 91) {
