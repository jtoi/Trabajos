...
if ($pasarela == 218) {//Moneytigo
	if (!$idtrans = $ent->isNumero($d['MerchantRef'],12)) $correoMi .= "No es válido el número de la operación {$d['data']} <br>";
	else {
		...
		$correoMi .= "<br>ESTADO>".$aarDaMo->Transaction_Status->State;

		if ($aarDaMo->Transaction_Status->State == '2') {//operación Aceptada
			if (!$codautorizacion = $ent->isAlfanumerico($aarDaMo->Transaction_Status->Bank_Code,10))$correoMi .= "No es válido el número de autorizo ".$d['auth']." <br>";
			$estado = 2;
			if ($aarDaMo->Card->Type == 'VISA') $idtar = 2;

			$pedazo = "tarjetas = '".$aarDaMo->Card->Number."', identificadorBnco = '".$d['TransId']."', id_tarjeta = '$idtar', ";
		} else {
			$estado = '3';
			$importe = null;
			$codautorizacion = null;
			$ok = 1;
			$iderror = $aarDaMo->Transaction_Status->Bank_Code." ". $aarDaMo->Transaction_Status->Bank_Code_Description;
		}
...
$correoMi .= "dserror=$dserror<br>";
	...
	// 		$temp->setQuery();
			...
			if (!$temp->num_rows() && $error != 0) {
				$correo->todo(12, 'No existe el error en la BD', "El codigo de error es $error en el terminal $psrl del TPV $va");
			} else {
