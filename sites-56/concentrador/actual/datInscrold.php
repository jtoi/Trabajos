...
if (strlen($d['Apellido2']) > 3)
	if (!($ape2 = ucwords(strtolower($ent->isAlfabeto($d['Apellido2'],50))))) muestraError ("falla por Apellido2", $correoMi);
...
// 	$sex = substr($sex, 1);
	...
	//compruebo para idcimex, usuario y correo que son las cosas que no se pueden repetir
	$arrTipo = array(
		"idcimex = '$id'"
		, "correo = '$email'"
		, "binary usuario = '$usur'"
	...
	for ($i=0; $i<count($arrTipo); $i++) {
		$q = "select id, idtitanes, usuario from tbl_aisCliente where ".$arrTipo[$i];
		$correoMi .= $q."\n<br>";
		error_log(' ');
		error_log($q);
		$temp->query($q);
		$idTit = 0;
		$cantT = $temp->num_rows();
		error_log("cantT=".$cantT);
		if ($cantT != 0){
			$qm = $arrTipo[$i];
			$idcli = $temp->f('id');
			$usur = $temp->f('usuario');
			$idTit = $temp->f('idtitanes');
			error_log("idcli=".$idcli);
			error_log("usur=".$usur);
...
error_log("Cliente nuevo");
		...
		if ($temp->num_rows()) {
			...
			$pase = false;
			$titSub = true;
			$q = "update tbl_aisCliente set idcimex = '$id', usuario = '$usur', fecha = '".time()."', subfichero = 1, correoenv = 0, borrficheros = 0, ficgrandes = 0, correo = '$email' where id = ".$temp->f('id');
		} else {
			//Verifico por fecha de nacimiento y nombre
			$q = "select * from tbl_aisCliente where fnacimiento = '$fechaNac' and nombre like '%$nombre%' and papellido like '%$ape1%'";
			$temp->query($q);
			$can = $temp->num_rows();
			$idTit = $temp->f('idtitanes');
			$idn = $temp->f('id');
			...
			$te = '';
			foreach ($arrCli[0] as $key => $value) {
				$te .= "$key => $value<br>";
			}
			if ($can > 0) {
				$correo->todo(9,'Duplicidad por fecha de nacimiento',"Se insertó el Cliente $nombre $ape1 con idcimex $id. Los datos anteriores eran <br> $te");
			}


			//Verifico el teléfono y nombre
			if ($can == 0) {
				$q = "select id, idcimex, idtitanes from tbl_aisCliente where telf1 = '$telf' and nombre like '%$nombre%'";
				$temp->query($q);
				$can = $temp->num_rows();
				$idTit = $temp->f('idtitanes');
				$idn = $temp->f('id');
				...
				if ($can > 0) {
					$correo->todo(9,'Duplicidad por teléfono',"Se insertó el Cliente $nombre $ape1 con idcimex $id. Los datos anteriores eran <br> $te");
				}
			}
...
//			}
			...
			if ($can > 0) {
				...
				$pase = false;
				$titSub = true;
				$q = "update tbl_aisCliente set idcimex = '$id', usuario = '$usur', fecha = '".time()."', subfichero = 1, ficgrandes = 0, borrficheros = 0, correoenv = 0, correo = '$email' where id = ".$idn;
			} else {
...
error_log("buscaCliente");
	...
	if ($cantT != 0) {
		$iben = $temp->f('id');
		$idTit =  $temp->f('idtitanes');
		$correoMi .= "iben = $iben<br>";
        $correoMi .= "idTit = $idTit<br>";
        $pase = false;
        updateBenef($nombre, $ape1, $ape2, $telf, $dir, $ciudad, $ci, $id, $iben);
    } else {
        //chequeo que hayan enviado un beneficiario duplicado buscando el CI en la BD
		$q = "select id, idtitanes from tbl_aisBeneficiario where numDocumento = '$ci'";
		$temp->query($q);
		if ($temp->num_rows()>0) {
            ...
            //verifico beneficiario duplicado por nombre - teléfono
			$q = "select id, idtitanes from tbl_aisBeneficiario where telf = '$telf' and nombre = '$nombre'";
			$temp->query($q);
			if ($temp->num_rows()>0) {
                ...
                //beneficiario pasa todos los chequeos, no está en la BD lo inserto
				$idTit = 0;
                ...
                $iben = $temp->last_insert_id(); //si se inscribió un benef nuevo saco el id
            }
        }
    }
...
error_log("idTit=".$idTit);
			...
			if ($idTit>2) {
				...
				if ($temp->f('total') == 0) {//si el id que mandan nos se encuenta asignado
					...
					$q = "update tbl_aisCliente set subfichero = 1, borrficheros = 0, ficgrandes = 0, $tid fechaAltaTitanes = ".time().", correoenv = 0 where idcimex = $id"; //para los clientes
					$temp->query($q);
