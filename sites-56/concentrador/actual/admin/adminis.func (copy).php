...
//		}
	...
	error_log("tasa aplic=".$tasaApl);
					...
					$query = "insert into tbl_reserva (id_admin, id_comercio, est_comer, codigo, nombre, email, servicio, valor_inicial, moneda, fecha, pMomento, idioma, pasarela, tiempoV, url, amex) values ({$_SESSION['id']}, '{$comId}', '$estCom', '$trans', '{$nmbr}', '{$correo}', '{$ser}', '" . str_replace ( ",", ".", $arrEnt ['importe'] ) . "', '{$arrEnt ['moneda']}', $fechaNow, '{$arrEnt ['pago']}', '{$arrEnt ['idioma']}', $pasarela, {$arrEnt ['tiempo']}, '{$_SERVER["SERVER_NAME"]}', {$arrEnt ['amex']})";
					$temp->query ( $query );
					$error = $temp->getErrorMsg ();
...
// 		var_dump($arrEnt);
	...
	// 					$message = str_replace ( '{texto5}', _TEXTO5, $message );
						...
						$corr->to ( "serv.tecnico@bidaiondo.com" );
...
//		);
		...
		$url .= 'Order/'.$pago[0].'/Cancel';
		$title = 'Devolución de operación';
		$data = array(
				"Reason"			=> $pago[2],
				"AmountToReceive"	=> $pago[1],
				"Signature"			=> strtoupper(hash("sha512", $pago[0].$pago[1].$pago[2].$llave))
		);
		...
		$url .= "Customer/".$datos['ClientId']."/Beneficiary";
		$title = 'Verificación de relación Cliente - Beneficiario';
		unset($datos['ClientId']);
		$method = 'GET';
		
	} else if ($accion == 'O') {
		$url .= "Order";
		$title = 'Inscribe Orden';
		$datos['Signature'] = hash("sha512", $datos['Signature'].$llave);
		$method = 'POST';
...
error_log("json=$json");
	...
	$correoM .= "<br><br>
				...
				CURLOPT_CUSTOMREQUEST	=> $method,<br>
				CURLOPT_URL				=> ".$url.",<br>
				CURLOPT_POSTFIELDS		=> ".($json)."<br>
...
function titToken($pas=37){
	error_log("enrta en token con pasarela=".$pas);
	$temp = new ps_DB();
