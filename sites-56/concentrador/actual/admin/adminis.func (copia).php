			...
			if ($pasarela != 0) {
				if ($arrEnt['importe'] >= $valMin) {
					...
					if (($arrEnt['importe'] / $tasamno) <= $importMax)  {
						...
						$hora = time ();
						$query = "insert into tbl_transacciones	(idtransaccion, idcomercio, identificador, tipoOperacion, fecha, fecha_mod, valor_inicial, tipoEntorno, moneda, estado, sesion, pasarela, idioma) values ('$trans', '$comer', '" . $fijo . "', 'T', $hora, $hora, '$importe', 'P', '{$arrEnt['monedas']}', 'P', '$firma', '$pasarela', '{$arrEnt['idioma']}')";
						// echo $query."<br>";
						$temp->query ( $query );
