...
if (!$ent->isReal($_POST['cambiar'])) {
	...
	$tripa .= opciones_sel("select idrol as id, nombre from tbl_roles where orden >= ".$_SESSION['grupo_rol']." order by orden");
...
//	echo "select idcomercio id, nombre from tbl_comercio where id in (".$_SESSION['idcomStr'].") order by nombre";
	...
	$tripa .= opciones_sel("select idrol as id, nombre from tbl_roles where orden >= ".$_SESSION['grupo_rol']." order by orden", $temp->f('idrol'));
			$tripa .= '</select>
					</td>
				</tr>
				<tr>
...
if (stripos($_SESSION['idcomStr'],',') ) {
	...
	$q = "select idrol as id, nombre from tbl_roles where orden >= ".$_SESSION['grupo_rol'];
