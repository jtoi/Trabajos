...
if ($d['soldeve']) {
	$sale = '';
	$temp->query("select idtransaccion from tbl_transacciones where estado = 'A' and solDev = 1 and idtransaccion = '".$d['soldeve']."'");
	if ($temp->num_rows() == 1) {
		$temp->query("delete from tbl_devoluciones where idtransaccion = '".$d['soldeve']."'");
		if ($temp->getErrorMsg()) {
			$sale = "Error en la operación - ".$temp->getErrorMsg();
		} else {
			$temp->query("update tbl_transacciones set solDev = 0 where idtransaccion = '".$d['soldeve']."'");
			if ($temp->getErrorMsg()) {
				$sale = "Error en la operación - ".$temp->getErrorMsg();
...
if ($d['tranid']) {// cambia la operación no procesada a aceptada
	...
	error_log($q);
		...
		if (!$temp->getErrorMsg()) {
			$sale = 'Operación correctamente Actualizada.';
		} else $sale = $temp->getErrorMsg();
	} else $sale = 'No existe la operación en la Base de Datos';
	$d['nombre'] = $d['tranid'];
	echo "<script language=\"JavaScript\" type=\"text/javascript\">alert('$sale')</script>";
}
...
// 	echo $valInicio;
	...
	$estadoArr = array(
		array("P','A','D','N','B','V','E','R','L", _REPORTE_TODOS),
...
// 		$whereView = " and e.id = p.idempresa ";
	...
	"CASE  
		when t.tipoPago != 'W' and t.idcomercio != '159958099335' then concat('<a href=\"index.php?componente=comercio&pag=cliente&val=', t.idtransaccion, '\">', t.identificador, '</a>')
		else identificador end identificador,".
	"case 
		when t.estado = 'P' then 'green' 
		when t.estado = 'A' then if (solDev = 0, 'black', 'gray')
	...
	case 
		...
		when t.tipoOperacion = 'P' or t.tipoOperacion = 'T' or t.tipoOperacion = 'R' then
			case
				...
				when t.estado = 'D' then 'Denegada' 
				when t.estado = 'N' then 'No Procesada' 
				when t.estado = 'B' then 'Anulada' 
				when t.estado = 'R' then 'Reclamada' 
				when t.estado = 'V' then 'Devuelta'
				else '' end
		else ''
		end estad,
	case 
...
// 	echo $tabView."<br>";
	...
	c.idcomercio,t.solDev,t.pasarela,t.tipoEntorno tipoE,t.moneda idmoneda, replace(t.codigo, '%', ' - ') codigo,t.id_error error,round(t.tasaDev,4) tasaDev, j.nombre tjta,
	...
	case t.tipoOperacion
		...
		when 'P' or 'T' then
			case t.estado
				when 'P' then 'En Proceso' 
				when 'A' then if (t.solDev = 0, 'Aceptada', 'Sol. Devolc.')
...
// echo $where;
	...
	if ($_SESSION['grupo_rol'] <= 5) {
		...
		$colEsp[] = array("z", 'Poner la transacción Reclamada', "css_borra", "Transacción Reclamada");
...
// 	$corCreo->todo(43, 'Select de monedas', $q);
	...
	array_push($columnas, array(_REPORTE_IP, "ip{ip}", "75", "center", "center" ),
					...
					array("T.tarjeta<br>ó T.pago", "tjta", "80", "center", "left" ),
