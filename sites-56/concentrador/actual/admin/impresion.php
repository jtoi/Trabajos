...
if ($_REQUEST['querys21']) { //exporta el reporte transacciones tipo CSV2
	if ($_REQUEST['pag'] == 'reporte') {
		...
		$conte .=  "<tr><td>No.</td><td>Id</td><td>Comercio</td><td>Referencia del comercio</td><td>Banca</td><td>Cliente</td><td>Referencia Banco</td><td>Fecha</td><td>Valor Inicial</td><td>Valor Devuelto</td><td>Valor</td><td>Moneda</td><td>Error</td><td>Estado</td><td>Tasa deCambio</td><td>Conversión Euro</td><td>IP</td><td>País</td><td>Fecha Mod.</td><td>Tipo Oper.</td>" ;
...
if ($_REQUEST['querys2']) { //exporta el reporte transacciones tipo CSV2
	if ($_REQUEST['pag'] == 'reporte') {
		$conte = "No.,Id,Comercio,Referencia del comercio,Banca,Cliente,Referencia Banco,Fecha,Valor Inicial,Valor Devuelto,Valor,Moneda,Error,Estado,Tasa de cambio,Conversión Euro,IP,País,Fecha Mod.,Tipo Operc" ;
...
// 			echo $conte;
			...
			$conte .= $temp->f('ip').'","' . $pai .'","' . date('d/m/Y H:i:s',$temp->f('fecha_mod')).'","'. $temp->f('tipo').'"';
			if (strpos($_REQUEST['querys2'], 'tbl_aisOrden') > 2)
	            $conte .= ',"' . $temp->f('titord').'"';
            $conte .= "\n";
		}
...
if ($_REQUEST['querys6']) { //exporta el reporte transacciones tipo CSV1 según la modificación de Anne
	if ($_REQUEST['pag'] == 'reporte') {
		$conte = "No.,Id,Comercio,Referencia del comercio,Banca,Cliente,Referencia Banco,Fecha Mod,Valor Inicial,Conversión Euro (VI),Tasa deCambio (VI),Fecha Devol,Valor Devuelto,Conversión Euros (VD),Tasa de cambio(VD),Fecha,Moneda,Valor,Estado,Empresa,Tipo Oper.,T. Tarjeta" ;
		...
		for ($i=0; $i<$temp->num_rows(); $i++) {
            ...
            $conte .= number_format($temp->f('tasaDev'),4).'","'. date('d/m/Y H:i:s', $temp->f('fecha')) .'","'.$temp->f('moneda') .'","'.
					number_format($temp->f('valor'),2).'","' . $temp->f('estad').'","' . $temp->f('empresa').'","' . $temp->f('tipo').'"';
	        $conte .= ',"' . $temp->f('tjta').'"';
			if (strpos($_REQUEST['querys6'], 'tbl_aisOrden') > 2)
	            $conte .= ',"' . $temp->f('titord').'"';
