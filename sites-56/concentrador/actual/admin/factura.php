...
//	lee los datos de la transferencia
	$query = "select c.nombre comercio, t.*, r.*,
                case t.idadmin when null then null else (select a.nombre from tbl_admin a where a.idadmin = t.idadmin) end autor
				from tbl_transferencias t, tbl_comercio c, tbl_transacciones r 
				where r.idtransaccion = t.idTransf and t.idCom = c.id and idTransf = '$id'";
// 	echo "<br>".$query;
	$temp->query($query);
	$arrSal = $temp->loadAssocList();
...
//        print_r($item);
	...
	if ($_SESSION['grupo_rol'] <= 1 || $_SESSION['grupo_rol'] == 18) {
		...
		$arrIdio = array ('P', 'A');
		$arrEtiq = array('Pendiente', 'Pagada');
	...
	if ($f = fopen ( $sFilename, "r" )) {
		...
		fclose ( $f );
			...
			$mensaje = str_replace ( '{idfactura}', $id, 
					str_replace ( '{clientenombre}', $temp->f('cliente'), 
					str_replace ( '{servicio}', str_replace("<br>", "", str_replace("\n", "", $temp->f('concepto'))), 
					str_replace ( '{serv}', _SERV, 
					str_replace ( '{val}', _VAL, 
					str_replace ( '{valor}', number_format ( ($temp->f('valor') / 100), 2 ) . ' ' . $temp->f('moneda'), 
					str_replace ( '{pagara}', $temp->f('cuenta'), 
