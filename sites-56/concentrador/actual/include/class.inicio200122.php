		...
		if ($this->pasa != 1) { //Si la pasarela es la de prueba salta todo
			...
			if ($this->comer != '122327460662') { // si no es Prueba
				if ($this->segura == 0) {//Para las pasarelas sin 3D
					...
					$this->db("select $term pasarela from tbl_colComerPasar c, tbl_pasarela p, tbl_colTarjPasar j where j.idPasar = p.idPasarela and $term = p.idPasarela and c.idcomercio = " . $this->comer . " and (" . time() . " between c.fechaIni and c.fechaFin) and p.secure = 0 and j.idTarj = " . $this->amex);
