...
// 						$this->pasa = $this->temp->f ( 'idPasarela' );
		...
		if ($this->pasa != 1) { //Si la pasarela es la de prueba salta todo
			...
			if ($this->comer != '122327460662' && $this->comer != '129025985109') { // si no es Prueba
				if ($this->segura == 0) {//Para las pasarelas sin 3D
					...
					$this->db("select $term pasarela from tbl_colComerPasar c, tbl_pasarela p, tbl_colTarjPasar j, tbl_colPasarMon m where j.idPasar = p.idPasarela and m.idpasarela = p.idPasarela and m.idmoneda = ".$this->mon." and m.estado = 1 and $term = p.idPasarela and c.idcomercio = " . $this->comer . " and (" . time() . " between c.fechaIni and c.fechaFin) and p.secure = 0 and j.idTarj = " . $this->amex);
					...
					if ($this->pweb == 0) { //si no es pago desde web busco el array 
						...
						$this->db("select r.idPasarela from tbl_rotComPas r, tbl_colTarjPasar j, tbl_colPasarMon m where j.idPasar = r.idPasarela and m.idpasarela = r.idPasarela and m.idmoneda = ".$this->mon."  and r.idcom = " . $this->datCom['id'] . " and r.activo = 1 and r.horas = 0 and j.idTarj = " . $this->amex ." order by r.orden");
						if ($this->temp->num_rows() > 0) {
							$this->psArray = $this->temp->loadResultArray();
	...
	/**
	 * Chequeo de los límites de Fincimex
	 *
	 * @return void
	 */
	private function ChequeoAis() {
		...
		if (date('n') >= 1 && date('n') <= 3) { // fechas del primer trimestre
			...
			if ($this->datAis['idremitente'] == '2622') return true;
		}
		...
		$this->db($q);
		if (($this->temp->f('idtitanes') * 1) == 0) {
			$this->err = $causa = "El Beneficiario tiene identificador 0:";
			$this->log .= $causa;
			return false;
		}
		...
		if (count($arrLimCom) > 0 ) {
			...
			if ($arrLimCom['limxoper'] > -1) {
				...
				if (($this->imp / 100) > $arrLimCom['limxoper']) {
					...
					$this->log .= $mes . "\n<br>";
					$this->arrMD[] = $this->err = "Máximo por operación con esta (".number_format(($this->imp / 100),2).") y el comercio tiene permitido{2}:".number_format($arrLimCom['limxoper'],2);
					return false;
				}
			}
			...
			if ($arrLimCom['limxdia'] > -1) {
				...
				if (($this->temp->f('valor') + $this->imp / 100) >= $arrLimCom['limxdia']) {
					...
					$this->log .= $mes . "\n<br>";
					$this->arrMD[] = $this->err = "Máximo por día, con esta operación (".number_format(($this->temp->f('valor') + $this->imp / 100),2)."), el comercio sobrepasó lo permitido para el día{4}:".number_format($arrLimCom['limxdia'],2);
					return false;
				}
			}
			...
			if ($arrLimCom['limadia'] > -1) {
				...
				if (($this->temp->f('valor') + $this->imp / 100) >= ($arrLimCom['limadia'] * date(j))) {
					...
					$this->log .= $mes . "\n<br>";
					$this->arrMD[] = $this->err = "Máximo acumulado por día, hasta el día de hoy acumula el comercio (".number_format(($this->temp->f('valor') + $this->imp / 100),2).") tiene situado para hoy{4}:".number_format(($arrLimCom['limadia'] * date(j)),2);
					return false;
				}
			}
			...
			if ($arrLimCom['limxmes'] > -1) {
				...
				if (($this->temp->f('valor') + $this->imp / 100) >= $arrLimCom['limxmes']) {
					...
					$this->log .= $mes . "\n<br>";
					$this->arrMD[] = $this->err = "Máximo por mes, el comercio con esta operación llega a ".number_format(($this->temp->f('valor') + $this->imp / 100),2)." y tiene situado{5}:".number_format($arrLimCom['limxmes'],2);
					return false;
				}
			}
			...
			if ($arrLimCom['limxano'] > -1) {
				...
				if (($this->temp->f('valor') + $this->imp / 100) >= $arrLimCom['limxano']) {
					...
					$this->log .= $mes . "\n<br>";
					$this->arrMD[] = $this->err = 'Máximo por año, el comercio con esta operación llega a '.number_format(($this->temp->f('valor') + $this->imp / 100),2). ' y tiene permitido'."{6}:".number_format($arrLimCom['limxano'],2);
					return false;
				}
			}
			...
			if ($arrLimCom['cantxdia'] > -1) {
				...
				if ($this->temp->f('valor') >= $arrLimCom['cantxdia']) {
					...
					$this->log .= $mes . "\n<br>";
					$this->arrMD[] = $this->err = 'Máximo de operaciones por día, el comercio acumula hoy '.$this->temp->f('valor'). ' y tiene permitido'."{7}:".($arrLimCom['cantxdia']);
					return false;
				}
			}
			...
			if ($arrLimCom['cantxmes'] > -1) {
				...
				if ($this->temp->f('valor') >= $arrLimCom['cantxmes']) {
					...
					$this->log .= $mes . "\n<br>";
					$this->arrMD[] = $this->err = 'Máximo de operaciones por mes, el comercio acumula hasta hoy '.$this->temp->f('valor'). ' y tiene permitido'."{8}:".$arrLimCom['cantxmes'];
					return false;
				}
			}
			...
			if ($arrLimCom['cantxano'] > -1) {
				...
				if ($this->temp->f('valor') >= $arrLimCom['cantxano']) {
					...
					$this->log .= $mes . "\n<br>";
					$this->arrMD[] = $this->err = 'Máximo de operaciones por año, el comercio acumula hasta hoy '.$this->temp->f('valor'). ' y tiene permitido'."{9}:".$arrLimCom['cantxano'];
					return false;
				}
			}
	}
	...
	private function CheqLimites($verif = true) {
		...
		if ($this->comer != '129025985109' && $this->pasa != 1) {
			...
			if (!in_array($this->pasa, $arrPa)) {
				$this->log .= $causa = "Cambio de pasarela, no es válida para el comercio{1}:".$this->pasa;
				$pase = 0;
			}
		}
		...
		if ($this->comer != '122327460662') {
			...
			if ($this->pasa == 44 || $this->pasa == 52) { // Si es de Sabadell3 o Bankia6 y además de alguno de los comercios
				if (in_array($this->comer, $arrCom) && $oprR > 0) {
						$this->log .= $causa = "Cambio de pasarela obligado operación repetida en menos de 0.5 horas{1}:";
						$pase = 0;
					}
			}
		}
		...
		if ($this->comer != '122327460662') {
			if ($pase && $this->ip != '127.0.0.1') {
				...
				$this->db($q);
				if ($this->temp->f('id')) {
					$this->log .= $causa = "Cambio de pasarela por país prohibido{1}:".$this->damepais();
					$pase = 0;
				}
			}
		}
		...
		if ($pase) {
			...
			if (($this->imp / 100) <= $datPas['LimMinOper']) {
				...
				$pasV = $this->pasa;
				$pase = 0;
				$causa = 'm&iacute;nimo por operación{3}:'.number_format($datPas['LimMinOper'],2);
				// if ($datPas['secure'] == 1) return false;
			}
		}
		...
		if ($pase) {
			...
			if (($this->imp / 100) > $datPas['LimMaxOper']) {
				...
				$pasV = $this->pasa;
				$pase = 0;
				$causa = 'm&aacute;ximo por operación{2}:'.number_format($datPas['LimMaxOper'],2);
				// if ($datPas['secure'] == 1) return false;
			}
		}
		...
		if ($pase) {
			...
			if (($this->temp->f('valor') + $this->imp / 100) >= ($datPas['LimDiar'] * $incPorc)) {
				...
				$pasV = $this->pasa;
				$pase = 0;
				$causa = 'montos diarios{1}:'.number_format(($datPas['LimDiar'] * $incPorc),2);
				// if ($datPas['secure'] == 1) return false;
			}
		}
		...
		if ($pase) {
			...
			if (($this->temp->f('valor') + $this->imp / 100) >= $datPas['LimMens']) {
				...
				$pasV = $this->pasa;
				$pase = 0;
				$causa = 'monto mensual{1}:'.number_format($datPas['LimMens'],2);
				// if ($datPas['secure'] == 1) return false;
			}
		}
		...
		if ($pase) {
			...
			if (($this->temp->f('valor') + $this->imp / 100) >= $datPas['LimAnual']) {
				...
				$pasV = $this->pasa;
				$pase = 0;
				$causa = 'monto anual{1}:'.number_format($datPas['LimAnual'],2);
				// if ($datPas['secure'] == 1) return false;
			}
		}
...
function envioSMSen($id, $mens) {
	...
	function alerSegur() {
		...
		if ($this->imp >= leeSetup('montoAlerta') * 100) {
			...
			if (!$this->envioSMSen(2,$mensage)) error_log("Error en el envío de los sms de límites");
		}
