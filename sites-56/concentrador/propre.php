...
if (isset($d['func']) && $d['func'] == 'firma') {
	echo json_encode(array('sha' => convierte256($d['comercio'], $d['transaccion'], $d['importe'], $d['moneda'], $d['operacion'])));
} else {
	...
	if (isset($d['comercio']) && isset($d['transaccion']) && isset($d['importe']) && isset($d['moneda']) && isset($d['operacion']) && isset($d['firma'])) {
		...
		if ($temp->num_rows() != 1) muestraError("Datos no coresponden a una Preautorizacion", $correoMi);
		$arrVal = $temp->loadAssocList();
		$arrVal = $arrVal[0];
		$arrIdent = explode('&', $arrVal['identificadorBnco']);

		//chequeo de importe por encima del que tiene la operación
		if ($arrVal['valor_inicial'] < $arrEnt['imp']) muestraError("Importe a cobrar supera el valor de la operacion", $correoMi);

		//poner el importe que tiene la oepración para las liberaciones
		if ($arrEnt['opr'] == 'L') $arrEnt['imp'] = $arrVal['valor_inicial'];
