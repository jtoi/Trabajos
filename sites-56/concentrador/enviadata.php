...
if (is_array($d) && count($d) > 0) {
	...
	error_log("hola");
	error_log("Desde la IP- ".GetIP()." ".addslashes($sql));
	error_log("aloh");

//	$ch = curl_init();
	($d['lugar'] == 'alla') ? $url = 'https://www.administracomercios.com/ejec.php' : $url = "http://localhost/concentrador/ejec.php";
...
<div class="largo">
	...
	format(<?php echo $elem; ?>, 2) valor,
	round(t.tasa,4) tasaM,
	format(<?php echo $elem; ?>, 2) euroEquiv,
...
where r.idTransf = t.idtransaccion
	...
	and p.tipo = 'T'
	and t.fecha > <?php echo $Mesatras; ?>
	and t.idcomercio like '%'
...
limit 0,200"
		...
		<input type="radio" class="quer" value="select t.idtransaccion, t.identificador, o.titOrdenId, format((t.valor/100),2) 'valor',
...
order by (c.idcimex *1) desc"
	...
	<div class="dos">
		<input type="radio" class="quer"
			value="select format(sum(<?php echo $elem; ?>) / <?php echo date('j'); ?> * , 2) 'Estimado'
FROM tbl_transacciones t
where estado in ('A','V')
	and tipoEntorno = 'P'
	and fecha_mod > '<?php echo $iniMes; ?>' "
			id="estMes" name="prueba" /> <label for="estMes">Estimado en el mes</label><br />
		<input type="radio" class="quer"
			value="select from_unixtime( fecha_mod, '%d/%m/%y - %W' ) as 'D&iacute;a', format(sum(<?php echo $elem; ?>),2) 'Valor', count(*) Transacc,
	format(sum(<?php echo $elem; ?>) / count(*), 2) 'val/trans'
FROM tbl_transacciones t
where estado in ('A','V')
	and tipoEntorno = 'P'
	and fecha_mod > '<?php echo $iniMes; ?>'
...
ORDER BY fecha_mod desc"
		...
		<input type="radio" class="quer"
			value="select date_add(from_unixtime(fecha_mod,'%Y-%m-01 23:59'), interval -1 day) as 'Mes', format(sum(<?php echo $elem; ?>),2) 'Valor',
	count(*) Transacc, format(sum(<?php echo $elem; ?>)/count(*), 2) 'val/trans'
FROM tbl_transacciones t
where estado in ('A','V')
	and tipoEntorno = 'P'
	and fecha_mod > '<?php echo $ini13A; ?>'
 ...
 order by fecha_mod desc"
		...
		<input type="radio" class="quer"
			value="select date_add(from_unixtime(fecha_mod,'%Y-%m-01 23:59'), interval -1 day) as 'Mes', format(sum(<?php echo $elem; ?>),2) 'Valor',
	count(*) Transacc, format(sum(<?php echo $elem; ?>)/count(*), 2) 'val/trans'
FROM tbl_transacciones t where estado in ('A','V') and tipoEntorno = 'P' and fecha_mod > '<?php echo $iniAno; ?>'
...
from (select estado, idtransaccion from tbl_transacciones where estado in ('A','V')  and fecha_mod > '<?php echo $iniDia; ?>' and tipoEntorno = 'P'
union all select estado, idtransaccion from tbl_transacciones where estado != 'A' and fecha_mod > '<?php echo $iniDia; ?>' and tipoEntorno = 'P') c
group by c.estado"
			id="transDiaAcep" name="prueba" /> <label for="transDiaAcep">Estado de transacciones en el d&iacute;a</label><br />
		<input type="radio" class="quer"
			value="select estado, count(idtransaccion) cantidad
from (select estado, idtransaccion from tbl_transacciones where estado in ('A','V')  and fecha_mod > '<?php echo $iniMes; ?>' and tipoEntorno = 'P'
union all select estado, idtransaccion from tbl_transacciones where estado != 'A' and fecha_mod > '<?php echo $iniMes; ?>' and tipoEntorno = 'P') c
group by c.estado"
			id="transMAcep" name="prueba" /> <label for="transMAcep">Estado de transacciones en el mes</label><br />
		<input type="radio" class="quer"
			value="select c.nombre, format(sum(<?php echo $elem; ?>),2) 'Valor', count(*) Transacc, format(sum(<?php echo $elem; ?>) / count(*), 2) 'val/trans'
FROM tbl_comercio c, tbl_transacciones t
where t.idcomercio = c.idcomercio and t.estado in ('A','V') and t.tipoEntorno = 'P' and fecha_mod > '<?php echo $iniDia; ?>'
group by t.idcomercio order by sum(case t.estado when 'A' then euroEquiv else euroEquivDev end) desc"
		...
		<input type="radio" class="quer"
			value="select c.nombre, format(sum(<?php echo $elem; ?>),2) 'Valor', count(*) Transacc, format(sum(<?php echo $elem; ?>) / count(*), 2) 'val/trans'
FROM tbl_comercio c, tbl_transacciones t
where t.idcomercio = c.idcomercio and t.estado in ('A','V') and t.tipoEntorno = 'P' and fecha_mod > '<?php echo $iniMes; ?>'
group by t.idcomercio order by sum(case t.estado when 'A' then euroEquiv else euroEquivDev end) desc"
...
order by fecha_mod desc"
		...
		<input type="radio" class="quer"
               value="select a.nombre, b.texto, from_unixtime(b.fecha,'%d/%m/%Y %H:%i:%s') fech
from tbl_baticora b, tbl_admin a
where a.idadmin = b.idadmin and a.nombre like '%%' and texto like '%%'
	and a.login like '%%' and from_unixtime(b.fecha,'%d/%m/%Y') like '<?php echo $hoy; ?>'
...
from tbl_cambio where from_unixtime(fecha, '%d%m%y') = '' and moneda like '%'"
		...
		<input type="radio" class="quer"
               value="select c.nombre 'comercio', from_unixtime(d.fecha, '%d/%m/%Y %H:%i') 'fecha sol.',
	...
	p.nombre 'pasarela'
from tbl_transacciones t, tbl_devoluciones d, tbl_admin a, tbl_pasarela p, tbl_comercio c, tbl_moneda m
...
order by bloqueada desc, i.fecha desc limit 0,10"
	...
	format(<?php echo $elem; ?>,2) 'EuroEquiv', t.estado, id_error error, tipoEntorno, ip
from tbl_transacciones t, tbl_comercio c, tbl_pasarela p, tbl_moneda m
where t.idcomercio = c.idcomercio and t.pasarela = p.idPasarela and t.moneda = m.idmoneda and tipoEntorno like '%' and fecha_mod > '<?php echo $ini24h; ?>'
order by fecha_mod desc"
...
where b.fecha > <?php echo $ini24h; ?> and a.idadmin != 10 and b.idadmin = a.idadmin and a.idrol = r.idrol and orden >=1
order by b.fecha desc limit 0,40"
			id="Bitac" name="prueba" /> <label for="Bitac">Bit&aacute;cora de las &uacute;ltimas 24hr</label><br />
		<input type="radio" class="quer"
			value="select c.nombre, format(sum(<?php echo $elem; ?>),2) 'Valor', count(*) Transacc, format(sum(<?php echo $elem; ?>) / count(*), 2) 'val/trans'
FROM tbl_comercio c, tbl_transacciones t
where t.idcomercio = c.idcomercio and t.estado in ('A','V') and t.tipoEntorno = 'P' and fecha_mod > <?php echo $iniAno; ?>
...
order by i.fecha desc limit 0,30;"
		...
		<input type="radio" class="quer" value="select format(sum(<?php echo $elem; ?>) / <?php echo date('j'); ?> * <?php echo $diasMesAct; ?>, 2) 'Estimado'
FROM tbl_transacciones t
...
select from_unixtime(t.fecha_mod, '%d/%m/%y - %W' ) as 'D&iacute;a',
	format(sum(<?php echo $elem; ?>),2) 'Valor',
	...
	format(sum(<?php echo $elem; ?>) / count(t.idtransaccion), 2) 'val/trans',
...
where t.estado in ('A','V','B','R')
	and t.tipoEntorno = 'P'
	and t.fecha_mod > '<?php echo $Mesatras; ?>'
...
where t.estado in ('A','V','B','R')
	and t.tipoEntorno = 'P'
	and t.fecha_mod > '<?php echo $ini13A; ?>'
...
select date_add(from_unixtime(fecha_mod,'%Y-%m-01 23:59'), interval -1 day) as 'Mes',
	format(sum(<?php echo $elem; ?>),2) 'Valor',
	count(t.idtransaccion) Transacc,
	format(sum(<?php echo $elem; ?>)/count(idtransaccion), 2) 'val/trans'
FROM tbl_transacciones t
where t.estado in ('A','V','B','R')
	and t.tipoEntorno = 'P'
	and t.fecha_mod > '<?php echo $iniAno; ?>'
...
select case t.estado
	...
	(count(t.idtransaccion)*100/(select count(m.idtransaccion)
		...
		where m.fecha_mod > '<?php echo $iniDia; ?>'
			and m.tipoEntorno = 'P'
			and m.pasarela = t.pasarela)) '%'
from tbl_transacciones t
where t.fecha_mod > '<?php echo $iniDia; ?>'
	and t.tipoEntorno = 'P'
group by t.estado;
...
select case t.estado
	...
	(count(t.idtransaccion)*100/(select count(m.idtransaccion)
		from tbl_transacciones m
		where m.fecha_mod > '<?php echo $iniMes; ?>'
			and m.tipoEntorno = 'P'
			and m.pasarela = t.pasarela)) '%'
from tbl_transacciones t
where t.fecha_mod > '<?php echo $iniMes; ?>'
	and t.tipoEntorno = 'P'
group by t.estado;
...
select m.moneda,
	...
	(count(t.idtransaccion)*100/(select count(r.idtransaccion)
		...
		where r.moneda = t.moneda
			and r.fecha_mod > '<?php echo $iniDia; ?>'
			and r.tipoEntorno = 'P'
...
where t.moneda = m.idmoneda
	and t.estado = 'A'
	and t.tipoEntorno = 'P'
	and t.fecha_mod > '<?php echo $iniDia; ?>'
group by t.moneda;
...
select m.moneda,
	...
	(count(t.idtransaccion)*100/(select count(r.idtransaccion)
		from tbl_transacciones r
		where r.moneda = t.moneda
			and r.fecha_mod > '<?php echo $iniMes; ?>'
			and r.tipoEntorno = 'P')) '%'
from tbl_transacciones t, tbl_moneda m
where t.moneda = m.idmoneda
	and t.tipoEntorno = 'P'
	and t.estado = 'A'
	and t.fecha_mod > '<?php echo $iniMes; ?>'
group by t.moneda;
...
select p.nombre pasarela,
	...
	(select count(r.idtransaccion)
		...
		where r.pasarela = t.pasarela
			...
			and r.estado = 'A'
			and r.fecha_mod > '<?php echo $iniDia; ?>') 'cantA',
	(select count(r.idtransaccion)
		from tbl_transacciones r
		where r.pasarela = t.pasarela
			and p.tipo = 'P'
			and r.moneda = t.moneda
			and r.estado = 'A'
			and r.fecha_mod > '<?php echo $iniDia; ?>')/count(idtransaccion)*100 '%'
from tbl_transacciones t, tbl_pasarela p, tbl_moneda m
where t.moneda = m.idmoneda
	and t.pasarela = idPasarela
	and p.tipo = 'P'
	and t.tipoEntorno = 'P'
	and fecha_mod > '<?php echo $iniDia; ?>'
...
select p.nombre pasarela,
	...
	(select count(r.idtransaccion)
		...
		where r.pasarela = t.pasarela
			and r.moneda = t.moneda
			and r.estado = 'A'
			and r.fecha_mod > '<?php echo $iniMes; ?>') 'cantA',
	(select count(r.idtransaccion)
		from tbl_transacciones r
		where r.pasarela = t.pasarela
			and r.moneda = t.moneda
			and r.estado = 'A'
			and r.fecha_mod > '<?php echo $iniMes; ?>')/count(idtransaccion)*100 '%'
from tbl_transacciones t, tbl_pasarela p, tbl_moneda m
where t.moneda = m.idmoneda
	and t.pasarela = p.idPasarela
	and t.tipoEntorno = 'P'
...
where c.idcomercio = t.idcomercio and t.moneda = m.idmoneda and p.idPasarela = t.pasarela and from_unixtime(t.fecha, '%d%m%y') = ''
	and t.estado not in ('D','P') and solDev like '%'
order by t.fecha_mod desc"
...
order by t.fecha_mod desc"
			id="transBusc" name="prueba" /> <label for="transBusc">Buscar transacci&oacute;n</label><br />
					<input type="radio" class="quer"
               value="select format(sum(<?php echo $elem; ?>),2) Acumulado, format(sum(<?php echo $elem; ?>) / <?php echo date('j'); ?> *
               <?php echo $diasMesAct; ?>, 2) 'Estimado', format(sum(<?php echo $elem; ?>) / <?php echo date('j'); ?>, 2) 'Promedio'
FROM tbl_transacciones t
where estado in ('A')
	and tipoEntorno = 'P'
	and fecha_mod > <?php echo $iniMes; ?>;
...
select from_unixtime( t.fecha_mod, '%d/%m/%y - %W' ) as 'D&iacute;a',
	...
	(select count(i.idtransaccion)
		from tbl_transacciones i
		where from_unixtime(t.fecha_mod,'%d%m%Y') = from_unixtime(i.fecha_mod,'%d%m%Y')
			and i.fecha_mod > (<?php echo $iniDia; ?>-(86400*1))
			and i.tipoEntorno = 'P') 'Tot trans',
...
where t.estado in ('A','V','B','R')
	and t.tipoEntorno = 'P'
	and t.fecha_mod > (<?php echo $iniDia; ?>-(86400*1))
GROUP BY from_unixtime( t.fecha_mod, '%d/%m/%y' )
order by t.fecha_mod desc;
...
select case t.estado
	...
	format((count(t.idtransaccion)*100/(select count(r.idtransaccion)
		from tbl_transacciones r
		where r.fecha_mod > '<?php echo $iniDia; ?>'
	...
	format((select count(n.idtransaccion)
		...
		where n.estado = t.estado
			and t.tipoEntorno = 'P'
			and n.fecha_mod > '<?php echo ($iniDia-(30*24*60*60)); ?>')*100/(select count(s.idtransaccion)
		from tbl_transacciones s
		where s.tipoEntorno = 'P'
			and s.fecha_mod > '<?php echo ($iniDia-(30*24*60*60)); ?>'),2) '% - 30'
...
select concat(p.nombre,' (',p.idPasarela, ') ') pasarela, case LimDiar when '100000000' then 'S/L' else format(LimDiar,2) end 'Lim Diario',
	format(sum(<?php echo $elem; ?>),2) 'Acum D&iacute;a' ,
...
select concat(c.nombre,' ',c.id,'-',t.idcomercio) comercio,
	...
	format(count(t.idtransaccion) *100/(select count(n.idtransaccion)
		...
		where n.tipoEntorno = 'P'
			and n.fecha_mod > '<?php echo $iniDia; ?>'
	...
	and fecha_mod > '<?php echo ($iniDia-(30*24*60*60)); ?>' ) *100/(select count(i.idtransaccion)
		from tbl_transacciones i
		where i.tipoEntorno = 'P'
			and i.fecha_mod > '<?php echo ($iniDia-(30*24*60*60)); ?>'
...
where l.idPasarela = t.pasarela
	...
	and a.estado = 'A'
	and t.fecha_mod > <?php echo $ini24h; ?>
group by t.pasarela
order by l.nombre;
</div>
</form>
