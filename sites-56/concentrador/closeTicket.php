...
if (strlen($ticc) >= 1 && strlen($ticc) <= 13) {echo "entra";
...
//	    echo $query;
		...
		if ($temp->num_rows() > 0) {
			...
			$textReem = "Su ticket tiene el número ".$d.". Puede consultar el estado en:<br />".
			...
			$muela = "Estimado(a) $cliente usted abrió el ticket No. $d el pasado $fecha<br /><br />Con asunto: $titulo<br /><br />";
			if ($fecham != '31/12/1969 19:00')
				$muela .= "El mismo fué modificado $fecham <br /><br />";
			$muela .= "Lea detenidamente el desarrollo del mismo, si desea argumentar algo más escriba al final. Si por el contrario está satisfecho<br />con la respuesta
						que el administrador le ha dado por favor ciérrelo oprimiendo el botón Cerrar Ticket.<br /><br />
						<span style='font-weight:bold;font-size:1.2em'>Desarrollo del Ticket</span><br /><br />$texto<br /><br />";
			$titulo = 'Ticket no. '.$d;
		} else
			$ponerAhi = "<div style=\"text-align:center; margin-top: 20px; color:green; font-family:Arial sans-serif; font-size:11px\">
							El Ticket ya se encuentra cerrado, deberá entrar al sitio y crear uno nuevo.</div>";

	} else
			$ponerAhi = "<div style=\"text-align:center; margin-top: 20px; color:green; font-family:Arial sans-serif; font-size:11px\">
							No es un ticket válido.</div>";
}
...
//	echo $query;
   ...
   $arrayTo = array(
	   array('Luis Mourelos', 'koldo@amfglobalitems.com'),
	   array('Julio Toirac', 'jotate@amfglobalitems.com'),
