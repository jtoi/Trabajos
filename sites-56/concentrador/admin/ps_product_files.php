...
class ps_product_files {
	...
	***************************************************************************/
	function validate_add( &$d ) {
		...
		if (!empty($_FILES["file_upload"]["name"])) {
			$q = "SELECT count(*) as rowcnt from tbl_{vm}_product_files WHERE";
	...
	***************************************************************************/
	function validate_delete( $file_id, &$d ) {
		...
		$q_dl = "SELECT attribute_value, attribute_name,file_id from tbl_{vm}_product_attribute,tbl_{vm}_product_files WHERE ";
	...
	**************************************************************************/
	function add( &$d ) {
		...
		$q = "INSERT INTO tbl_{vm}_product_files ";
	...
	**************************************************************************/
	function update( &$d ) {
		...
		$q_dl = "SELECT attribute_name,file_id from tbl_{vm}_product_attribute,tbl_{vm}_product_files WHERE ";
		$q_dl .= "product_id='".$d["product_id"]."' AND attribute_name='download' ";
		...
		if( $db->next_record() ) {
			...
			if( !empty($_FILES['file_upload']['name'])) {
				// new file uploaded
				$qu = "UPDATE tbl_{vm}_product_attribute ";
		...
		if( !empty($_FILES['file_upload']['name']) ) {
			...
			$q = "INSERT INTO tbl_{vm}_product_files SET ";
			if( !empty($_FILES['file_upload']['name'])) {
				$q .= "file_id='" . $d["file_id"] . "', ";
			...
			$q = "UPDATE tbl_{vm}_product_files SET ";
			if( !empty($_FILES['file_upload']['name'])) {
				$q .= "file_name='" . $filename."', ";
				$q .= "file_extension='$ext', ";
				$q .= "file_mimetype='" .$_FILES['file_upload']['type']."', ";
	...
	*/
	function delete_record( $record_id, &$d ) {
		...
		$q = "SELECT file_name,file_is_image FROM `tbl_{vm}_product_files` WHERE file_id='$record_id'";
		$dbf->setQuery($q);
		$dbf->query();
		$dbf->next_record();
		...
		$q = "DELETE FROM tbl_{vm}_product_files WHERE file_id='$record_id'";
		$dbf->setQuery($q);
		$dbf->query();
	...
	**************************************************************************/
	function get_file_list( $product_id ) {
		...
		$dbf = new ps_DB;
		$html = "";
		$sql = 'SELECT attribute_value FROM tbl_{vm}_product_attribute WHERE `product_id` = \''.$product_id.'\' AND attribute_name=\'download\'';
		$dbf->query( $sql );
		$dbf->next_record();
		...
		$sql = 'SELECT DISTINCT file_id, file_mimetype, file_title, file_name'
			. ' FROM `tbl_{vm}_product_files` WHERE ';
		if( $exclude_filename ) {
