...
if ($d['modificar']) {
	...
	if (!_CAMB_LIM) {//verifico si el cambio de limites por moneda está habilitado
		...
		if (strpos($d['monedaE'],',')) $arrMon = explode("', '", $d['monedaE']);
			...
			$temp->query("update tbl_colPasarMon set estado = '".$d['actm']."' where idpasarela = '".$d['modificar']."' and idmoneda = '".$d['monedaE']."'");
		}
	...
	$temp->query($q);
	if (_MOS_CONFIG_DEBUG) echo "<br>".$q."<br>";
...
from tbl_pasarela where idPasarela = ".$d['cambiar'];
	...
	if (!_CAMB_LIM) {//verifico si el cambio de limites por moneda está habilitado
		...
		$q = "select estado from tbl_colPasarMon where idmoneda = '$valSel' and idpasarela = ".$d['cambiar'];
		$temp->query($q);
		$avl = $temp->f('estado');
		$valInicio = array('0','1');
		$etiq = array('No', 'Si');
		$html->inRadio('Moneda Activa', $valInicio, 'actm', $etiq, $avl);
...
$vista = "select p.idPasarela id, p.nombre, ".
			...
			"LimOperDia, p.idempresa, idagencia, amex, a.nombre agencia, e.nombre empre ".
		"from tbl_pasarela p, tbl_empresas e, tbl_agencias a ";
...
//					alert(vale.valor);
			...
			if (datos.est.length == 0){
				...
				$("#div_actm").show();
				if (datos.est == '0') $("#0_actm").prop('checked', true);
				
				if (datos.est == '1') $("#1_actm").prop('checked', true);
