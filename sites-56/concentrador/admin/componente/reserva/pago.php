...
//calcula los máximos para las operaciones por pasarelas segura y no segura
$q = "SET GLOBAL group_concat_max_len = 2048";
// $temp->query($q);
if (!_CAMB_LIM) {//verifico si el cambio de limites por moneda está habilitado
	$q = "select ceil(LimMaxOper) from tbl_pasarela where idPasarela in (select group_concat(distinct pasarelaAlMom) from tbl_comercio where id in ($comer)) and secure = 0";
} else {
	$q = "select ceil(valor) 'LimMaxOper' from tbl_colPasarLimite where idLimite = 2 and idPasar in (select group_concat(distinct pasarelaAlMom) from tbl_comercio where id in ($comer))";
}
echo $q;
$temp->query($q);
$result = $temp->f('LimMaxOper');
...
// 	$(".amex").click(function(){
// 		if ($(this).val() == 1) {
//			alert ('<?php echo _COMERCIO_EXPLICA; ?>');
// 		}
// 	});
