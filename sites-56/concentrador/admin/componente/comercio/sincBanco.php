...
$correo = new correo;
$error = '';
$GLOBALS["statesDevC_AB_array"] = [];
$GLOBALS["statesAC_DevB_array"] = [];
$GLOBALS["bank_ops_array"] = [];
$GLOBALS["tefpay_operations"] = [];
$GLOBALS["AB_DenegConc"]=[];
$GLOBALS["redsys_file_headers"] = [];
					...
					while (!feof($handle)) {
						...
						if (strlen($buffer) > 3) {
							...
							if ($_REQUEST['banco'] == 9) {// Stripe
								...
								if (isset($arrDat[0])) {
                                    array_push($GLOBALS["tefpay_operations"], $arrBuff);
                                    $error .= buscatr($arrDat);
                                }
								...
								if (!stripos($buffer, ";;;")) {
                                    ...
                                    redsys_position_headers($arrBuff);
									...
									if (!strstr($arrBuff[0],'Fecha')) {
										if (!strstr($arrBuff[6], 'Reintentada') ) {
											...
											if (strlen($arrBuff[0]) < 5) {
												...
												$arrDat[3] = $arrBuff[0];
                                                //fecha en human
												$arrDat[2] = $arrBuff[5];														//identificador de la operación//-laynier: de casualidad
                                                $id_op_posit=getheaderposition("pedido");
                                                $arrDat[2] = $id_op_posit ? $arrBuff[$id_op_posit] : $arrBuff[5];	//mejor, no?;

                                                $tipo_op = getheaderposition("Tipo operaci");
                                                $tipo_op_pos = $tipo_op ?: 4; //quedarse con el cable en caso extremo
                                                if (stristr($arrBuff[$tipo_op_pos], 'Autorizac') !==false) {//!==false es la forma correcta									//Estado de la Operación
													$arrDat[6] = 'A';
													$response_pos = getheaderposition("Resultado");
                                                    $result = $response_pos ?: 6;
                                                    if (strpos($arrBuff[$result], 'Autorizada') !== false){
														...
														$arrDat[0] = '';
														$arrDat[2] =$id_op_posit ? $arrBuff[$id_op_posit] : $arrBuff[5];
													}
												} else if (stristr($arrBuff[4], 'Devoluc') !== false) {
													// echo "devuelta<br>";
													if (strpos($arrBuff[6], 'Autorizada') !== false) {//devuelta aceptada la devolución
														$arrDat[6] = 'B';
													} else { //devuelta pero no aceptada la devolución
														$arrDat[6] = 'AB';
													}
												}														//fecha en human

												//cajamar
                                                /* //ya no hace falta 309-322 pq no importa en la posicion que venga la columna
												if (stristr($arrBuff[3], 'Autorizac') !== false) {									//Estado de la Operación
													$arrDat[6] = 'A';
													if (strpos($arrBuff[$response_pos], 'Autorizada') !== false) {
                                            ...
                                            $cur_pos = getheaderposition("Moneda");
                                            $currency_type = $arrBuff[$cur_pos];
                                            $eur_val_pos = getheaderposition("Euros");
											$arrBuff[$eur_val_pos] = str_replace(",", ".", $arrBuff[$eur_val_pos]);
											...
											if (strpos($arrBuff[$eur_val_pos], 'EUR') !== false) {
												$arrDat[4] = str_replace(" EUR", "", $arrBuff[$eur_val_pos]);								//monto de la operac
											 } else {
											 	$arrDat[4] = $arrBuff[$eur_val_pos];				//euros en el banco
											}
                                            //xdebug_break();
                                            $card_type_pos = getheaderposition("Tipo de pago");
											$arrDat[7] = tarje($arrBuff[$card_type_pos], $arrBuff[$id_op_posit]);													//tipo de tarjeta
											// echo "<br>arrDat=". json_encode($arrDat); exit;
											$error .= buscatr($arrDat);
                                            array_push($GLOBALS["bank_ops_array"], $arrBuff); //va guardando las ops analizadas para reanalizar.
										}
									}
								}
...
function tarje ($cont, $id, $num = null) { //xdebug_break();
	...
	if (stripos(strtolower($cont), 'visa') !==false) return 2;
	if (stripos(strtolower($cont), 'master') !==false) return 3;
	if (stripos(strtolower($cont), 'amex') !==false || stripos(strtolower($cont), 'american') !==false) return 1;
...
//esta function guarda la position de cada columna en el archivo, no importa cuantas sean
function redsys_position_headers($arrBuff){
    if (count( $GLOBALS["redsys_file_headers"])==0){
        for ($i=0; $i < count($arrBuff); $i++){
            $GLOBALS["redsys_file_headers"][$arrBuff[$i]] = $i;
        }
    }
}
//obtener la posicion de un encabezado de redsys
function getheaderposition($needle_string){
    $header_array=$GLOBALS["redsys_file_headers"];//comodidad, se vale
    $p = false;
    foreach ($header_array as $header=>$position){
        if (stripos($header, $needle_string) !== false){//esto no es ocioso ya que la pos 0 es false tambien
            //si es el header que busco dame su posicion
            $p = $position;
            break;
        }
    }
...
function buscatr($arrEnt, $pas=null) {
		// echo "pas=$pas - ".json_encode($arrEnt)."<br>";
		global $psrla;
		$temp = new ps_DB;
		$arrCub = array();
		$inst = false;
...
//		var_dump($arrEnt);
	...
	// echo $q."<br>";
		...
		if ($temp->num_rows() == 1) { // la transacción se encuentra en el Concentrador recojo sus datos
            ...
            /* Reina - Ya no se va a calcular la tasa de Cimex aqui sino en buscaTasa por el cron de las 8:00 am Cuba
	...
	// print_r($arrCub);
			...
			if (!$acep &&  (strlen($arrEnt[0]) > 3) && (substr($arrEnt[0], 0, 1) ==! "0")) {// la operación no está Aceptada en el Concentrador pero viene Aceptada del banco
                $fail_id = $arrCub[7];
                $fail_text = "<strong>ERROR:</strong> La operación ".$arrCub[7]." de ".$arrCub[3]." que se realizó por {$arrCub[5]} está Aceptada en Banco pero no en el Concentrador!<br>\n";
                if (!in_array($fail_id, array_keys($GLOBALS['AB_DenegConc']))){
                    $GLOBALS['AB_DenegConc'][$fail_id] = $fail_text;
                }
			...
			} elseif ($acep &&  strlen($arrEnt[0]) == 0 && $arrEnt[6] == 'A') { //operación Aceptada en el Concentrador y denegada en Banco
			...
			if ($arrEnt[6] == 'B'){
				if ($arrCub[0] == 'A' ) {
                    $fail_id = $arrCub[7];
                    $fail_text =  "<strong>ERROR:</strong> La operación ".$arrCub[7]." de ".$arrCub[3]." que se realizó por {$arrCub[5]} está Aceptada en el Concentrador pero devuelta en el Banco!<br>\n";

                    if (!in_array($fail_id, array_keys($GLOBALS["statesAC_DevB_array"]))){
                        $GLOBALS["statesAC_DevB_array"][$fail_id]=$fail_text;
                    }
				...
				if ($arrCub[0] == 'B' || $arrCub[0] == 'V' || $arrCub[0] == 'R' ) {
                    $fail_id = $arrCub[7];
                    $fail_text = "<strong>ERROR:</strong> La operación ".$arrCub[7]." de ".$arrCub[3]." que se realizó por {$arrCub[5]} está Devuelta en el Concentrador pero aceptada en el Banco!<br>\n";
                    if (!in_array($fail_id, array_keys($GLOBALS["statesDevC_AB_array"]))){
                       $GLOBALS["statesDevC_AB_array"][$fail_id]=$fail_text;
                    }
	...
	// si trae valor en arrDat[4] y NO ES una operacion en EUR
...
//		echo $error;
 ...
 if (count($GLOBALS["statesDevC_AB_array"])> 0){
     //call the function for this case
     foreach (array_keys($GLOBALS["statesDevC_AB_array"]) as $operation_id){
         foreach ($GLOBALS["bank_ops_array"] as $operation){
             if ($operation_id == $operation[5]) {//aparece la operacion
                 $e = true;

                 // $email = 'USER@EXAMPLE.com';
                // echo stristr($email, 'e'); // outputs ER@EXAMPLE.com
                 $str_dev = stristr($operation[4], 'dev');
                 $str_aut = stristr($operation[4], 'aut');

                 //revisa si es la devolucion
                 if (strlen($str_aut)==0 && strlen($str_dev)>0){
                     //si entra aqui ya ha encontrado la devolucion
                     $e=false;
                     break;
                 }
             }// ends aparece la operacion

         }// ends foreach operations
         //hasta aqui ha recorrido las operaciones del archivo. Si $e sigue true
         //en verdad hay un problema.
         if ($e){ $error.= $GLOBALS["statesDevC_AB_array"][$operation_id];}

     }
 }elseif (count($GLOBALS["statesAC_DevB_array"])> 0){
     //call the function for this array
 }elseif (count($GLOBALS["AB_DenegConc"])>0){

     foreach ($GLOBALS["AB_DenegConc"] as $failed_id=>$failed_msg){

         //la condicion deberia ser si quitando el cero quedan 3 caracteres, pero saldra para el 0190 primero
         foreach($GLOBALS["tefpay_operations"] as $tfoperations){
             if ($failed_id == substr($tfoperations[5], 0, 12)){
                 if (($tfoperations[13] == "0190") or ($tfoperations[14] == "0190") ){break 2;}
             }
         }
         $error .= $GLOBALS["AB_DenegConc"][$failed_id];
     }
 }

 //nada es eterno, excepto el amor
unset($GLOBALS["tefpay_operations"], $GLOBALS["AB_DenegConc"], $GLOBALS["bank_ops_array"],
    $GLOBALS["statesAC_DevB_array"],  $GLOBALS["statesDevC_AB_array"]);
if (strlen($error)) {
	echo "<div style='color:red;text-align:center;font-weight: bolder;'>El fichero procesado fu&eacute; $nom y devolvi&oacute; los errores:<br><br> $error <br><br>Fué enviado un correo con todos estos errores a Julio</div>";
	$correo->todo(39, 'Transacciones con problemas en el banco', $error);
} elseif (strlen($_FILES['fichero']['tmp_name']) > 3) 
	echo "<div style='color:green;text-align:center;font-weight: bolder;'> El fichero procesado fu&eacute; $nom y la comprobación fué exitosa, no hay problemas</div>";
