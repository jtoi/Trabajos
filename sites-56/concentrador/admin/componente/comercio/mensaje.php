...
if ($d['mensaje']) $mensaje = caractEspecialesHtml ($d['mensaje']);
// echo $mensaje."<br>";
...
if (!$d['cambiar']) { // Valores para insertar nuevos Art&iacute;culos
...
$temp->query("select id, nombre from tbl_comercio where activo = 'S' order by nombre");
$arrCom = $temp->loadRowList();
array_unshift($arrCom, array("todos", "Todos"));
$html->inSelect('Comercio', 'comer', 3, $arrCom, $comr, '', '', 'multiple=multiple');
$arrVal = array('S','N');
$arrEtq = array('Si','No');
...
$vista = 'select m.idmensaje as id, m.mensaje mensaje, m.fechaInicio, m.fechaFin, m.fecha,
