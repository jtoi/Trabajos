...
<div class="todT">
    ...
    and id not in (25240, 25145, 24785, 22519, 21269, 62575, 62322, 62656, 64773, 62286, 62422, 63282, 64400, 62203, 62469, 62471, 62529, 62600, 62525, 62242, 63892, 62541, 62641, 62396, 63347, 62255, 62195, 62557, 62659, 62353, 62362, 62725, 62184, 62568, 63304, 62237, 63324, 62303, 62208, 62116, 63227, 63230, 63241, 63247, 63254, 63248, 63250, 63253, 63259, 63261, 63262, 63270, 63277, 63278, 63281, 63289, 63301, 63307, 63311, 63315, 63316, 63322, 63327, 63355, 63353, 63356, 63354, 63359, 62547, 62598, 62626, 62589, 62593, 62596, 62599, 62603, 62616, 62619, 62625, 62630, 62635, 62646)
	order by fecha desc
	limit 0,200;
	select id, idtitanes, concat(nombre,' ',papellido,' ',sapellido) cliente, idcimex, usuario,
		correo, numDocumento, from_unixtime(fechaDocumento,'%d/%m/%Y') fecDoc, CP, direccion, ciudad, formateaF(fecha, 10) fec, formateaF(fechaAltaCimex, 10) 'fecCimex',
	...
	limit 0,100" id="OPrrfnD" name="prueba" /> <label for="OPrrfnD">Operaciones Fincimex Detenidas</label><br />
			<input type="radio" class="quer" value="select t.idtransaccion, t.identificador, o.titOrdenId, formateaO((t.valor_inicial/100),2,10) 'valor',
		t.moneda, c.idcimex 'IdCli', c.idtitanes, concat(c.nombre, ' ', c.papellido) cliente, c.usuario, b.idcimex 'IdBen', concat(b.nombre, ' ', b.papellido) beneficiario,
		formateaF(t.fecha, 10) 'fecha', formateaF(t.fecha_mod, 10) 'fechaMod', case t.estado
			when 'P' then 'En Proceso'
	...
	limit 0,100" id="OPrrfnx" name="prueba" /> <label for="OPrrfnx">Operaciones Fincimex</label><br />
			<input type="radio" class="quer" value="select  concat(t.identificador,' ','(', t.idtransaccion, ')',' ',  o.titOrdenId,' ','de ', c.nombre, ' ', c.papellido, ' (', c.usuario, ')',' ','enviado a ',b.nombre, ' ', b.papellido) dato
	from tbl_transacciones t, tbl_aisOrden o, tbl_aisCliente c, tbl_aisBeneficiario b
	where t.idtransaccion = o.idtransaccion
		and b.id = o.idbeneficiario
		and c.id = o.idcliente
		...
		and o.titOrdenId in ()
	order by t.idtransaccion
	limit 0,100" id="OPrrfnx3" name="prueba" /> <label for="OPrrfnx3">Operaciones Fincimex +3000</label><br />
			<input type="radio" class="quer" value="select c.idcimex 'idCliente', c.usuario 'Usuario', concat(c.nombre,' ',c.papellido) 'Cliente', b.idcimex 'idBenef', b.idtitanes, concat(b.nombre,' ',b.papellido) 'Benefic'
	from tbl_aisCliente c, tbl_aisBeneficiario b, tbl_aisClienteBeneficiario r 
	where b.id = r.idbeneficiario and r.idcliente = c.id
	order by (c.idcimex *1) desc" id="borraP" name="prueba" /> <label for="borraP">Ver los Beneficiarios de cada Cliente de AISRemesas</label><br />
			<input type="radio" class="quer" value="UPDATE tbl_aisCliente SET fecha = unix_timestamp(), subfichero = '1', borrficheros = 0 WHERE binary usuario = 'aaa';
	delete from tbl_aisFicheros where idcliente = (select id from tbl_aisCliente where binary usuario = 'aaa')" id="ficsais" name="prueba" /> <label for="ficsais">Poner para que un Cliente vuelva a subir sus ficheros</label><br />
			...
			<input type="radio" class="quer" value="select sum(o.recibe) total1, sum(t.valor) total2 from tbl_aisOrden o, tbl_transacciones t, tbl_aisCliente c where o.idtransaccion = t.idtransaccion and o.idcliente = c.id and t.estado = 'A' and t.fecha between <?php echo $fec1Ais ?> and <?php echo $fec2Ais ?> and c.idcimex = ****;
			select sum(o.recibe) total1, sum(t.valor) total2 from tbl_aisOrden o, tbl_transacciones t, tbl_aisBeneficiario b where o.idtransaccion = t.idtransaccion and o.idbeneficiario = b.id and t.estado = 'A' and t.fecha between <?php echo $fec1Ais ?> and <?php echo $fec2Ais ?> and b.idcimex = ****;"; id="reviss" name="prueba" /> <label for="reviss">Revisa el acumulado trimestral de Cliente y Beneficiario</label><br />
		</div>
		<div class="dos">
	...
	order by d.fecha desc" id="Devolr" name="prueba" /> <label for="Devolr">Devoluciones realizadas</label><br />
			<input type="radio" class="quer" value="delete from tbl_devoluciones where idtransaccion = 'aaaaaaaaaaaa';update tbl_transacciones set solDev = 0 where idtransaccion = 'aaaaaaaa';" id="canDev" name="prueba" /> <label for="canDev">Cancela Devoluciones</label><br />
	...
	create table tbl_trazaBack like tbl_traza_<?php echo date('Ymd') ?>" id="Trzbk" name="prueba" /> <label for="Trzbk">Crea tbl_traza_<?php echo date('Ymd') ?> y pone tbl_trazaBack en 0</label><br />
			...
			<input type="radio" class="quer" value="select from_unixtime(fecha, '%Y') fecha, format(sum(valor/100/tasa),2) 'Valor'
from tbl_transacciones where estado in ('A', 'B', 'V', 'R') and tipoEntorno = 'P'
group by from_unixtime(fecha, '%Y') 
union all
select from_unixtime(fecha, '%Y') fecha, format(sum(valor/100/tasa),2) 'Valor'
from tbl_transaccionesOld where estado in ('A', 'B', 'V', 'R') and tipoEntorno = 'P'
group by from_unixtime(fecha, '%Y') order by fecha desc" id="Histio" name="prueba" /> <label for="Histio">Hist&oacute;rico Empresa</label><br />
