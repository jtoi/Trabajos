...
$html->inTextb('Valor en la factura', $valfact, 'valfac');
$html->inTextb('Valor en el cierre', $valcierr, 'valcierr');
$html->inTextb('Valor en transferencia', $valtransf, 'valtransf');
...
$vista = "select r.cierre, r.idcierre id, r.idtransaccion trasaccion, c.nombre comercio, formateaF(r.fechaCierre, ".$_SESSION['id'].") cles, r.numFacturas, format(r.valor,2) 'valo', m.moneda,
			(select group_concat(nombre separator ',<br>') from tbl_factura f where f.idcierre = r.idcierre) fact,
			(select format(sum(valor),2) from tbl_factura f where f.idcierre = r.idcierre) 'valfact',
			{adiciona} 'vall', 
...
if ($d['valfac'] != ''){
	if (stripos($where, 'having'))
		$where .= " and valfact like '%". $d['valfac']."%'";
	else 
		$where .= " having valfact like '%". $d['valfac']."%'";

}

if ($d['valcierr'] != '' ){
	if (stripos($where, 'having'))
		$where .= " and vall like '%". $d['valcierr']."%'";
	else 
		$where .= " having vall like '%". $d['valcierr']."%'";
...
if ($d['valtransf'] != '' ){
	...
	$adiciona = "(select group_concat(format(t.valor,2) separator ',<br>') from tbl_amfTransf t, tbl_factura f where t.idfactura = f.id and f.idcierre = r.idcierre)";

	$where .= " and r.idcierre in (select f.idcierre from tbl_factura f, tbl_amfTransf a where a.idfactura = f.id and a.valor like '%". $d['valtransf']."%')";
	// else 
	// 	$where .= " having vall like '%". $d['valtransf']."%'";

} else {
	$adiciona = "(select format(sum(t.valor),2) from  tbl_amfTransf t, tbl_factura f where t.idfactura = f.id and f.idcierre = r.idcierre)";
}

$vista = str_replace('{adiciona}', $adiciona, $vista);
...
function verifica() {
	...
	if (numFact > 0) {
		...
		for (i=0;i<=numFact;i++) {
			...
			if ($("#valest"+i).val() > 15) {// hay valor para la transferencia
				ok = false;
