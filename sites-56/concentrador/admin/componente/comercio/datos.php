...
function ejecutaD(){
	...
	var dpvm = dommon = estim = estimBR = datos = estado = moneda = tpv = tpvm = banc = pbanc = pcom = ptpv = bancm = tpvc = comerc = common = commonw = bancos = comseg = comercios = acept = ecept = 0;
	...
	if ($('#commonw').attr('checked') == 'checked') commonw = 1;
	...
	$.ajax({
		...
		success: function (data) {
			...
			if ($("#compa").attr("checked") == 'checked') {
				...
				if (datos.error) alert('hay error');
				$("#muestra1").html('');
				$("#muestra2").html('');
...
$(document).ready(function(){
	$("#compa").click(function(){
		if ($("#compa").attr('checked') == 'checked'){
			...
			$('#commonw').attr('disabled', 'disabled');
...
$html->inCheckBox("Análisis Comercios Web - Monedas", 'commonw', 5, 1);
