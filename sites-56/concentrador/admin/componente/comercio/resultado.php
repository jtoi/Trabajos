...
if ($_POST['trans'] === "F") {
			...
			if (is_file($file)) {
				for ($i=0;$i<count($arrCom);$i++){
					if (stripos($file,$arrCom[$i]) > -1) {
						// if ($_SESSION['grupo_rol'] < 4 || $_SESSION['grupo_rol'] == 5 ) {
						...
						// } else {
							$salida .= "<td style='padding:4px 7px;display:inline;white-space:nowrap;'><div class='dicborr'><a href='" . $diral . $file . "' target='_blank'> " . substr($file, 0, -4) . "</a></div></td></tr>";
						// }
					}
				}
			}
	...
	$arrCom = $temp->loadResultArray();
	$salida = "<tr class='encabezamiento2'>";
	$salida .= "<td>Reclamaciones</td></tr>";
	$i=1;

	$dir = "../../reclamaciones/".$_POST['ano']."/";
	error_log($dir);
	if (file_exists($dir)) {
		// $dir = "../mydir/";
		chdir($dir);
	...
	} else $salida = '<tr><td colspan="2" style="text-align:center;">El directorio para las reclamaciones de '.$comercio.' no existe. Contacte con el administrador.</td></tr>';
