...
//echo $d['moneda'];
	// $d['estado']? $esta = $d['estado']:$esta = "V','B','A','D','P', 'T" ;
	$d['estado']? $esta = $d['estado']:$esta = "T" ;
...
//		echo $query;
	...
	$estadoArr = array(
		array("P', 'A', 'D', 'N', 'B', 'V', 'T", _REPORTE_TODOS)
		, array('T', _REPORTE_PENDIENTE)
		, array('PA', _REPORTE_APOBADA)
		, array('PR', _REPORTE_PROCESO)
		, array('D', _REPORTE_CANCELADA)
...
$vista = "select t.idTransf id, t.idCom, c.nombre 'comercio', t.idadmin, a.nombre 'usr', t.cliente, t.email, t.facturaNum, t.fecha, ".
			"r.fecha_mod, (t.valor/100) 'valor{val}', (r.valor/100/r.tasa) 'euroEquiv{val}', ".
			"case t.estado when 'P' then 
				case 
					when vista = 0 and (t.fecha + 10*86400) > unix_timestamp() = 1 then 'Aprobada'
					when vista = 1 and (t.fecha + 10*86400) > unix_timestamp() = 1 then 'En Proceso'
					when (t.fecha + 10*86400 <= unix_timestamp()) = 1 then 'Vencida'
				else '' end				
			when 'A' then 'Aceptada' when 'T' then 'Pendiente' when 'D' then 'Cancelada' when 'N' then 'No Procesada' end 'estad', ".
			"t.moneda, m.moneda, t.idPasarela, p.nombre 'pasarela', ".
			"case t.activa when 1 then 'Si' else 'No' end 'activa', ".
			"case t.estado when 'P' then 
				case 
					when vista = 0 and (t.fecha + 10*86400) > unix_timestamp() = 1 then 'olive'
					when vista = 1 and (t.fecha + 10*86400) > unix_timestamp() = 1 then 'green'
					when (t.fecha + 10*86400 <= unix_timestamp()) = 1 then 'purple'
				else '' end
			when 'T' then '#00597d' when 'D' then 'red' when 'N' then 'violet' else 'black' end 'color{col}', ".
			"case t.vista when 1 then 'Vista' else '-' end 'vista', case t.enviada when 1 then 'Enviada' else 'Aprobada' end 'enviada ', ".
			"case when r.tipoOperacion = 'T' then 'Transferencia' end 'tipo'";
...
if ($nombreVal) $where .= " and t.facturaNum like '%$nombreVal%'
	...
	if ($esta == 'PA') {
		$where .= stripslashes(" and t.estado in ('P') and vista = 0 and (t.fecha + 10*86400) > unix_timestamp()");
	} else if ($esta == 'PR') {
		$where .= stripslashes(" and t.estado in ('P') and vista = 1 and (t.fecha + 10*86400) > unix_timestamp()");
	} else if ($esta == 'PV') {
		$where .= stripslashes(" and t.estado in ('P') and (t.fecha + 10*86400) <= unix_timestamp()");
	} else {
...
if ($_SESSION['rol'] == 1 || $_SESSION['rol'] == 24 || $_SESSION['rol'] == 16 || $_SESSION['rol'] == 10) {
	$colEsp = array(
				array("s", 'Revisar Transferencia', "css_transf", _TAREA_ANULAR)
