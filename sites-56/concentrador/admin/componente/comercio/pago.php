...
if ($d['inserta']) {
	...
	$nombre 	= $arrEnv['nombre'];
	$email 		= $arrEnv['email'];
	$pasaval 	= $arrEnv['pasarela'];
	$validi 	= $arrEnv['idioma'];
	$servicio 	= $arrEnv['servicio'];
	$valamex 	= $arrEnv['amex'];
	$valcom 	= $arrEnv['id'];
	$valors 	= $arrEnv['pMomento'];
	$precio 	= $arrEnv['valor_inicial'];
	$valmon 	= $arrEnv['moneda'];
	$fecamb 	= date('mdHi');
	$codigo 	= $arrEnv['codigo'];
	$envio 		= true;
	$place = "";
	$repot = envPago($arrEnv);

	if (strpos($repot, '|') ) {
		$arrRepot = explode('|', $repot);
		error_log("repot->".$arrRepot[1]);
		switch ($arrRepot[1]) {
			case '33':
				$place = 'trans';
				break;
		...
		$repot = str_ireplace('|'.$arrRepot[1].'|', '', $repot);
	}
...
if ($envio) { ?>
	...
	setTimeout(function(){$('#moneda option[value='+$('#valmon').val()+']').attr('selected','selected');$('#importe').val('<?php echo $precio ?>');},6500);
	</script>
<?php }
...
// c es una letra del alfabeto
function isOnlyLetter (c) {
...
var uppercaseLetters = "ABCDEFGHIJKLMNOPQRSTUVWXYZï¿½ï¿½ï¿½ï¿½\/ÁÉÍÓÚÜÑ";
// s tiene solo letras, numeros y guion
function isAlphanumericWithHyphen (s) {
    var i;

    if (isEmpty(s))
        if (isAlphanumericWithHyphen.arguments.length == 1) return defaultEmptyOK;
        else return (isAlphanumericWithHyphen.arguments[1] == true);

    for (i = 0; i < s.length; i++)
    {
        var c = s.charAt(i);
        if (! (isOnlyLetter(c) || isDigit(c) || c == '-') )
            return false;
    }
...
function checTrans() {
    var msg = "Usted ha insertado caracteres prohibidos, use sólo números, guión(-) y letras del alfabeto, sin espacios en blanco";
	...
	if ($("#trans").val().length > 0) {
        ...
        if(isEmpty($("#trans").val())) return true;
        if (!isAlphanumericWithHyphen($("#trans").val())){
            warnInvalid(document.forms[0].trans,msg);
            $("#trans").focus();
            return false;
        } //else alert('OK');

        // if(!checkField (document.forms[0].trans, isUrl, '')) { Reina
        //     $("#trans").focus();
        //     return false;
        // }
	}
...
//		}
//	});
	setTimeout(function(){$("#<?php echo $place; ?>").css('border','solid 1px red')}, 5000);
});
