...
if ($d['modifica'] > -1) { // modifica la tupla
    ...
    $temp->query("insert into tbl_rotPasarOperac (idcomercio, idpasarela, idmoneda, cantOperac, orden, id, activo, fecha) values ('".$d['comercio']."', '".$d['pasarela']."', '".$d['moneda']."', '".$d['cantidad']."', '".($pos+1)."', null, 1, unix_timestamp())");
...
if (isset($d['borrar']) && $d['borrar'] > 0) {
	...
	$temp->query("delete from tbl_rotPasarOperac where id = ".$d['borrar']);
}
    ...
    while ($total != 0) {
        ...
        if ($total > 0) {
            ...
            for ($i = 0; $i < count($arrVals); $i++) {
                ...
                $temp->query("insert into tbl_rotPasarOperac (idcomercio, idpasarela, idmoneda, cantOperac, orden, id, activo, fecha) values ('".$arrVals[$i]['idcomercio']."', '".$arrVals[$i]['idpasarela']."', '".$arrVals[$i]['idmoneda']."', '".$arrVals[$i]['cantOperac']."', '".$ord++."', null, 1, unix_timestamp() )");
            }
        }
    }
    return $arrEnt[2];
