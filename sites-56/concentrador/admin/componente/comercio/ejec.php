...
if ($d['fun'] == 'damfic') {
	if ($d['com'] > 10){
		$dir = "../../../documentos/".$d['com']."/";

		$arrFic = scandir($dir);
		$sale = $error = '';
		
		$error = json_encode($arrFic);
		foreach ($arrFic as $fic) {
			if (is_file($dir.$fic)){
				$sale .= "<div id='div_' style='width:600px' class='lineaT'><div id='' style='width:590px' class='centro1'><span class='formul'><a href='".$dir.$fic."' target='_blank'>".substr($fic,0,stripos($fic,'.'))."</a></span></div></div>";
			}
		}
	}
	echo json_encode(array("sale" => $sale, "error" => $error));

} elseif ($d['fun'] == 'damficlegal') {
	if ($d['com'] > 10){
		$dir = "../../../legales/".$d['com']."/";
...
} elseif ($d['fun'] == 'borraFac') {
	$dir = "../../facturas/".date('Y')."/".$d['elem'];
	if (file_exists($dir)) {
		// $dir = "../mydir/";
		chdir($dir);
		$error = unlink($dir);
	}
	echo json_encode(array("cont"=>'true', "error"=>''));

} elseif ($d['fun'] == 'cheqNsec') {
...
} elseif ($d['fun'] == 'cambSol') {
	$error='';
	$temp->query("select estado, solDev from tbl_transacciones where idtransaccion = '".$d['cie']."'");
	if ($temp->f('estado') == 'A' && $temp->f('solDev') == '1') {
		$temp->query("delete from tbl_devoluciones where idtransaccion = '".$d['cie']."'");
		$temp->query("update tbl_transacciones set solDev = 0 where idtransaccion = '".$d['cie']."'");
		$sale = $d['cie'];
	} else $error = "La operación no está en solicitud de devolución";
	echo json_encode(array("cont"=>$sale, "error"=>$error));
...
// 	print_r($arrSal);
	...
	while (stripos($d['pas'], ',,') > 0) {
		$d['pas'] = str_replace(",,", ",", $d['pas']);
	}
	...
	if (strlen($d['com']) > 0) {
		($d['opr'] == 'P') ? $opr = "'P','A','R'" : $opr = "'A'";
		
		//verifica si admite pagos seguros
		$q = "select permnsec, operEur from tbl_comercio where id = ".$d['com'];
		$temp->query($q);
		$pan = $temp->f('permnsec');//Pagos seguros
		$euro = $temp->f('operEur');//forma de operar: 0-moneda habilitada, 1-opera solo con euros, 2-tiene las dos variantes

		//carga las pasarelas habilitadas para el comercio
		if (strlen($d['pas']) == 0 || ($_SESSION['rol'] == 1 || $_SESSION['rol'] == 24 || $_SESSION['rol'] == 16 || $_SESSION['rol'] == 10)){
			$q = "select idPasarela id, nombre from tbl_pasarela where tipo in ($opr) and activo = 1";
			if ($d['pag'] == 'N' && $pan == 0) $q .= " and secure = 1 ";
			$q .= " order by nombre";
		} else {
			$q = "select group_concat(p.idPasarela) id, case secure when 1 then 'Segura' else 'NO Segura' end nombre from tbl_pasarela p where tipo in ($opr) and p.idPasarela in ({$pass}) ";
			if ($d['pag'] == 'N' && $pan == 0) $q .= " and secure = 1 ";
	...
	echo utf8_encode(json_encode(array("cont"=>$arrSal, "error"=>$q, "euro" => $euro, "cambio"=>$arrCamb)));
	
} elseif ($d['fun'] == 'cheqLim') {
	//obtiene el monto máximo por operación de la o las pasarelas escogidas
	$d['pas'] = str_replace(',,',',',$d['pas']);
	$i=0;
	while (stripos($d['pas'],',,')){
		$d['pas'] = str_replace(',,',',',$d['pas']);
	...
	if (!_CAMB_LIM) {//verifico si el cambio de limites por moneda está habilitado
		$q = "select max(LimMaxOper) 'lim' from tbl_pasarela where idPasarela in (".$d['pas'].")";
	} else {
		$q = "select max(valor) 'lim' from tbl_colPasarLimite where idLimite = 2 and idPasar in (".$d['pas'].")";
		if (isset($d['mon'])) $q .= " and idmoneda = '".$d['mon']."'";
	}
	// echo $q;
	$temp->query($q);
	$lim = $temp->f('lim');
	...
	echo utf8_encode(json_encode(array("lim"=>$lim, "q"=>$q)));

} elseif($d['fun'] == 'pagoEur') { //extrae la tasa de cambio para los comercios que tienen permitido el cambio de USD a EUR
	$error = '';$arrCamb = $arrMo = array();
	$q = "select operEur, cambOperEuro from tbl_comercio where id = ".$d['com'];
	$temp->query($q);
	$euroTipo = $temp->f('operEur');//forma de operar: 0-moneda habilitada, 1-opera solo con euros, 2-tiene las dos variantes
	$euroCamb = $temp->f('cambOperEuro');//0- no pone las tasas para las oper sólo en Euros, 1- Si pone las tasas 
//error_log("euroTipo=$euroTipo");
//error_log("euroCamb=$euroCamb");
		
	if ($euroTipo != 0){//Puede operar sólo en Euros determino las tasas a coger
		if (is_array($d['mds'])) {
			$strMon = implode(',', $d['mds']);
		} else {
			$strMon = $d['mds'];
		}
//		$strMon = str_replace(",978", "", str_replace("978,", "", $strMon));
//error_log("strMon=$strMon");
		
		if (strlen($strMon) > 1) {
			if ($euroCamb == 1){//busca las tasas de cambio que debe haber puesto el comercio
				//monedas que tiene puesto por el TPV
				$q = "select m.moneda, t.tasa from tbl_tasaComercio t, tbl_moneda m where t.monedaBas = m.idmoneda and t.idcomercio = ".$d['com']." and monedaBas in ($strMon) and monedaCamb = 978 and t.fecha = (select max(fecha) from tbl_tasaComercio where t.idcomercio = ".$d['com']." )";
				$temp->query($q);
				$arrMon = $temp->loadResultArray(0);
			} else {// trabaja con las tasas de cambio que ha puesto el BCE para las distintas monedas
				$q = "select m.moneda, t.tasa from tbl_colCambBanco t, tbl_moneda m where t.idmoneda = m.idmoneda and t.idbanco = 26 and t.idmoneda in ($strMon) and t.fecha = (select max(b.fecha) from tbl_colCambBanco b where b.idbanco = 26 )";
				$temp->query($q);
				$arrMon = $temp->loadResultArray(0);
			}
		}
//error_log($q);
//error_log("arrMon=".implode("','",$arrMon));
//error_log("pos=".strpos($strMon, '978'));
		if (strpos($strMon, '978')>-1) {
			array_push($arrMon, 'EUR');
		}
//error_log("arrMon=".implode("','",$arrMon));
	
		if (count($arrMon) >= 1) {
			$q = "select idmoneda, moneda from tbl_moneda where moneda in ('".implode("','",$arrMon)."');";
			$temp->query($q);
			$arrMo = $temp->loadAssocList();
		} else {
			$error = 'salerror';
		}
//error_log("tasa=".$tasa);
//error_log("error=".$error);
//error_log("arrCamb=".$arrCamb);
//error_log("arrMo=".$arrMo);
	}
//	if ($tasa > 0) {$q = "";}
//	else {$q = "Error: Su comercio no tiene definido Tasa de cambio del USD al EUR";}
	
//	echo json_encode(array("error"=>'$errorlalalalala'));
	echo utf8_encode(json_encode(array("tasa"=>$tasa, "error"=>$error, "cambio"=>$arrCamb, "sale"=>$arrMo)));
...
// 	echo $q;
	...
	$elemBR = "(case t.estado  
	when 'B' then if ((t.fecha > $fecha1 and t.fecha < $fecha2), (t.valor_Inicial/100/t.tasa), 0)  
	when 'V' then if ((t.fecha > $fecha1 and t.fecha < $fecha2), (t.valor_Inicial/100/t.tasa), 0)
	when 'R' then if ((t.fecha > $fecha1 and t.fecha < $fecha2), (t.valor_Inicial/100/t.tasa), 0)
	when 'A' then if ((t.fecha_mod > $fecha1 and t.fecha_mod < $fecha2), (t.valor/100/t.tasa), 0) else 0.0 end) ";
...
// 				if($j==7 && $i == 4) $salida .= $q.$arrBnc[$j]['banco']."<br>";
				...
				if ($temp->f('cant') > 0) $vale = $cant/$temp->f('cant')*100;
				else $vale = 0;
				$salida .= "<li style='width:$ancho3;'>".formatea_numero ($vale,true)."</li>";
				$salidcsv .= ";".$vale;
...
// 			$TTot += $arrTrT[$i]['ttrans'];
		...
		$salidcsv .= "Total: ".(count($Tval));
		for($i=0; $i<count($Tval);$i++) {
		$salida .= "<li style='width:$ancho3;'>".formatea_numero($Tval[$i])."</li>
					<li style='width:$ancho3;'>".$TAcep[$i]."</li>
					<li style='width:$ancho3;'>".formatea_numero(($TAcep[$i]/$TTot[$i]*100),true)."</li>";
		$salidcsv .= ";".($Tval[$i]).";".($TAcep[$i]).";".($TAcep[$i]/$TTot[$i]*100);
		}
// 			.formatea_numero($Tval)."".formatea_numero($TAcep,false).""
// 			."".formatea_numero($Tval/$TAcep)."</li><li style='width:70px;'>"
// 			.formatea_numero($TTot,false).<li style='width:70px;'>"
// 			.formatea_numero($TAcep/$TTot*100)."</li>";
	...
	$salida .= "<span class='titule'>Datos en el intervalo de tiempo</span>";
		...
		$salida .= "
				<form name=\"exporta\" action=\"impresion.php\" method=\"POST\">
					<input type=\"hidden\" name=\"querys11\" value=\"1\">
		...
		for ($i=0;$i<count($arrIn);$i++){
			...
			$TAcep += $arrTrA[$i]['traceptada'];
			$TTot += $arrTrT[$i]['ttrans'];
		$salidcsv .= "{$arrIn[$i]['dia']};".($arrIn[$i]['valor']).";".($arrTrA[$i]['traceptada']).";".($arrIn[$i]['valor']/$arrTrA[$i]['traceptada']).";".($arrTrT[$i]['ttrans']).";".($arrTrA[$i]['traceptada']/$arrTrT[$i]['ttrans']*100)."{n}";
		}
		...
		$salida .= "</div>";
		$salidcsv .= "Total:;".($Tval).";".($TAcep).";".($Tval/$TAcep).";".($TTot).";".($TAcep/$TTot*100)."{n}";
	...
	$salida .= "<span class='titule'>Valor de Ingresos ocurridos en el intervalo de tiempo seleccionado</span>";
		$q = "select if (t.estado = 'A', from_unixtime(t.fecha_mod, '$inSql' ), from_unixtime(t.fecha, '$inSql' )) dia, ".
			" sum($elemBR) valor ".
			" FROM tbl_transacciones t ".
			" where t.estado in ('A','V','B','R') ".
				" and t.tipoEntorno = 'P' ".
				" and if (t.estado = 'A', t.fecha_mod between $fecha1 and $fecha2, t.fecha between $fecha1 and $fecha2 ) 
			GROUP BY from_unixtime(if (t.estado = 'A', t.fecha_mod, t.fecha), '$inSql' ) 
			ORDER BY (if (t.estado = 'A', t.fecha_mod, t.fecha)) desc";
		// sendTelegram($q,null);
		$temp->query($q);
//		error_log($q);
		if($temp->getErrorMsg()) $salida .= $temp->getErrorMsg();
		...
		for ($i=0;$i<count($arrIn);$i++){
			...
			$TAcep += $arrTrA[$i]['traceptada'];
			$TTot += $arrTrT[$i]['ttrans'];
			$salidcsv .= "{$arrIn[$i]['dia']};".($arrIn[$i]['valor']).";".($arrTrA[$i]['traceptada']).";".($arrIn[$i]['valor']/$arrTrA[$i]['traceptada']).";".($arrTrT[$i]['ttrans']).";"
				.($arrTrA[$i]['traceptada']/$arrTrT[$i]['ttrans']*100)."{n}";
		}
		$salida .= "<ul class='respta tot' style='width:$ancho'><li style='width:105px;'>Total:</li><li style='width:70px;'>"
			.formatea_numero($Tval)."</li><li style='width: 70px;'>".formatea_numero($TAcep,false)."</li>"
			."<li style='width:70px;'>".formatea_numero($Tval/$TAcep)."</li><li style='width:70px;'>"
...
//		var_dump($arr1);
		for ($j= 0; $j<count($arr1);$j++) {
			...
			foreach ($arrban as $banid) {
				...
				$salcsv .= ";".($arr3[0]['valor']).";".$cantop.";".$res;
				$k++;
			}
			$salida .= "$sale</div>";
			$salidcsv .= $salcsv."{n}";
		...
		for($i = 0; $i < count($arrCant); $i++){
			...
			$salidcsv .= ($arrVol[$i]).";".$arrCant[$i].";".$res.";";
		}
		$salidcsv .= "{n}";
		$salida .= "</div>";
		$salida .= "</div>";
	...
	if ($d['estado'] == 1) {
		...
		for ($i=0;$i<count($arrEsT);$i++){
			...
			$salidcsv .= $arrEsT[$i]['estad'].";".($arrEsT[$i]['cantidad']).";". ($arrEsT[$i]['cantidad']/$nuT*100)."{n}";
		}
		$salida .= "</div>";
		$salida = str_replace("{dat1}", $salidcsv, $salida);
	}
...
// 		$salida .= $q;
		...
		for ($i=0;$i<count($arr1);$i++){
			...
			$salidcsv .= "{$arr1[$i]['moneda']};".($arr1[$i]['valor']).";".($temp->f('canta')).";".($arr1[$i]['cantt']).";". ($temp->f('canta')/$arr1[$i]['cantt']*100)."{n}";
		}
		$salida .= "</div>";
...
// 		$salida .= $q;
		...
		$q = "select sum($elem) valor, count(t.idtransaccion) cantt
				from tbl_transacciones t, tbl_pasarela p
				where t.pasarela = p.idPasarela
					and t.tipoEntorno = 'P'
					and t.estado in ('A','V','B','R')
		...
		$temp->query($q);
		if($temp->getErrorMsg()) $salida .= $temp->getErrorMsg();
		$tot1 = formatea_numero($temp->f('valor'));
		$tot3 = formatea_numero($temp->f('cantt'),false);

		$q = "select count(t.idtransaccion) cans
				from tbl_transacciones t, tbl_pasarela p
				where t.pasarela = p.idPasarela
					and t.tipoEntorno = 'P'
					and t.fecha_mod between $fecha1 and $fecha2";
		...
		$temp->query($q);
		if($temp->getErrorMsg()) $salida .= $temp->getErrorMsg();
		$tot2 = formatea_numero($temp->f('cans'),false);
...
// 			echo $q;
			...
			$salidcsv .= utf8_encode($arr1[$i]['nombre']).";".($arr1[$i]['valor']).";".($arr1[$i]['cantt']).";".($temp->f('cant')).";".
						($arr1[$i]['valor']/$arr1[$i]['cantt']).";".($arr1[$i]['cantt']*100/$temp->f('cant')).";".
		...
		$salida .= "<ul class='respta tot' style='width:$ancho'>
						...
						."<li style='width:70px;'>".formatea_numero($tot1/$tot3)."</li>"
						."<li style='width:70px;'>".formatea_numero($tot3*100/$tot2)."</li></ul>";
		$salidcsv .= "Total;".$tot1.";".$tot3.";".$tot2.";". ($tot1/$tot3).";". ($tot3*100/$tot2)."{n}";
		$salida .= "</div>";
	...
	if ($d['dpvm'] == 1) {
		...
		for ($i=0;$i<count($arr1);$i++){
			...
			$salidcsv .= utf8_encode($arr1[$i]['pasarela']).";".$arr1[$i]['moneda'].";".str_replace("&#147;","",utf8_decode($arr1[$i]['id_error'])).";".($arr1[$i]['cant']).";"
	...
	if ($d['tpvm'] == 1) {
		...
		for ($i=0;$i<count($arr1);$i++){
			...
			$salida .= "<ul class='respta' style='width:$ancho;'>"
							...
							."<li style='width:70px;'>".formatea_numero($temp->f('canta')/$arr1[$i]['cantt']*100)."</li></ul>";
			$salidcsv .= utf8_encode($arr1[$i]['pasarela']).";".$arr1[$i]['moneda'].";".($arr1[$i]['valor']).";".($temp->f('canta')).";".($arr1[$i]['cantt']).";".($arr1[$i]['valor']/$temp->f('canta')).";".($temp->f('canta')/$arr1[$i]['cantt']*100)."{n}";
	...
	/*	Analisis por Banco	*/
		...
		for ($i=0;$i<count($arr1);$i++){
			...
			$salida .= "<ul class='respta' style='width:$ancho;'>"
							...
							."<li style='width:70px;'>".formatea_numero($arr1[$i]['valor'] / $j * $int)."</li></ul>";
			$salidcsv .= utf8_encode($arr1[$i]['nombre']).";".($arr1[$i]['valor']).";".($temp->f('cant')).";".($arr1[$i]['cantt']).";".($arr1[$i]['valor']/$temp->f('cant')).";".($temp->f('cant')/$arr1[$i]['cantt']*100).";".($arr1[$i]['valor'] / $j * $int)."{n}";
			$tot1 += $arr1[$i]['valor'];
			$tot2 += $temp->f('cant');
			$tot3 += $arr1[$i]['cantt'];
		}
	...
	if ($d['pbanc'] == 1) {
		...
		for ($i=0;$i<count($arr2);$i++){
			...
			$arrban[] = $arr2[$i]['id'];
			$salidcsv .= utf8_encode($arr2[$i]['banco']).";;";
		}
		$salida .= "</div>";
		$salidcsv .= "{n}";
		...
		$salidcsv .= utf8_encode("Países").";Operaciones;Valores";
		for($i = 0; $i<count($arr2);$i++) {
...
//		var_dump($arr1);
	...
	if ($d['pcom'] == 1) {
		...
		for ($i=0;$i<count($arr2);$i++){
			...
			$arrban[] = $arr2[$i]['id'];
			$salidcsv .= utf8_encode($arr2[$i]['nombre']).";;";
		}
		$salida .= "</div>";
		$salidcsv .= "{n}";
		$listbnc = implode(",", $arrban);
...
// $salida .=$q."<br>";
			...
			$salida .= "<ul class='respta' style='width:$ancho;'>"
							...
							."<li style='width:70px;'>".formatea_numero($arr1[$i]['valor']/$temp->f('canta'))."</li>"
							."<li style='width:70px;'>".formatea_numero($temp->f('canta')/$arr1[$i]['cantt']*100)."</li></ul>";
			$salidcsv .= $arr1[$i]['nombre'].";".$arr1[$i]['moneda'].";".($arr1[$i]['valor']).";".($temp->f('canta')).";".($arr1[$i]['cantt']).";".($arr1[$i]['valor']/$temp->f('canta')).";".($temp->f('canta')/$arr1[$i]['cantt']*100)."{n}";
			$tot1 += $arr1[$i]['valor'];
			$tot2 += $temp->f('canta');
			$tot3 += $arr1[$i]['cantt'];
		}
	...
	if ($d['tpvc'] == 1) {
		...
		$salida .= "<ul class='respta ttle' style='width:$ancho'>"
			...
			."<li style='width:70px;'>Valor/Trans</li>"
			."<li>% Aceptadas</li></ul>";
		$salidcsv .= "Pasarela;Comercio;Valor (Euro);Total;Aceptadas;Valor/Tran;% Aceptadas{n}";
		...
		for ($i=0;$i<count($arr1);$i++){
			...
			$salida .= "<ul class='respta' style='width:$ancho;'>"
							...
							."<li style='width:70px;'>".formatea_numero($arr1[$i]['valor']/$temp->f('canta'))."</li>"
							."<li style='width:70px;'>".formatea_numero($temp->f('canta')/$arr1[$i]['cantt']*100)."</li></ul>";
			$salidcsv .= $arr1[$i]['pasarela'].";".utf8_encode($arr1[$i]['comercio']).";{$arr1[$i]['valor']};{$arr1[$i]['cantt']};{$temp->f('canta')};".($arr1[$i]['valor']/$temp->f('canta')).";".($temp->f('canta')/$arr1[$i]['cantt']*100)."{n}";
			$tot1 += $arr1[$i]['valor'];
			$tot2 += $temp->f('canta');
			$tot3 += $arr1[$i]['cantt'];
		}
	...
	$arr1 = $arr2 = $arr3 = array();
		...
		$salida .= "<ul class='respta ttle' style='width:$ancho'>"
						...
						$salidcsv .= "Comercio;Valor Conc;Valor Web;Valor (Euro);Valor/Trans;Aceptadas Conc / %;Aceptadas Web / %;Aceptadas / %;Denegadas / %;En proceso / %;Total;Estimado{n}";
		for ($i=0;$i<count($arr1);$i++){
		    $q = "select count(idtransaccion) canta from tbl_transacciones where
					fecha_mod between $fecha1 and $fecha2
					and tipoEntorno = 'P'
			...
			$salidcsv .= utf8_encode($arr1[$i]['comercio']).";{$valc};{$valw};{$arr1[$i]['valor']};".($arr1[$i]['valor']/$tra).";".$trac.' / '.($trac/$arr1[$i]['cantt']*100).";".$traw.' / '.($traw/$arr1[$i]['cantt']*100).";".$tra.' / '.($tra/$arr1[$i]['cantt']*100).";".$trd.' / '.($trd/$arr1[$i]['cantt']*100).";".$trp.' / '.($trp/$arr1[$i]['cantt']*100).";".$arr1[$i]['cantt'].";".($arr1[$i]['valor']/$j*$int)."{n}";
			$tot1 += $arr1[$i]['valor'];
			$tot2 += $tra;
			$tot3 += $arr1[$i]['cantt'];
			$tot4 += $trd;
		...
		$salidcsv .= "Total:;".($tot8).";".($tot9).";".($tot1).";".($tot2).";".($tot6).' / '.($tot6/$tot103*100).";".($tot7).' / '.($tot7/$tot11*100).";".($tot2).' / '.($tot2/$tot3*100).";".$tot4.' / '.($tot4/$tot3*100).";".$tot3.";".$tot5.' / '.($tot5/$tot3*100)."{n}";
		$salida .= "</div>";
...
//		$qq = '';
		...
		$salida .= "<ul class='respta ttle' style='width:$ancho'>"
						...
						."<li style='width:70px;'>% En tot oper den</li>"
						."<li style='width:70px;'>Total Gen</li></ul>";
		$salidcsv .= "Comercio;Moneda;Error;Cantidad;Total Den;% En tot oper den;Total Gen.{n}";
		...
		for ($i=0;$i<count($arr1);$i++){
			...
			$salida .= "<ul class='respta' style='width:$ancho;'>"
							...
							."<li style='width:70px;'>".formatea_numero($arr1[$i]['cant']*100/$arr1[$i]['total'])."</li>"
							."<li style='width:70px;'>".formatea_numero($arr1[$i]['totG'],false)."</li></ul>";
			$salidcsv .= utf8_encode($arr1[$i]['comercio']).";".$arr1[$i]['moneda'].";".str_replace("&#147;","",utf8_decode($arr1[$i]['id_error'])).";".$arr1[$i]['cant'].";".($arr1[$i]['total']).";".($arr1[$i]['cant']*100/$arr1[$i]['total']).";".$arr1[$i]['totG']."{n}";
	...
	if ($d['common'] == 1) {
		...
		$salida .= "
				...
				<div style='margin-left: -30px;width: 577px;'><span style=\"cursor: pointer;\" class=\"css_x-office-document\" onclick=\"document.exporta.submit()\"
						onmouseover=\"this.style.cursor=&quot;pointer&quot;\" alt=\"Exportar a CSV\" title=\"Exportar a CSV\"></span>";
		$tot1 = $tot2 = $tot3 = 0;
		$salida .= "<div id='acoge'>";
		$salida .= "<ul class='respta ttle' style='width:$ancho'>"
						...
						."<li style='width:70px;'>Aceptadas</li>"
						."<li style='width:70px;'>Valor/Trans</li>"
						."<li style='width:70px;'>% Aceptadas</li></ul>";
		...
		for ($i=0;$i<count($arr1);$i++){
			...
			$salida .= "<ul class='respta' style='width:$ancho;'>"
							...
							."<li style='width:70px;'>".formatea_numero($arr1[$i]['valor']/$temp->f('canta'))."</li>"
							."<li style='width:70px;'>".formatea_numero($temp->f('canta')/$arr1[$i]['cantt']*100)."</li></ul>";
			$salidcsv .= utf8_encode($arr1[$i]['comercio']).";{$arr1[$i]['moneda']};{$arr1[$i]['valor']};{$temp->f('canta')};".($arr1[$i]['valor']/$temp->f('canta')).";".($temp->f('canta')/$arr1[$i]['cantt']*100)."{n}";
			$tot1 += $arr1[$i]['valor'];
			$tot2 += $temp->f('canta');
			$tot3 += $arr1[$i]['cantt'];
		}
	...
	/*	Analisis por Comercio Web - Moneda	*/
	if ($d['commonw'] == 1) {
		$salidcsv = '';
		$arr1 = $arr2 = $arr3 = array();
		$salida .= "<span class='titule'>An&aacute;lisis por Comercio Web - Moneda</span>";
		$q = "select c.nombre comercio,
				m.moneda,
				sum($elem) valor, count(idtransaccion) cantt, t.idcomercio idc, t.moneda idm
			from tbl_transacciones t, tbl_comercio c, tbl_moneda m
			where t.moneda = m.idmoneda
				and t.idcomercio = c.idcomercio
				and t.idtransaccion not in (select id_transaccion from tbl_reserva)
				and t.tipoEntorno = 'P' and t.tipoOperacion = 'P'
				and t.estado in ('A','V','B','R','D','P')
				and fecha_mod between $fecha1 and $fecha2
			group by t.idcomercio, t.moneda order by c.nombre, t.moneda";
		$temp->query($q);
		...
		$salida .= "
				...
				<div style='margin-left: -30px;width: 630px;'><span style=\"cursor: pointer;\" class=\"css_x-office-document\" onclick=\"document.exporta.submit()\"
						onmouseover=\"this.style.cursor=&quot;pointer&quot;\" alt=\"Exportar a CSV\" title=\"Exportar a CSV\"></span>";
		$tot1 = $tot2 = $tot3 = 0;
		$salida .= "<div id='acoge'>";
		$salida .= "<ul class='respta ttle' style='width:$ancho'>"
						...
						."<li style='width:70px;'>Valor &euro;</li>"
						."<li style='width:70px;'>Aceptadas</li>"
						."<li style='width:120px;'>Total Opr</li>"
						."<li style='width:70px;'>Valor/Trans</li>"
						."<li style='width:70px;'>% Aceptadas</li></ul>";
		$salidcsv .= "Comercio;Moneda;Valor (Euro);Aceptadas;Valor/Trans;% Aceptadas{n}";
		for ($i=0;$i<count($arr1);$i++){
			$q = "select count(idtransaccion) canta from tbl_transacciones where
					fecha_mod between $fecha1 and $fecha2
					and tipoEntorno = 'P' and tipoOperacion = 'P'
					and estado in ('A','V','B','R')
					and idtransaccion not in (select id_transaccion from tbl_reserva)
					and idcomercio = {$arr1[$i]['idc']}
					and moneda = {$arr1[$i]['idm']}";
			$temp->query($q);
			...
			$salida .= "<ul class='respta' style='width:$ancho;'>"
							...
							."<li style='width:70px;'>".formatea_numero($arr1[$i]['valor'])."</li>"
							."<li style='width:70px;'>".formatea_numero($temp->f('canta'),false)."</li>"
							."<li style='width:120px;'>".formatea_numero($arr1[$i]['cantt'],false)."</li>"
							."<li style='width:70px;'>".formatea_numero($arr1[$i]['valor']/$temp->f('canta'))."</li>"
							."<li style='width:70px;'>".formatea_numero($temp->f('canta')/$arr1[$i]['cantt']*100)."</li></ul>";
			$salidcsv .= utf8_encode($arr1[$i]['comercio']).";{$arr1[$i]['moneda']};{$arr1[$i]['valor']};{$temp->f('canta')};".($arr1[$i]['valor']/$temp->f('canta')).";".($temp->f('canta')/$arr1[$i]['cantt']*100)."{n}";
			$tot1 += $arr1[$i]['valor'];
		...
		$salida .= "<ul class='respta tot' style='width:$ancho'>
						...
						."<li style='width:70px;'>".formatea_numero($tot1)."</li>"
						."<li style='width:70px;'>".formatea_numero($tot2,false)."</li>"
						."<li style='width:120px;'>".formatea_numero($tot3,false)."</li>"
						."<li style='width:70px;'>".formatea_numero($tot1/$tot2)."</li>"
						."<li style='width:70px;'>".formatea_numero($tot2/$tot3*100)."</li></ul>";
		$salidcsv .= "Total:;;$tot1;$tot2;".($tot1/$tot2).";".($tot2/$tot3*100)."{n}";
		$salida .= "</div>";
...
// echo $q.";<br>";
		$arr3 = $temp->loadAssocList();
			...
			$salidcsv .= utf8_encode($arr1[$i]['comercio']).";{$arr1[$i]['cantt']};{$arr1[$i]['valort']};{$arr2[0]['cans']};{$arr2[0]['vals']};{$arr3[0]['canns']};{$arr3[0]['valns']}{n}";
			$tot1 += $arr1[$i]['cantt'];
...
} elseif ($d['fun'] == 'verAmexUsd') {
	$error = '';
	$q = "select usdxamex total from tbl_comercio where id = ".$d['comercio'];
	$temp->query($q);
	
	echo json_encode(array('error'=>utf8_encode($error),'pase'=>utf8_encode($temp->f('total'))));
...
//	if (!$ent->isAlfanumerico($d['pas'])) $error .= "pasarela incorrecta\n";
	...
	$q = "select distinct idPasarela, idcenauto from tbl_pasarela where tipo in ('P','A','R') and activo = 1 and idPasarela in (".$d['pas'].")";
	$temp->query($q);
...
// 		
// 		//carga las monedas
		$q = "select distinct m.idmoneda, m.moneda from tbl_moneda m, tbl_colPasarMon c where c.idmoneda = m.idmoneda and m.activo = 1 and c.estado = 1 and c.idpasarela in (".$d['pas'].")";
		$temp->query($q);
		$arrSal = $temp->loadAssocList();
		
		//carga las tarjetas de la(s) pasarela(s)
		$temp->query("select  t.id, convert(cast(convert(t.nombre using utf8) as binary) using latin1) 'nombre' from tbl_tarjetas t, tbl_colTarjPasar c where c.idTarj = t.id and c.idPasar in (".$d['pas'].") group by t.id order by t.tipo desc");
...
} elseif ($d['fun'] == 'cambLim') { //cambia los limites por monedas en la pág de core/pasarela

	$est = '';

	if (strpos($d['mon'],',')) {
		$arrSal = array(
			array("idLimite"=>"1", "valor"=>"0"),
			array("idLimite"=>"2", "valor"=>"S/L"),
			array("idLimite"=>"3", "valor"=>"S/L"),
			array("idLimite"=>"4", "valor"=>"S/L"),
			array("idLimite"=>"5", "valor"=>"S/L"),
			array("idLimite"=>"6", "valor"=>"S/L"),
			array("idLimite"=>"7", "valor"=>"S/L")
		);
	} else {
		$q = "select estado from tbl_colPasarMon where idmoneda = '". $d['mon'] ."' and idpasarela =  '".$d['pas']. "'";
		$temp->query($q);
		$est = $temp->f('estado');

		$q = "select idLimite, valor from tbl_colPasarLimite where idPasar = '".$d['pas']. "' and idmoneda = '". $d['mon'] ."'";
		$temp->query($q);
		$arrSal = $temp->loadAssocList();
		for ($i=1; $i<count($arrSal); $i++){
			if (($i >= 1 && $i <= 4) && $arrSal[$i]['valor'] == '100000000') $arrSal[$i]['valor'] = 'S/L';
			if ($i >4 && $arrSal[$i]['valor'] == '1000') $arrSal[$i]['valor'] = 'S/L';
		}
	}
		$q = '';

	// print_r($arrSal);
	echo json_encode(array('error'=>$q, 'sale'=>$arrSal, 'est'=>$est));
} elseif ($d['fun'] == 'vermonamx') {
	$error = '';

	$q = "select secure from tbl_pasarela where idPasarela in (".$d['pasa'].")";
	$temp->query($q);
	$sec = $temp->f('secure');

	if ($d['valor'] == 1) {// la tarjeta es amex
		if ($sec == 1 && stripos($d['pasa'], '93') > -1) { //si es segura y la pasarela es Navarra Amex 3D pone euros y dólares
			$arrSal = array(array("idmoneda"=>"840", "moneda"=>"USD"), array("idmoneda"=>"978", "moneda"=>"EUR"));
		} else { // si no sólo euros
			$arrSal = array(array("idmoneda"=>"978", "moneda"=>"EUR"));
		}
	} else {
		
		$q = "select distinct m.idmoneda, m.moneda from tbl_moneda m, tbl_colPasarMon c where c.idmoneda = m.idmoneda and m.activo = 1 and c.estado = 1 and c.idpasarela in (".$d['pasa'].")";
		$temp->query($q);
		$arrSal = $temp->loadAssocList();
	}
// var_dump($arrSal);
	echo json_encode(array('error'=>utf8_encode($error), 'sale'=>$arrSal));
} elseif ($d['fun'] == 'vertraza') {
	$error = '';

	$q = "select from_unixtime(fecha,'%d/%m/%Y %H:%i:%s') fecha, titulo, traza from tbl_traza where id = ".$d['id'];
	$temp->query($q);
	$arrSal = array('fecha' => $temp->f('fecha'),'titulo' => $temp->f('titulo'),'traza' => $temp->f('traza'));
