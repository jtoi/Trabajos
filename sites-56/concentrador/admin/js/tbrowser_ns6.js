...
strict=false;
if((!q145)&&(document.doctype))strict=true;
...
for(m=0;m<q18;m++) if(q105[m])q0(m+"");
for(j=0;j<q19.length;j++) q0(q19[j]);

write_mainbar();;

function write_mainbar(){
	sd="<div id='dqmbar' style='position:absolute;visibility:hidden;z-index:900;top:0;left:0;'>";
	mitemx=0;mitemy=0;
	...
	for(i=0;i<q18;i++){
		...
		q47=" cursor:pointer;font-style:"+mft+";font-weight:"+mfw+";font-size:"+mfs+"px;font-family:"+mff+";padding-left:"+dqm__main_margin_left+"px;padding-right:"+dqm__main_margin_right+"px;padding-top:"+mtm+"px;padding-bottom:"+mbm+"px;";dibw=dqm__main_border_width;
		sd+="<div align='"+q130+"' style='position:absolute;cursor:pointer;top:-11px;left:"+mitemx+"px;width:"+(mitemw+(dibw*2))+"px;height:"+(mitemh+(dibw*2))+"px;";
		
		if(dqm__main_border_color!="transparent") sd+="background-color:"+dqm__main_border_color+";";
		
		sd+="'>";
		tvadj=0;
		thadj=0;
		tvadj=mtm+mbm;
		thadj=dqm__main_margin_left+dqm__main_margin_right;
		...
		sd+="<div  id='menu"+i+"' style='position:absolute;top:"+dibw+"px;left:"+dibw+"px;width:"+(mitemw-thadj)+"px;height:"+(mitemh-tvadj)+"px;";
		...
		sd+="</div>";
		sd+="<div id='qmim"+i+"'   onclick=\"q32('"+i+"')\" align='"+q130+"' style='position:absolute;cursor:pointer;visibility:hidden;top:"+dibw+"px;left:"+dibw+"px;width:"+(mitemw-thadj)+"px;height:"+(mitemh-tvadj)+"px;";
...
function q0(mindex){
	...
	sd="<div id=qm"+mindex+" style='z-index:900"+level+";cursor:pointer;position:absolute;top:"+0+"px;left:"+0+"px;visibility:hidden;width:"+q50+"px;";
	
	if(bc!="transparent") sd+=" background-color:"+bc+";";
	...
	while(q16("window.dqm__subdesc"+mindex+"_"+i)){
		...
		if(q16("window.dqm__subdesc"+id+"_0")) q19=q19.concat(new Array(id));
		...
		q47="style='position:absolute;cursor:pointer;left:"+bw+"px;top:"+bw+"px;width:"+(q50-(bw*2)-ttn)+"px;";
		...
		if((tval || tval==0)&& q16("window.dqm__2nd_icon_image"+tval)){
			...
			if(q150 && q145) q140=dqm__margin_left;
			
			(q130=="left")? tval=(q50-(bw*2)-dqm__margin_right-q127[0]+q128[0]-q140):tval=bw+dqm__margin_left+q128[0];
		...
		sd+="<div align='"+q130+"' id=qmitemhl"+id+" "+q47+" visibility:hidden;background-color:"+mbgc_hl+";";sd+=q48+" text-decoration:"+dqm__hl_textdecoration+";color:"+hltc+";"+item_q98;
		sd+="' onclick=\"q32('"+id+"')\">";
		q125="";
...
function q1(id,main){
	sub=q141("qm"+id);
	if(!q145)subc=sub.getElementsByTagName("div");
	else subc=sub.children;
	
	bw=q15("dqm__border_width",0,id);
	dh=q15("dqm__divider_height",0,id);
	q140=0;
	
	if(q145 && q150) q140=dqm__margin_top+dqm__margin_bottom;
	...
	if(main){
		if(!q145)q140=document.getElementById("menu"+id);
		else q140=eval("menu"+id);
...
function q103(){
	if(!q145) q140=document.getElementById("dqmbar");
	else q140=dqmbar;
	
	q87=q17(q141("imgbar"));
	q140.style.left=q87.x+"px";
	q140.style.top=q87.y+"px";
	q140.style.visibility="visible";
};
...
function detectSource(o){
	if(!q145)q64=o.parentNode;
	else q64=o.parentElement;
	
	while(q64 !=null){
		if(q64.id!="")return q64.id;
		
		if(!q145)q64=q64.parentNode;
		else q64=q64.parentElement;
	}
	return "";
};
...
function dqm__handleMouseMove(e){
	...
	else  if(q81.indexOf("menu")>-1){
		q111();
		tid=q81.substring(4);
		q79(tid);
	}
	else {
		if(q84.indexOf("qmitem")>-1)tid=q84;
		else  if(q81.indexOf("qmitem")>-1)tid=q81;
		else {
			if((q82!=null)&&(q84.indexOf("qm")<0)&&(q81.indexOf("qm")<0)){
				q122(true);
				q94=q82;
...
function q79(id){
	if(q82!=id){
		if(q82!=null){
			if(eval("window.dqm__subdesc"+q82+"_0"))q6(q82);
			q142(q82);
		}
		...
		if(eval("window.dqm__showmenu_code"+q82))eval(eval("dqm__showmenu_code"+q82));
	}
};
...
function q6(id){
	if(eval("window.dqm__subdesc"+id+"_0")){
		...
		if(tm.lasthl!=null) q141("qmitemhl"+tm.lasthl).style.visibility="hidden";
...
function q15(pname,rv,id){
	...
	if(q16("window."+pname+tindex)) return q16(pname+tindex);
	else return rv;
};
...
function q141(id){
	if(!q145) return document.getElementById(id);
	else return eval(id);
};
...
function q17(o){
	...
	if(q150 && q145) q64=o.parentElement;
	else q64=o.offsetParent;
...
function dqm__handleOnload(){
	if(q61)return;
	dqm__handleResize();
	if(q150 && q145)dqm__handleResize();
	q61=true;
	
	if(!q150 && q145) q61=false;
