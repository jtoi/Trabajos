...
if(q150){
	if(!q151){
		off_x=dqm__os9_ie5mac_offset_x;
		off_y=dqm__os9_ie5mac_offset_Y;
		offmain_x=dqm__mainitems_os9_ie5mac_offset_x;
		offmain_y=dqm__mainitems_os9_ie5mac_offset_y;
	}else {
		off_x=dqm__osx_ie5mac_offset_x;
		off_y=dqm__osx_ie5mac_offset_Y;
		offmain_x=dqm__mainitems_osx_ie5mac_offset_x;
		offmain_y=dqm__mainitems_osx_ie5mac_offset_y;
	}
}
...
function write_mainbar(){
	sd="<div id='dqmbar' style='position:relative;z-index:900;visibility:hidden;top:0;left:0;'>";
	mitemx=0;
	mitemy=0;
	...
	for(i=0;i<q18;i++){
		...
		q47=" font-style:"+mft+";font-weight:"+mfw+";font-size:"+mfs+"px;font-family:"+mff+";padding-left:"+dqm__main_margin_left+"px;padding-right:"+dqm__main_margin_right+"px;padding-top:"+mtm+"px;padding-bottom:"+mbm+"px;";sd+="<div align='"+q130+"' style='position:absolute;cursor:pointer;top:"+mitemy+"px;left:"+mitemx+"px;width:"+(mitemw+(dibw*2))+"px;height:"+(mitemh+(dibw*2))+"px;";
		...
		sd+="<div  id='menu"+i+"' style='position:absolute;top:"+dibw+"px;left:"+dibw+"px;width:"+(mitemw-smargin)+"px;height:"+(mitemh-bmargin)+"px;";
		...
		sd+="</div>";
		sd+="<div id='qmim"+i+"'   onclick=\"q32('"+i+"')\" align='"+q130+"' style='position:absolute;visibility:hidden;top:"+dibw+"px;left:"+dibw+"px;width:"+(mitemw-smargin)+"px;height:"+(mitemh-bmargin)+"px;";
...
function q0(mindex){
	...
	if(dqm__dropshadow_color.toLowerCase()!="none")sd+=" "+t_val+"progid:DXImageTransform.Microsoft.dropshadow(color="+dqm__dropshadow_color+",offx="+dqm__dropshadow_offx+",offy="+dqm__dropshadow_offy+");";
	...
	while(window["dqm__subdesc"+mindex+"_"+i]){
		...
		q47="style='position:absolute;cursor:pointer;left:"+bw+"px;top:"+bw+"px;width:"+(q50-(bw*2)-smargin)+"px;";
		...
		sd+=q131+q134+q132+"</div><div align='"+q130+"' id=qmitemhl"+id+" "+q47+" visibility:hidden;background-color:"+mbgc_hl+";";
		sd+=q48+" text-decoration:"+dqm__hl_textdecoration+";color:"+hltc+";"+item_q98;sd+="' onclick=\"q32('"+id+"')\">";
		q125="";
		...
		(q130=="right")? sd+=tval+q125:sd+=q125+tval;sd+=q131+q129+q132+"</div>";
...
function q1(id,main){
	sub=q16("qm"+id);
	q113=sub.style;subc=sub.children;
	...
	if(main){
		q85=q16("menu"+id);
		tc=q17(q85);
		q113.left=(tc.x+sxy[0]+q85.offsetWidth+off_x)+"px";
		q113.top=(tc.y+sxy[1]+off_y)+"px";
	}else {
...
function q103(){
	...
	dqmbar.style.left=(q87.x+offmain_x)+"px";
	dqmbar.style.top=(q87.y+offmain_y)+"px";
	q16("dqmbar."+q58);
};
...
function q5(menu,hl_id){
	...
	if(item_q98!="" &&(sub_q98=="" || menu.filters[0].status!=2)){
		tval=q16("q16(qmitemhl"+hl_id+")."+"filters[0]");
		tval.apply();
		tval.play();
	}
