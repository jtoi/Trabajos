...
function sendTelegram ($texto,$otro,$bot='G') {
    ...
    $env = (isset($GLOBALS['environment'])) ? $GLOBALS['environment'] : '';
    $newtexto = (strlen($env) > 0) ? ' (' . $env . ') <br>' . $texto : $texto;

	if ($bot == 'G') {
		$bot_token = "5038443834:AAHk906Tuj0zWRt1kITZtWk1XXX4vZeDnGc"; //envío a mí
		$id="-1001652358529";
	} elseif ($bot == 'RL') {
        $bot_token = "5782711854:AAF8TpwK28qcMj5QDQpI2AHP1GwWIP1BoFU"; // envia a Reina y Laynier
        $id="-742638810";
    } else {
...
// error_log("PASO3=$paso");
				$alert = "La moneda definida no existe|30|";
			}
		}
	} else {
		$paso = false;
// error_log("PASO4=$paso");
		$alert = "No se definió la moneda de la operación|30|";
	}
...
//							$message .= _INVITACION_DEPAGO . $adic;
				...
				return "<div style=\"text-align:center; margin-top: 20px; color:red; font-family:Arial sans-serif; font-size:11px\">" . _COMERCIO_CODE_YA . "|33|</div>";
		} else
			return "<div style=\"text-align:center; margin-top: 20px; color:red; font-family:Arial sans-serif; font-size:11px\">" . _COMERCIO_CODE_YA . "|33|</div>";
	} else
...
// 		var_dump($arrEnt);
			...
			if ($pasarela != 0) {
				if ($arrEnt['importe'] >= $valMin) {
					...
					if (($arrEnt['importe'] / $tasamno) <= $importMax)  {
						...
						$hora = time ();
						$query = "insert into tbl_transacciones	(idtransaccion, idcomercio, identificador, tipoOperacion, fecha, fecha_mod, valor_inicial, tipoEntorno, moneda, estado, sesion, pasarela, idioma, tipoPago) values ('$trans', '$comer', '" . $fijo . "', 'T', $hora, $hora, '$importe', 'P', '{$arrEnt['monedas']}', 'T', '$firma', '$pasarela', '{$arrEnt['idioma']}', 'T')";
						// echo $query."<br>";
						$temp->query ( $query );
						...
						$query = "insert into tbl_transferencias (idTransf, cliente, cuentaB, idcomercio, idCom, facturaNum, email, fecha, fechaTransf, valor, moneda,
									concepto, idioma, idPasarela, idadmin, estado) values
									('$trans', '{$arrEnt['nombre']}', '{$arrEnt['cuenta']}', '$comer', '$idCom', '" . $fijo . "', '{$correo}', '{$hora}',
									{$hora}, '$importe', '{$arrEnt['monedas']}', '{$arrEnt['servicio']}', '{$arrEnt['idioma']}', $pasarela, {$_SESSION['id']}, 'T')";
						$temp->query ( $query );
						// echo $temp->_sql;
						...
						/**Poner acá el envío del correo al Cliente que puso la solicitud de transferencia */
	...
	// 					$image = $comer . ".jpg";
						...
						$valor = $importe/100;
						$message = str_replace ( '{cliente}', $arrEnt['nombre'], str_replace('{valor}', $valor, str_replace('{moneda}', $moneda, _TBIOREV_TEXTCORREO)));

						$subject = _TBIOREV_SUBJECT;
						...
						if (_MOS_CONFIG_DEBUG) echo ($message."<br>to-> ".$corr->to."<br>header-> ".$corr->headers);
...
//				$cont = $i;  Reina
			...
			error_log($pasarel);
			if (($pasarel == '66' || $pasarel == '177' || $pasarel == '200' || $pasarel == '55') && $mon != '978') {error_log("ERRROOOOR en pasarela - moneda");exit;}
			return $pasarel;
		}
	}
...
function regCoxData() {
    ...
    $temp->query(sprintf("update tbl_ipBL set login = 0, fecha = unix_timestamp() where ip = '%s'", $ip));
}
...
<?php for ($cont = 0; $cont < count($colEsp); $cont++) {
		...
		elseif ($colEsp[$cont][0] == "x") {
			if ($_SESSION['rol'] != 14 && $_SESSION['rol'] != 15) //se prohibe que Vendedores y Contables de los comercios soliciten devolución
				$oncli = "return alerta('".str_replace (',', '', $usr->f("valor{val}"))."', '".$usr->f("estado")."',  '".$usr->f("id")."', 'SD', '".$usr->f("solDe")."', '".$usr->f("tipo")."',  '".$usr->f("solRe")."' );";
		...
		elseif ($colEsp[$cont][0] == "s")//revisar transferencia
			$oncli = "return alerta(1, '".$usr->f("tipo")."',  '".$usr->f("id")."', 'S' );";
		...
		elseif ($colEsp[$cont][0] == "a")
			$oncli = "return trazaView(".$usr->f("id").")";
			// $oncli = "window.open('index.php?componente=comercio&pag=traza&trazal=".$usr->f("id")."" ."','_blank')";
...
function alerta(valor=null, estado=null, id=null, accion, solDe=null, tipo=null, solRe=null) {
// debugger;
	 if (accion == 'T') {
...
//			return true;
	 ...
	 if (accion == 'S') {
		if (estado == "Transferencia") {
			/*document.pag.factura.value=id;
			envia(this.form);*/
			window.open('index.php?componente=comercio&pag=revisaFactura&tf='+id, "_self");
//			return true;
	 ...
	 if (accion == 'M') {
		...
		if (estado == 'A' && solDe == 1){
			...
			alert("La operación no está en solicitud de devolución ni No Procesada");
		}
		...
		return true;
	 	...
	 	if (valor > 0 && (estado == 'V' || estado == 'A' ) && solDe == 1) {
			//alert('<?php //echo $alerta5 ?>//'); Reina
            alert ('Esta operación ya fue Solicitada para Devolver.');
			...
			if (accion == 'A') {
				...
				if (tipo == 'Transferencia')
                    ...
                    alert('Esta operación está en Proceso de Reclamación y no admite devoluciones');
                } else
