...
class PowerGraphic {
    ...
    function PowerGraphic()
    {
        ...
        $this->descy     = (isset($_GET['descy']))     ? $_GET['descy']     : "";
        $this->descz     = (isset($_GET['descz']))     ? $_GET['descz']     : "";
        ...
        $this->legend_exists        = ($this->type == '5' || $this->type == '6') ? true : false;
    ...
    function create_graphic()
    {
        ...
        if (!empty($this->title))
        {
            ...
            imagestring($this->img, 5, $center, 2, $this->title, $this->color['title']);

//			if ($this->type == 2 )
			($this->color['bars_2_shadow']) ? $colo = $this->color['bars_2_shadow'] : $colo = $this->color['line_2'];
			($this->color['bars_shadow']) ? $col = $this->color['bars_shadow'] : $col = $this->color['line'];

			if ((strlen($this->descy) > 0) || (strlen($this->descz) > 0)) {
//				imagestring($this->img, 2, 90, 16, "Leyenda:", $this->color['title']);
				imagestring($this->img, 2, 140, 16, $this->descy, $col);
				imagestring($this->img, 2, 140, 30, $this->descz, $colo);
			}
        }
    ...
    function load_color_palette()
    {
        switch ($this->skin)
        {
            ...
            case 4:
                ...
                $this->color['axis_line']   = imagecolorallocate($this->img,  50, 100,  50);
                $this->color['bg_lines']    = imagecolorallocate($this->img, 200, 224, 180);
                $this->color['bg_legend']   = imagecolorallocate($this->img, 230, 230, 230);
...
'    <div id="add_parameter"></div>
  ...
  <input type="submit" value="Create" style="cursor: pointer; margin: 10px 0px 0px 60px;" />
    ...
    function create_query_string() {
        ...
        $graphic['descy']     = $this->descy;
        $graphic['descz']     = $this->descz;
