...
/* Modificaciones hechas para agrupar las pasarelas por tipos */
...
//			 echo $query;
		...
		/* Integración por tipo de pasarela */
		
		
		$q = "select nombre, dato1, dato4, p.datos, comercio from tbl_terminal t tbl_pasarela p where p.idPasarela = t.idpasarela and t.idpasarela = $pasarela and t.idmoneda = '$moneda'";
		$temp->query($q);
		if ($temp->num_rows() > 0) {
			$arrDat = explode(',',$temp->f('datos'));
			$variante = $arrDat[0];
			$urlPasarela = $arrDat[1];
			$idCom = $temp->f('dato4');
			$clave = $temp->f('dato1');
			$terminal = $temp->f('nombre');
			$comName = $temp->f('comercio');
		}
		
		
		/* fin de la integración por tipo de pasarela */
        
		
		if ($pasarela == 1) { //Pasarela de prueba del BBVA
...
//                $message = $importe . $trans . $idCom . $moneda . $clave;
			...
			else if ($moneda == '837') $terminal = '16'; //bolívar fuerte
...
//                $message = $importe . $trans . $idCom . $moneda . $clave;
			...
			$correoMi .= "cadenSal= " . $cadenSal . "<br>\n";
			echo $cadenSal;
		} elseif ($pasarela == 19 || $pasarela == 22 || $pasarela == 23 || $pasarela == 25 || $pasarela == 26 || $pasarela == 27 || $pasarela == 28 || $pasarela == 29 || $pasarela == 30 || $pasarela == 32) {
...
//				$urlPasarela = 'https://sis.redsys.es/sis/realizarPago';
				...
				$urlPasarela = 'https://sis-t.redsys.es:25443/sis/realizarPago';
                if			($moneda == '840') {$terminal = '006'; $clave = 'qwertyasdf0123456789';} //USD
                else if		($moneda == '826') {$terminal = '007'; $clave = 'qwertyasdf0123456789';} //GBP
                else if		($moneda == '392') {$terminal = '008'; $clave = 'qwertyasdf0123456789';} //JPY
                else if		($moneda == '32')  {$terminal = '009'; $clave = 'qwertyasdf0123456789';} //ARS
                else if		($moneda == '124') {$terminal = '010'; $clave = 'qwertyasdf0123456789';} //CAD
                else if		($moneda == '152') {$terminal = '011'; $clave = 'qwertyasdf0123456789';} //CLP
                else if		($moneda == '170') {$terminal = '012'; $clave = 'qwertyasdf0123456789';} //COP
                else if		($moneda == '356') {$terminal = '013'; $clave = 'qwertyasdf0123456789';} //INR
                else if		($moneda == '484') {$terminal = '014'; $clave = 'qwertyasdf0123456789';} //MXN
                else if		($moneda == '604') {$terminal = '015'; $clave = 'qwertyasdf0123456789';} //PEN
                else if		($moneda == '756') {$terminal = '016'; $clave = 'qwertyasdf0123456789';} //CHF
                else if		($moneda == '986') {$terminal = '017'; $clave = 'qwertyasdf0123456789';} //BRL
                else if		($moneda == '937') {$terminal = '018'; $clave = 'qwertyasdf0123456789';} //VEF
                else if		($moneda == '949') {$terminal = '019'; $clave = 'qwertyasdf0123456789';} //TRY
                else if		($moneda == '978') {$terminal = '020'; $clave = 'qwertyasdf0123456789';} //EUR
                else {
...
//				$clave_encriptacion = "T21RAFBM";
			...
			echo $cadenSal;
			$correoMi .= "cadenSal= " . $cadenSal . "<br>\n";
		} elseif ($pasarela == 24) {//PAYTPV Caixa 3Dp
            $url = 'https://www.paytpv.com/gateway/fsgateway.php';
            if ($moneda != '978'
				//&& $moneda != '826' && $moneda != '840' && $moneda != '392'
				) {
