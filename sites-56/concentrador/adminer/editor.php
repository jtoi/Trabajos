...
Min_Driver($h);if(!is_object($h)||($ld=$b->login($_GET["username"],get_password()))!==true)auth_error((is_string($h)?$h:(is_string($ld)?$ld:lang(69))));if($Da&&$_POST["token"])$_POST["token"]=$xf;$n='';if($_POST){if(!verify_token()){$Mc="max_input_vars";$sd=ini_get($Mc);if(extension_loaded("suhosin")){foreach(array("suhosin.request.max_vars","suhosin.post.max_vars")as$y){$X=ini_get($y);if($X&&(!$sd||$X<$sd)){$Mc=$y;$sd=$X;}}}$n=(!$_POST["token"]&&$sd?lang(70,"'$Mc'"):lang(62).' '.lang(71));}}elseif($_SERVER["REQUEST_METHOD"]=="POST"){$n=lang(72,"'post_max_size'");if(isset($_GET["sql"]))$n.=' '.lang(73);}if(!ini_bool("session.use_cookies")||@ini_set("session.use_cookies",false)!==false)session_write_close();function
