...
class formHTML {
	...
	//id y nombre
	function opciones_sel_Arr($select, $id = null, $todos = null) {
		$cadena_orden = '';
...
//		si debe mostrar el primer option del select como la sumatoria de todos los elementos
		if ($todos) {
			...
			if (count(array_intersect($salArr, $arrResult)) == count($arrResult	)) 
				$cadena_orden .= '<option selected=true value="' . implode("', '", $grup->loadResultArray()) . '">Todos</option>' . "\n";
		...
		foreach ($arrResult as $item) {
			if (is_array($id) && in_array($item['id'], $id))
				$cadena_orden .= '<option selected=true value="' . $item['id'] . '">' . $item['nombre'] . '</option>' . "\n";
			elseif ($item['id'] == $id)
				$cadena_orden .= '<option selected=true value="' . $item['id'] . '">' . $item['nombre'] . '</option>' . "\n";
			else
				$cadena_orden .= '<option value="' . $item['id'] . '">' . $item['nombre'] . '</option>' . "\n";
		}
		return $cadena_orden;
	}
...
//		si debe mostrar el primer option del select como la sumatoria de todos los elementos
		...
		while ($grup->next_record()) {
			if (is_array($id) && in_array($grup->f('id'), $id))
				$cadena_orden .= '<option selected=true value="' . $grup->f('id') . '">' . $grup->f('nombre') . '</option>' . "\n";
			elseif ($grup->f('id') == $id)
				$cadena_orden .= '<option selected=true value="' . $grup->f('id') . '">' . $grup->f('nombre') . '</option>' . "\n";
			else
	...
	function opciones_sel($select, $id = null, $tipo = 1, $nombre = null) {
		...
		while ($grup->next_record()) {
			...
			$trad = $fun->idioma($grup->f('nombre'));
			if (strlen($trad) == 0) {
				$trad = $grup->f('nombre');
			}
			if ($tipo == 1) {
				if ($grup->f('id') == $id)
					$cadena_orden .= '<option selected=true value="' . $grup->f('id') . '">' . $trad . '</option>' . "\n";
				else
					$cadena_orden .= '<option value="' . $grup->f('id') . '">' . $trad . '</option>' . "\n";
			} elseif ($tipo == 2) {
				if (in_array($grup->f('id'), $id))
					$cadena_orden .= '<div class="radElm" ><input type="checkbox" checked="checked" id="'.$nombre.'_'.$i.'" class="' . $this->classCss . '" name="' . $nombre . 
										'" value="'. $grup->f('id') .'" /><label for="'.$nombre.'_'.$i.'" >'.$trad .'</label></div>';
				else
...
// 		if (_MOS_PHP_DEBUG) trigger_error("Valores array= $valores_arr");
		$cadena_orden = '';
		for ($i=0; $i<count($valores_arr); $i++) {
//		foreach ($valores_arr as $value => $key) {
			
			if (is_array($valores_arr[0])) {
				
				$trad = $fun->idioma($valores_arr[$i][1]);
				if (strlen($trad) == 0) {
					$trad = $valores_arr[$i][1];
				}
				
				if (!is_array($val_sel)) {
					error_log($valores_arr[$i][0]." == ".$val_sel);
					if ($valores_arr[$i][0] == $val_sel) $cadena_orden .= '<option selected=true value="' . $valores_arr[$i][0] . '">' . $trad . '</option>' . "\n";

					else $cadena_orden .= '<option value="' . $valores_arr[$i][0] . '">' . $trad . '</option>' . "\n";
				} else {
					if (in_array($key, $val_sel)) $cadena_orden .= '<option selected=true value="' . $valores_arr[$i][0] . '">' . $trad . '</option>' . "\n";

					else $cadena_orden .= '<option value="' . $valores_arr[$i][0] . '">' . $trad . '</option>' . "\n";
				}
			} else {
				
				$trad = $fun->idioma($valores_arr[$i]);
				if (strlen($trad) == 0) {
					$trad = $valores_arr[$i];
				}
				if (!is_array($val_sel)) {
					if ($valores_arr[$i] == $val_sel) $cadena_orden .= '<option selected=true value="' . $valores_arr[$i] . '">' . $valores_arr[$i] . '</option>' . "\n";

					else $cadena_orden .= '<option value="' . $valores_arr[$i] . '">' . $valores_arr[$i] . '</option>' . "\n";
				} else {
					if (in_array($key, $val_sel)) $cadena_orden .= '<option selected=true value="' . $valores_arr[$i][0] . '">' . $valores_arr[$i] . '</option>' . "\n";
	...
	/**
	 * genera pares de options en base a dos números (lo mismo puede ser aumentando que disminuyendo)
	...
	function opciones($inicio, $final, $id) {
		...
		if ($final > $inicio) {
			for ($x = $inicio; $x <= $final; $x++) {
				if ($x == $id) $cadena_orden .= '<option selected=true value="' . $x . '">' . $x . '</option>' . "\n";
	 ...
	 */
	function inicio($sigue=false, $id='', $css='') {
//		if ($sigue) 
		$cont = '<div class="container"><div class="card card-container '.$css.'" id="'.$id.'"><h2 id="ttitle"></h2>'
				. '<span id="respue_spam" class="label label-success col-sm-12 hide"></span>'
				. '<form class="form-signin" enctype="multipart/form-data" method="POST" onsubmit="return(verifica());">';
//		else
...
//		$this->contenido .= '<div class="botForm" style="width:' . $this->anchoTabla . 'px">';
		if (!$botones) $this->contenido .= '<input class="btn btn-lg btn-primary btn-block btn-signin" id="enviaForm" name="enviar" type="submit" value="'.$fun->idioma('Enviar').'" /><input name="reset" type="reset" id="reset" class="btn btn-lg btn-block btn-signin btn-default" value="'.$fun->idioma('Cancelar').'" />';
...
//		return $this->contenido .= '</div></div></div></form></div></div>';
		return $this->contenido .= '</form></div></div>';
	}
	 ...
	 * @param string $adicional permite adicionar una serie de par�metros al textbox directamente pj. readonly, javascript, etc
	 */
	function inHide($valor, $nombre, $id = null, $adicional = null) {
		if ($id == null) $id = $nombre;
		$this->tripa .= '<input type="hidden" value="' . $valor .'" name="' . $nombre .'"	id="'. $id .'" '.$adicional.' />';
	}
	 ...
	 * @return string
	 */
	function iniDiv($texto, $id, $class = null) {
		return '<div class="form-group group-'.str_replace(" ", "", $class).'" id="idvf-'.$id.'"><label class="'.$this->classAncho.'" for="'.$id.'">'.$texto.':</label>';
	}
	 ...
	 * @param string $clase clase con la que se aplicar css seg�n fichero de css
	 * @param string $estilo estilo que le ser� aplicado
	 */
	function inCajaini($id, $clase='', $estilo='') {
		$this->tripa .= '<div id="caj_'.$id.'" class="'.$this->classAncho.' '.$clase.'" style="'.$estilo.'">';
	}
	
	/**
	 * Cierra la caja Div construida con la funci�n anterior
	 */
	function inCajaout(){
		$this->tripa .= '</div>';
	}
	 ...
	 * @param string $valor
	 * @param string $nombre
	 * @param string $tipo puede ser  text, password, datetime, datetime-local, date, month, time, week, number, email, url, search, tel, and color
	 * @param string $id id del text box, si no se suministra usa el mismo nombre
	 * @param string $texFin texto que se adiciona al final del textbox como aclaratoria, una marca de obligatorio al llenar, etc
	 * @param string $adicional permite adicionar una serie de par�metros al textbox directamente pj. readonly, javascript, etc
	 * @param string $aclar label aclaratorio que saldr� en mouseover sobre texto
	 */
	function inTextb($texto, $valor, $nombre, $tipo = 'text', $id = null, $texFin = null, $adicional = null, $aclar = null, $class = null) {
		if ($id == null) $id = $nombre;
		if ($aclar != null) $texto = '<spam class="haslabel"><label for="'.$id.'" title="'.$aclar.'">'.$texto.'</label></span>';

//			$this->tripa .= '<div id="'.$id.'" class="input-group form-group"><label style="display:inline-block !important;" class="col-sm-12" for="'.$id.'" >'.$texto.'</label><label class="input-group-btn"><span class="btn btn-primary">Browse&hellip; <input id="'.$id.'" type="file" style="display: none;" multiple></span></label><input type="text" style="margin:5px 0 !important;" class="form-control" readonly></div>';
		if ($tipo == 'file')
			$this->tripa .= '<div id="idvf-'.$id.'" class="input-group form-group"><label class="input-group-btn"><span class="btn btn-primary">'.$texto.'<input id="'.$id.'" type="file" style="display: none;" multiple></span></label><input type="text" style="margin:5px 0 !important;" value="Tamaño máximo 3.5 MB" class="form-control" readonly></div>';
		else
			$this->tripa .= $this->iniDiv($texto, $id, $class).'<input maxlength="' . $this->maxLenght . '" class="form-control ' . $this->classAncho . $class . '" type="'.$tipo.'" value="' . $valor . '" name="' . $nombre . '" id="' . $id . '" '.$adicional.' />'.$texFin.$this->finDiv;
	}
	...
	/**
	 * Crea un select
	 * tipo 1: Corresponde a un select con query pero para m�ltiples elementos seleccionados
	 * tipo 2: Una query con un solo elemento seleccionado
	 * tipo 3: Pares de opciones generados en base a un array
	 * tipo 4: Opciones generadas en base a dos n�meros (la entrada es un array con ambos n�meros)
	 * tipo 5: Corresponde a un select con query pero para m�ltiples elementos seleccionados y el primer elemento de la lista es el conjunto de todos los otros
	 *
	 * @param string $texto
	...
	function inSelect($texto, $nombre, $tipo, $valInicio, $valSelec = null, $id = null, $aclar = null, $adicional = null) {
		...
		if (preg_match('/multiple/', $adicional)) $this->tripa .= $this->iniDiv($texto, $id).'<select class="form-control ' . $this->classAncho . '" name="' . $nombre . '[]" id="' . $id . '" '.$adicional.'>';
		else $this->tripa .= $this->iniDiv($texto, $id).'<select class="form-control ' . $this->classAncho . '" name="' . $nombre . '" id="' . $id . '" '.$adicional.'>';
	...
	function inCheckBox($texto, $nombre, $tipo, $valInicio, $valSelec = null, $id = null, $aclar = null, $adicional = null) {
		...
		switch ($tipo) {
			...
			case 2:
				...
				break;
			case 3:
				$this->tripa .= $this->check_arr($valInicio, $val_sel, $nombre);
				break;
			case 4:
...
//genera pares de options en base a una query los elementos seleccionados son multiples pasados en el array $id
	...
	function check_elem_Arr($select, $nombre, $id = null) {
		...
		while ($grup->next_record()) {
			...
			$cadena_orden .= ' <label for="' . $grup->f('id') . '">' . $grup->f('nombre') . '</label>' . "<br>";
		}
		return $cadena_orden;
	}
	 ...
	 * @param string $id id del textbox que mostrar� la fecha es obligatoria
	 * @param string $altImag texto al mostrar al pasar el mouse por encima de la im�gen que muestra el calendario
	 * @param string $aclar label aclaratorio que saldr� en mouseover sobre texto
	 * @param string $adicional permite adicionar una serie de par�metros al textbox directamente pj. readonly, javascript, etc si $es null por defecto se escribe readonly
	 ...
	 * @param string $nombre nombre del campo
	 * @param integer $rows cantidad de l�neas en el textarea
	 * @param string $id id del text box, si no se suministra usa el mismo nombre
	 * @param string $texFin texto que se adiciona al final del textbox como aclaratoria, una marca de obligatorio al llenar, etc
	 * @param string $adicional permite adicionar una serie de par�metros al textbox directamente pj. readonly, javascript, etc
	 ...
	 * @param string $aclar label aclaratorio que saldr� en mouseover sobre texto
	 */
	function inTexarea($texto, $valor, $nombre, $rows, $id = null, $texFin = null, $adicional = null, $cols = 54, $aclar = null) {
		if ($id == null) $id = $nombre;
		if ($aclar != null) $texto = '<spam class="haslabel"><label for="'.$id.'" title="'.$aclar.'">'.$texto.'</label></span>';
	 ...
	 *
	 * @param string $texto texto a mostrar al cliente
	 * @param array $valorIni array con los valores que tomar�n los radio buttons
	 * @param string $nombre nombre del campo
	 * @param array $etiq array con los nombres de las etiquetas de los radios
	 * @param string $valor valor por defecto
	 * @param string $aclar label aclaratorio que saldr� en mouseover sobre texto
	 * @param boolean $separador si es true los radios se separan por cambio de l�nea sino por espacios
	 */
...
//		else $separador = '<br />';
		...
		for ($i=0; $i< count($valorIni); $i++) {
			...
			$this->tripa .= '<label class="radio-inline"><input type="radio" id="'.$i.'_'.$nombre.'" '.$adic.' name="'.$nombre.'" value="'.$valorIni[$i].'" />'.$etiq[$i].'</label>';
		}
		$this->tripa .= $texFin.$this->finDiv;
	}
	...
	function inTexto($texto, $valor) {
		$this->tripa .= $this->iniDiv($texto, "textoSimple").
						'<span class="' . $this->classCss . '" >'.$valor.'</span>'.$texFin.$this->finDiv;
	}

	/**
	 * Permite en el formulario poner texto solamente en una l�nea completa
	 * @param string $texto
	 * @param string $idDiv id del layer donde se mostrara texto
	 */
	function inTextoL($texto, $idDiv = null) {
		$this->tripa .= '<div id="div_'.$id.'" style="width:' . $this->anchoTabla . 'px" class="lineaT"><div id="'.$idDiv.'" style="width:' . ($this->anchoTabla - 10) . 'px" class="centro1"><span class="' . $this->classCss . '" >'.$texto.'</span></div></div>';
	}
