...
$(document).ready(function(){
	...
	$("#btnBusc").focus();
	$("#idvf-idioma").show();
	$("#obseres").markItUp(mySettings);
	$("#obseren").markItUp(mySettings);
	$("#decles").markItUp(mySettings);
	$("#declen").markItUp(mySettings);
//	$("#obser").markItUp(mySettings);
	$("#idvf-activo").show();
...
function verifica(){
	...
	var datos = $("#nombre").val() + '|' + $("#email").val() + '|' + $("#seudo").val() + '|' + $('#direcc').val() + '|' + $("input[name='activo']:checked").val() + '|' + $('#id').val()  + '|' + coord + '|' + $('#idioma').val() + '|' + $('#accion').val();
	var file_data = $('#imagen').prop('files')[0];   
	var form_data = new FormData();                  
	form_data.append('file', file_data);
	form_data.append('mdr', 'obr');
