...
/* 
 * Pagina para los estados de los pagos de las obras
 */
global $temp, $html, $tabla, $corCreo, $fun;

/**
 * Carga los datos de los pagos para ponerlos en el formulario
 * @global object $ent
 * @global object $temp
 * @global object $fun
 * @param integer $id
...
function eepg($id) {
	...
	$temp->query("select nombre from tbl_estadoPago where id = $id");
	$arrSal[] = $id;
...
function iepg($param) {
	...
	if ($acc == 'inserta') {// se va a insertar el estado
		...
		if (!$temp->query("insert into tbl_estadoPago (nombre) values (".$fun->carcHange($arrIdi["es"]).")")) 
			return json_encode(array("data" => '', "error" => "Error: {$temp->getErrorMsg()}"));
		
		
	} elseif ($acc == 'modifica') {// se actualiza el estado
		//obtengo el nombre que tenía antiguamente
		$temp->query("select nombre from tbl_estadoPago where id = $ida");
		$nombAnt = $temp->f('nombre');
		
		if (!$temp->query("update tbl_estadoPago set nombre = '".$fun->carcHange($arrIdi["es"])."' where id = $ida")) 
			return json_encode(array("data" => '', "error" => "Error: {$temp->getErrorMsg()}"));
	...
	if (!$temp->query("insert into tbl_bitacora (idadmin, texto) values ({$_SESSION['id']}, 'Se inserta o modifica el estado del pago '".$fun->carcHange($arrIdi["es"])."', los datos son: ". html_entity_decode($_POST['datos'], ENT_QUOTES)."'')")) 
		return json_encode(array("data" => '', "error" => "Error: {$temp->getErrorMsg()}"));
...
$titag = $fun->idioma("Estado del Pago");
$anchTit = "320px";
...
$html->inHide('tbl_estadoPago', 'tablas');
$html->inHide('', 'buscar');
...
$js = $homepage = file_get_contents('pagina/js/epg.js');
$scriptInf = "
