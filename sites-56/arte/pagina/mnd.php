...
/**
 * Carga los datos de la moneda para ponerlos en el formulario
 * @global object $ent
 * @global object $temp
 * @global object $fun
 * @param integer $id
...
function eepg($id) {
	...
	$temp->query("select denominacion, moneda from tbl_moneda where id = $id");
	$arrSal[] = $id;
	$arrSal[] = $temp->f('moneda');
	$arrSal[] = $temp->f('denominacion');
	
	$arrIdiom = $fun->idiomas();
	for ($i=0; $i<count($arrIdiom);$i++){
		$arrSal[$arrIdiom[$i]] = $fun->idioma($arrSal[2],$arrIdiom[$i]);
	}
...
function iepg($param) {
	...
	for($i = 0; $i < count($arrDatos); $i++) {
		switch ($i) {
			...
			case 1:
				...
				break;
			case 2:
				if (!($cod = $ent->isEntero($arrDatos[$i], 3))) {
					return json_encode(array("error" => $fun->idioma("Error en el codigo")));
				}
				break;
			case 3:
				if (!($iso = $ent->isAlfabeto($arrDatos[$i], 3))) {
					return json_encode(array("error" => $fun->idioma("Error en el ISO")));
				}
				break;
			default:
	...
	if ($acc == 'inserta') {// se va a insertar el estado
		...
		if (!$temp->query("insert into tbl_moneda (denominacion, moneda, id) values ('".$fun->carcHange($arrIdi["es"])."', '". strtoupper($iso)."', '{$cod}')")) {
			return json_encode(array("data" => '', "error" => "Error: {$temp->getErrorMsg()}"));}
		
		
	} elseif ($acc == 'modifica') {// se actualiza el estado
		//obtengo el nombre que tenía antiguamente
		$temp->query("select denominacion from tbl_moneda where id = $ida");
		$nombAnt = $temp->f('denominacion');
		
		if (!$temp->query("update tbl_moneda set denominacion = '".$fun->carcHange($arrIdi["es"])."', id = $cod, moneda = '". strtoupper($iso)."' where id = $ida")) {
			return json_encode(array("data" => '', "error" => "Error: {$temp->getErrorMsg()}"));}
	...
	if (!$temp->query("insert into tbl_bitacora (idadmin, texto) values ({$_SESSION['id']}, 'Se inserta o modifica la moneda '".$fun->carcHange($arrIdi["es"])."', los datos son: ". html_entity_decode($_POST['datos'], ENT_QUOTES)."')")) {
		return json_encode(array("data" => '', "error" => "Error: {$temp->getErrorMsg()}"));}
...
$html->inHide("id '".$fun->idioma('Editar')."{edit}', id '".$fun->idioma('Codigo')."', moneda 'ISO', denominacion '".$fun->idioma('Moneda')."'", 'columnas');
$html->inHide('tbl_moneda', 'tablas');
$html->inHide('', 'buscar');
...
$html->inTextb($fun->idioma('Codigo'), '', 'codigo', 'text', '', '', 'required autofocus');
$html->inTextb('ISO', '', 'iso', 'text', '', '', 'required autofocus');

$temp->query("select idioma, iso2 from tbl_idioma order by idioma ");
$arrIdiom = $temp->loadAssocList();
for($i=0;$i<count($arrIdiom);$i++) {
	$html->inTextb($fun->idioma('Moneda')." - ".$arrIdiom[$i]['idioma'], $nombre, $arrIdiom[$i]['iso2'], 'text', '', '', 'required autofocus',null,' estsd');
}

$formulario = "<div id='form' class='hide'>" . $html->salida() . "</div>";


$js = $homepage = file_get_contents('pagina/js/mnd.js');
$scriptInf = "
