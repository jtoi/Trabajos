...
function eepg($id) {
	...
	if (!($id = $ent->isEntero($id, 3))) return json_encode (array("error"=>$fun->idioma("Error en identificador del medio")));
	
	$temp->query("select nombre from tbl_medio where id = $id");
	$arrSal[] = $id;
...
function iepg($param) {
	...
	if ($acc == 'inserta') {// se va a insertar el estado
		...
		if (!$temp->query("insert into tbl_medio (nombre) values ('".$fun->carcHange($arrIdi["es"])."')")) {
			return json_encode(array("data" => '', "error" => "Error: {$temp->getErrorMsg()}"));}
		
		
	} elseif ($acc == 'modifica') {// se actualiza el estado
		//obtengo el nombre que tenía antiguamente
		$temp->query("select nombre from tbl_medio where id = $ida");
		$nombAnt = $temp->f('nombre');
		
		if (!$temp->query("update tbl_medio set nombre = '".$fun->carcHange($arrIdi["es"])."' where id = $ida")) {
			return json_encode(array("data" => '', "error" => "Error: {$temp->getErrorMsg()}"));}
	...
	if (!$temp->query("insert into tbl_bitacora (idadmin, texto) values ({$_SESSION['id']}, 'Se inserta o modifica el medio de expresión ".$fun->carcHange($arrIdi["es"]).", los datos son: ". html_entity_decode($_POST['datos'], ENT_QUOTES)."')")) 
		return json_encode(array("data" => '', "error" => "Error: {$temp->getErrorMsg()}"));
...
$titag = $fun->idioma("Medio de expresion");
$anchTit = "320px";
...
$html->inHide("id '".$fun->idioma('Editar')."{edit}', nombre '".$fun->idioma('Medio')."'", 'columnas');
$html->inHide('tbl_medio', 'tablas');
$html->inHide('', 'buscar');
$html->inHide("'{$fun->idioma('Medio')}' asc", 'orden');
...
for($i=0;$i<count($arrIdiom);$i++) {
	$html->inTextb($fun->idioma('Medio')." - ".$arrIdiom[$i]['idioma'], $nombre, $arrIdiom[$i]['iso2'], 'text', '', '', 'required autofocus',null,' estsd');
}

$formulario = "<div id='form' class='hide'>" . $html->salida() . "</div>";


$js = $homepage = file_get_contents('pagina/js/mdo.js');
$scriptInf = "
