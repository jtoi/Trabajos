<?php
/**
* KeyCAPTCHA Plugin for Joomla 1.6.x
* @version $Id: keycaptcha.php 2011-02-10 $
* @package: KeyCAPTCHA
* ===================================================
* @author
* Name: ITNP, Jsc, www.keycaptcha.com
...
if ( !class_exists('KeyCAPTCHA_CLASS') )
{
	class KeyCAPTCHA_CLASS
	{
		...
		function __construct($a_private_key='', $a_js_code='')
		{
			...
			$this->p_kc_session_id = uniqid() . '-3.0.0.012';
...
class plgSystemKeyCAPTCHA extends JPlugin
{
	...
	function getParamsForForm( $aextname, $atask, $aview, $forcheck=false, $content='' ) {
				...
				foreach($all_tags[0] as $k => $p) {					
					if ( preg_match( '/'.$at2[1].'/i', $p ) ) {						
						...
						if ( preg_match_all( '/ id=(["\'].+?["\'])/i', $p, $atmp ) ) {
							...
							$np = str_replace('<'.$at2[0].' ','<'.$at2[0].' id="kc_submit_but'.$bn.'" ', $p);
							if ( $captcha_before == $p ) {
								$captcha_before = $np;
							}
							$content = str_replace($p,$np,$content);
							$all_submit_names[] = 'kc_submit_but'.$bn;
							$bn++;
						}
					}
				}
		...
		$kc_o = new KeyCAPTCHA_CLASS($this->params->get('keycaptcha_site_private_key'), str_replace( '#BUTTONS#', implode( ',', $all_submit_names ), $this->params->get('keycaptcha_code') ) );
